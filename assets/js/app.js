...
function renderTimeline(host, tracks, opt = {}) {
  ...
  for (let t = d0; t <= d1; t += step) {
    ...
    if (p < 99.5) ticks += `<span class="tl-lb" style="left:${p}%">${tlHM(d)}</span>`;
  ...
  host.innerHTML = `
    ...
    <div class="tl-foot">
      <div class="tf-l">
        ...
        <button class="btn-icon" data-tledit title="경로 편집"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><path d="M3 13l.6-2.6L10.4 3.6a1.1 1.1 0 011.6 0l.4.4a1.1 1.1 0 010 1.6L5.6 12.4z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/><path d="M9.4 4.6l2 2" stroke="currentColor" stroke-width="1.2"/></svg></button>
        <button class="btn-icon${tlEditOn() ? ' on' : ''}" data-tlvedit title="영상 클립 편집"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="4" cy="4.5" r="2" fill="none" stroke="currentColor" stroke-width="1.2"/><circle cx="4" cy="11.5" r="2" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M5.7 5.6L14 12.5M5.7 10.4L14 3.5" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></button>
      </div>
      <div class="tf-zoom" title="타임라인 확대/축소">
        <button class="btn-icon zb" data-tlz="out" title="축소"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="7" cy="7" r="4.6" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M10.4 10.4L14 14M5 7h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></button>
        <input type="range" class="tl-zr" min="1" max="8" step="1" value="${TL.zoom}">
        <button class="btn-icon zb" data-tlz="in" title="확대"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="7" cy="7" r="4.6" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M10.4 10.4L14 14M5 7h4M7 5v4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></button>
      </div>
...
function ctrlHTML(opt = {}) {
  ...
  const left = `<div class="grp l${dim}">
    ...
    ${b('가상펜스 검색', 'i-tool-path')}
  </div>`;
  const center = `<div class="grp c">${playCtrlHTML()}</div>`;
  const right = `<div class="grp r">
    <span class="tg${dim}">${b('히트맵', 'i-tool-heatmap')}${b('인접 카메라', 'i-tool-cctv')}</span>
    <span class="tb-div${dim}"></span>
    <span class="tg"><button class="btn-icon vm1 on" title="단일 영상 view"><svg class="vmi" viewBox="0 0 24 24" aria-hidden="true"><g class="o"><rect x="3" y="5" width="15" height="11" rx="1.6" fill="none" stroke="currentColor" stroke-width="1.4"/></g><g class="f"><rect x="3" y="5" width="15" height="11" rx="1.6" fill="currentColor"/></g><circle class="cut" cx="18" cy="17" r="5"/><circle cx="18" cy="17" r="3.8" fill="none" stroke="currentColor" stroke-width="1.2"/><text x="18" y="19.1" text-anchor="middle" font-size="5.6" font-weight="700" fill="currentColor">1</text></svg></button>
      <div class="vm4"><button class="vm4-go" title="멀티 뷰"><svg class="vmi" viewBox="0 0 24 24" aria-hidden="true"><g class="o" fill="none" stroke="currentColor" stroke-width="1.3"><rect x="3" y="5" width="7" height="5" rx=".8"/><rect x="11.5" y="5" width="7" height="5" rx=".8"/><rect x="3" y="11.5" width="7" height="5" rx=".8"/><rect x="11.5" y="11.5" width="4.5" height="5" rx=".8"/></g><g class="f" fill="currentColor"><rect x="3" y="5" width="7" height="5" rx=".8"/><rect x="11.5" y="5" width="7" height="5" rx=".8"/><rect x="3" y="11.5" width="7" height="5" rx=".8"/><rect x="11.5" y="11.5" width="4.5" height="5" rx=".8"/></g><circle class="cut" cx="18" cy="17" r="5"/><circle cx="18" cy="17" r="3.8" fill="none" stroke="currentColor" stroke-width="1.2"/><text x="18" y="19.1" text-anchor="middle" font-size="5.6" font-weight="700" fill="currentColor">4</text></svg><span class="vm4-lb">동시 포착</span></button><button class="vm4-dd" title="영상 유형 선택"><i class="i i-16 i-chevron i-down caret"></i></button></div></span>
  </div>`;
...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609141828" alt="맵뷰">
...
const VH_BM = `<button class="tb" data-vbm title="북마크"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><path d="M4 2.5h8v11L8 10.8 4 13.5z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/></svg></button>`;
const VH_RATIO = `<button class="tb" data-vratio title="영상 비율 설정"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><rect x="1.8" y="3.2" width="12.4" height="9.6" rx="1.2" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M4.6 8h6.8M4.6 8l1.6-1.6M4.6 8l1.6 1.6M11.4 8L9.8 6.4M11.4 8L9.8 9.6" stroke="currentColor" stroke-width="1.2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg></button>`;
/* 사양 v0.8 : 영상 슬롯 머리말 = 북마크 · 영상 비율 · 영상 확대 */
const VH_ZOOM = `<button class="tb" data-vzoom title="영상 확대"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="7" cy="7" r="4.6" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M10.4 10.4L14 14M5 7h4M7 5v4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></button>`;
...
function bindCmpView() {
  ...
  vm4.querySelector('.vm4-go').onclick = () => { CMP.multi = true; cmpApplyMulti(); paint(); };
  vm4.querySelector('.vm4-dd').onclick = e => { e.stopPropagation(); if (menu) menu.hidden = !menu.hidden; };
  if (menu) menu.onclick = e => {
    e.stopPropagation();
    const row = e.target.closest('.vm4-row'), mt = e.target.closest('[data-mt]'), ms = e.target.closest('[data-ms]');
    if (!mt && !ms) { if (row) menu.querySelectorAll('.vm4-row').forEach(r => r.classList.toggle('open', r === row && !r.classList.contains('open'))); return; }
    if (mt) DT.mvType = mt.dataset.mt;
    if (ms) DT.mvSplit = +ms.dataset.ms;
    menu.hidden = true; menu.querySelectorAll('.vm4-row').forEach(r => r.classList.remove('open'));
    if (typeof syncVm4 === 'function') syncVm4();
    CMP.multi = true; cmpApplyMulti(); paint();
  };
  if (!bindCmpView._doc) { bindCmpView._doc = true;
    document.addEventListener('click', e => { if (!e.target.closest('#cmpCtrl .vm4')) document.querySelectorAll('#cmpCtrl .vm4-menu').forEach(m => m.hidden = true); }); }
  paint(); cmpApplyMulti();
}
...
function syncMapToCursor() {
  ...
  document.querySelectorAll('.dt-map, .pn-map').forEach(map => {
    ...
    const solid = map.querySelector('#dtMap3d:not([hidden]), #dtMapGL:not([hidden]), .zone-map:not([hidden]), .flat-stage.scaled');
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609141828" alt="">
      ${poly}
...
function renderMapLayers(paths, camName) {
  ...
  if (cctvOn) {
    $('#dtMapCones').innerHTML = MAP_CCTV.map((c, i) =>
      `<span class="map-cone ${coneType(c.cam)}" data-mc="${i}" title="${c.cam} · ${({ play: '현재 재생 카메라', hit: '대상 포착 카메라', miss: '대상 미포착 카메라' })[coneType(c.cam)]}" style="left:${c.x}%;top:${c.y}%;rotate:${c.deg}deg"><i></i><b></b></span>`).join('');
  ...
  $('#dtMapPath').innerHTML = `<svg viewBox="0 0 100 100" preserveAspectRatio="none">${fpaths.map(p => {
    ...
    for (let i = 1; i < q.length; i++) {
      ...
      d += `<path d="M${q[i - 1].x},${q[i - 1].y} L${q[i].x},${q[i].y}" stroke="${out ? 'var(--exit)' : slotColor(p.slot)}"
  ...
  $('#dtMapWps').innerHTML = fpaths.map((p, pi) => p.pts.map((t, ti) => {
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609141828' : 'assets/img/floor.png?v=202609141828';
...
(() => {
  ...
  menu.onclick = e => {
    e.stopPropagation();
    const row = e.target.closest('.vm4-row');
    const mt = e.target.closest('[data-mt]'), ms = e.target.closest('[data-ms]');
    if (!mt && !ms) { if (row) { menu.querySelectorAll('.vm4-row').forEach(r => r.classList.toggle('open', r === row && !r.classList.contains('open'))); } return; }
    if (mt) DT.mvType = mt.dataset.mt;
    if (ms) DT.mvSplit = +ms.dataset.ms;
    menu.hidden = true; menu.querySelectorAll('.vm4-row').forEach(r => r.classList.remove('open'));
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609141828' : 'assets/img/floor.png?v=202609141828';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609141828" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609141828" alt="외부 지도" draggable="false">
      ...
      ${groups.map(g => g.pts.slice().sort((a, b) => a.n - b.n).map(t => `<span class="map-wp zm-pt" data-pt="z-${g.slot || 'A'}-${t.n}" data-slot="${g.slot || ''}"${g.off ? ' data-off="1"' : ''} data-cam="${t.cam}" data-hh="${t.hh}" data-x="${t.x}" data-y="${t.y}"
        title="${t.cam} · 이 지점으로 이동" style="left:${t.x}%;top:${t.y}%;background:${col(g)}${g.off ? ';opacity:.3' : ''}">${t.n}</span>`).join('')).join('')}
...
function renderFloorPane(host, pts, color) {
  ...
  const flb = (M3_FLOORS.find(f => f.key === fl) || {}).label || fl;
  fp.innerHTML = `<img src="assets/img/floor.png?v=202609141828" alt=""><span class="dt-floor">${flb}</span>
    ...
    ${mine.map(t => `<span class="map-wp" data-cam="${t.cam}" data-hh="${t.hh}" data-x="${t.x}" data-y="${t.y}"
      title="${t.cam} · 이 지점으로 이동" style="left:${t.x}%;top:${t.y}%;background:${color}">${t.n}</span>`).join('')}`;
}
function renderSplitPane(paths) {
  ...
  if (!p2) {
    ...
    p2.innerHTML = `<div class="zp-head"><div class="select sm dt-bldg" id="dtBldg2" data-value="">
        <button class="select-btn">-<i class="i i-16 i-chevron i-down caret"></i></button><div class="select-menu"></div></div></div>
      <div class="zp-zoom"><button type="button" data-zz="1" title="확대">+</button><button type="button" data-zz="-1" title="축소">−</button></div>`;
    wrap.appendChild(p2);
...
function flatStage() {
  ...
  if (!st) {
    ...
    const img = document.getElementById('dtMapImg');
    img.before(st);
    ['dtMapImg', 'dtMapPath', 'dtMapCones', 'dtMapWps'].forEach(id => { const n = document.getElementById(id); if (n) st.appendChild(n); });
