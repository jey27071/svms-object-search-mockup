...
function renderResults() {
  ...
  $('#resCount').innerHTML = `총 <em class="rc-badge">${(aiOn && S.aiStage === 'done' ? aiList.length : S.results.length)}</em> 건`
    /* 와이어프레임 : 건수 옆 `결과 더보기 ⓘ` — 유사도 기준 아래 결과까지 더 불러오고 다시 정렬한다 */
    + (!aiOn && S.searched && S.results.length
      ? `<button type="button" class="rh-more" id="btnResMore" ${S.moreLoaded ? 'disabled' : ''}>결과 더보기<i class="rh-info" data-tip="더보기 시 순서가 조정될 수 있습니다.">i</i></button>` : '');
  { const mb = $('#btnResMore'); if (mb) mb.onclick = e => {
      if (e.target.closest('.rh-info')) return;
      S.moreLoaded = true; runSearch(true); toast('결과를 더 불러왔습니다. 순서가 조정될 수 있습니다.');
    }; }
  renderSortMenu();
...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609151707" alt="맵뷰">
...
function renderArea() {
  ...
  if (a.mode === 'line' && a.line) {
    ...
    if (a.done) {
      ...
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609151707" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609151707" alt="">
      ${poly}
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609151707' : 'assets/img/floor.png?v=202609151707';
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609151707' : 'assets/img/floor.png?v=202609151707';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609151707" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609151707" alt="외부 지도" draggable="false">
...
function renderFloorPane(host, pts, color) {
  ...
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609151707" alt=""><span class="dt-floor">${flb}</span>
