...
/* ===================== 유틸 ===================== */
/* 2026-09-16 구두 : **사양에 없는 토스트는 전부 끈다.** 거의 모든 조작에서 떠서 방해가 됐다.
   호출부(55곳)는 그대로 두고 여기서 한 번에 막는다 — 되살릴 땐 이 반환문만 지우면 된다. */
const TOAST_ON = false;
function toast(msg) {
  if (!TOAST_ON) return;
  /* 사양 v0.8 : 화면 중앙 하단 · 체크 아이콘 + 닫기 */
  const t = $('#toast');
  t.innerHTML = `<i class="tt-ok" aria-hidden="true"><svg viewBox="0 0 16 16"><circle cx="8" cy="8" r="7" fill="currentColor"/><path d="M4.8 8.2l2.1 2.1 4.3-4.4" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></i><span></span><button type="button" class="tt-x" title="닫기">✕</button>`;
  t.querySelector('span').textContent = msg;
  t.querySelector('.tt-x').onclick = () => { t.hidden = true; };
  t.hidden = false;
...
const ALERT = {
  chatDel:    { title: '대화 목록 삭제', desc: '선택한 대화 목록이 영구적으로 삭제됩니다.<br>삭제하시겠습니까?',                              ok: '삭제', danger: true  },
...
function tabTitleHTML(t) {
  ...
  let nm = cam ? String(t.title).slice(0, -cam.length - 1) : t.title;
  /* 대상명이 따로 없으면 위치명이 이름 자리에도 들어와 `외부 CCTV 외부 CCTV` 처럼 두 번 보였다 — 한 번만 */
  const dup = cam && nm.trim() === cam;
  if (dup) nm = cam;
  const tip = dup ? cam : t.title;
  return `<span class="tt" title="${tip}"><b class="tt-n">${nm}</b>${cam && !dup ? `<em class="tt-c">${cam}</em>` : ''}</span>`;
}
function renderTabs() {
  const box = $('#tabs'); box.innerHTML = '';
  /* `검색 결과` 탭은 검색 패널이 안 보일 때 늘 첫 탭으로 남는다 —
     시안 5126:58065 처럼 상세 화면에서도 이 탭으로 목록에 돌아간다.
     펼침 버튼은 **검색 화면에서 접었을 때만** 붙는다.
     검색 상세에는 검색결과 패널을 불러오는 기능이 없다. */
  const onSearch = S.activeTab === 'search';
  if (S._collapsed || !onSearch) {
    const st = el('div', 'tab tab-search' + (onSearch ? ' on' : ''));
    st.innerHTML = `<span>검색 결과</span>` + (S._collapsed && onSearch
      ? `<button class="tx" title="검색 패널 펼치기" aria-label="검색 패널 펼치기">
    ...
    const tx = st.querySelector('.tx');
    if (tx) tx.onclick = e => { e.stopPropagation(); setCollapsed(false); };
    st.onclick = e => {
      if (e.target.closest('.tx')) return;
      S.activeTab = 'search'; renderTabs(); syncPanels();
    };
    box.appendChild(st);
  }
  S.tabs.forEach(t => {
    ...
    n.onclick = e => {
      if (e.target.closest('.x')) {
        ...
        else if (!S.noAskTabClose) {
          /* 사양 v0.8 공통 팝업 8 : 탭 닫기 alert (다시 묻지 않기) */
          alertBox({ title: '탭 닫기', ok: '닫기', danger: false,
            desc: `선택한 탭을 닫을까요?<br>사건으로 등록되지 않은 검색 기록은 히스토리에 최근 50개까지 보관되며,<br>이후에는 다시 확인할 수 없습니다.<label class="check sm al-noask"><input type="checkbox" id="alNoAsk"><i></i>다시 묻지 않기</label>`,
            onOk: () => {
              const na = document.getElementById('alNoAsk');
              if (na && na.checked) { S.noAskTabClose = true; try { localStorage.setItem('svms.noAskTabClose', '1'); } catch (_) {} }
...
function syncPanels() {
  ...
  if (swapAreas.st && swapAreas.st.el !== swapTarget()) swapRestore();
  const isCmp = !isSearch && !!t && t.kind === 'compare';
  const isDetail = !isSearch && !!t && !!t.obj && !isCmp;
  $('#detail').hidden = !(isDetail || isCmp);
  /* 사양 : 상세화면 진입 시 LNB 미노출 */
  document.body.classList.toggle('in-detail', isDetail || isCmp);
...
/* 시안 1-1안(Solid/요청사항 반영) : 위치 → 기간 → 유사도 순.
   색상 필터는 시안에서 빠졌다. */
const FILTER_DEF = {
  /* 순서는 GUI 시안 260915(5295:84020) : 카메라(위치) · **기간 · 유사도** · 색상.
     종전엔 와이어프레임 4862:108451 을 따라 유사도가 기간보다 위였다(2026-09-15 구두로 교체). */
  text:   ['cam', 'period', 'sim', 'color'],
  /* 인물 검색은 이미지 검출·등록 인물 두 방식을 함께 쓰므로 유사도가 붙는다 */
  person: ['cam', 'period', 'sim'],
  algo:   ['cam', 'period', 'sim'],
  /* 차량번호 : 차종 필터 삭제, 유사도 · 색상(차량) 추가 */
  car:    ['cam', 'period', 'sim', 'color'],
  aim:    []
};
...
/* 트리에서 체크가 바뀔 때 머리말 요약을 즉시 고쳐 쓴다 */
function syncCamSum() {
  const el = document.querySelector('.acc[data-acc="cam"] .acc-sum');
  if (el) el.innerHTML = accSumHTML('cam');
}
/* 와이어프레임 : 카메라(위치) 배지 `N개 선택 ✕` — ✕ 로 선택 해제.
   많이 고르면(CAM_SLOW 이상) 검색 시간이 길어질 수 있다고 알려 준다 */
const CAM_SLOW = 5;
function accSumHTML(key) {
  const t = accSummary(key);
  if (key !== 'cam' || !(S.cams || []).length) return t;
  /* 2026-09-16 구두 : 칩의 `ⓘ` 표시 삭제. 느린 검색 안내 자체는 사양에 있으니
     아래 생성 코드는 남겨 두고 호출만 끊는다(되살릴 땐 slow 를 다시 붙이면 된다). */
  const slow = false && S.cams.length >= CAM_SLOW;
  /* 2026-09-16 구두 : 칩의 ✕(선택 해제) 삭제 */
  return `${t}` + (false ? `<i class="acc-x" data-camclear role="button" title="선택 해제">✕</i>` : '')
    + (slow ? `<i class="acc-warn" title="${CAM_SLOW}개 이상 선택 시 검색 완료까지 다소 시간이 걸릴 수 있습니다.">i</i>` : '');
}

function accSummary(key) {
  if (key === 'sim') return `${S.sim}% 이상`;
  if (key === 'period') return S.period || '오늘';
  if (key === 'cam') {
  ...
  if (key === 'color' && S.mode === 'car') {
    const k = S.carColor || [];
    return k.length ? k.map(x => (COLORS.find(c => c.k === x) || {}).label).filter(Boolean).join('·') : '전체';
  }
  if (key === 'color') {
...
function accBlock(key, title, bodyHTML, open) {
  return `<div class="acc${open ? ' open' : ''}" data-acc="${key}">
    <button class="acc-head">${title}<em class="acc-sum">${accSumHTML(key)}</em>${ICON.caret}</button>
...
function camTreeHTML() {
  ...
  return `<div class="cam-wrap">
    <div class="ct-search"><input id="fCamQ" placeholder="검색" value="${S.camQ || ''}"><i class="i i-24 i-g-find"></i></div>
...
function buildFilters(mode) {
  ...
  const parts = [];
  /* 펼침 기본값은 카메라(위치) 하나. 다만 사용자가 연 섹션은 **기억**한다 —
     슬라이더를 놓을 때마다 다시 그리는데, 기본값만 보면 그때 닫혀 버린다. */
  if (!S.accOpen) S.accOpen = new Set(['cam']);
  const openOf = f => on && S.accOpen.has(f);
  for (const f of FILTER_DEF[mode]) {
    ...
    if (f === 'color' && mode === 'car') parts.push(accBlock('color', '색상', `
    ...
    else if (f === 'color') parts.push(accBlock('color', '색상', `
      ...
      <div class="color-row">${COLORS.map(c => `<span class="sw${S.bottom.includes(c.k) ? ' on' : ''}" data-part="bottom" data-c="${c.k}" style="background:${c.hex}" title="${c.label}"></span>`).join('')}</div>`, openOf('color')));
    ...
    if (f === 'period') parts.push(accBlock('period', '기간', `
      ...
      <div class="date-wrap">
        <div class="date-grp"><span class="lb">시작일</span>
          <div class="date-row">
            <span class="date-fld"><input type="date" id="dFrom" value="${S.dFrom}" placeholder="YY.MM.DD">${ICON.cal}</span>
            <span class="date-fld"><input type="time" id="tFrom" value="${S.tFrom}" placeholder="시간 선택">${ICON.clock}</span>
          </div></div>
        <div class="date-grp"><span class="lb">종료일</span>
          <div class="date-row">
            <span class="date-fld"><input type="date" id="dTo" value="${S.dTo}" placeholder="YY.MM.DD">${ICON.cal}</span>
            <span class="date-fld"><input type="time" id="tTo" value="${S.tTo}" placeholder="시간 선택">${ICON.clock}</span>
          </div></div>
      </div>`, openOf('period')));
  }
...
function bindFilters(box) {
  $$('.acc-head', box).forEach(h => h.onclick = () => {
    const acc = h.parentElement;
    const open = acc.classList.toggle('open');
    const k = acc.dataset.acc;
    if (!S.accOpen) S.accOpen = new Set(['cam']);
    open ? S.accOpen.add(k) : S.accOpen.delete(k);
  });
  ...
  $$('[data-cam]', box).forEach(cb => cb.onchange = () => {
    ...
    savePrefs(); buildFilters(S.mode); markDirty();
  });
  $$('.sw', box).forEach(s => s.onclick = () => {
    const p = s.dataset.part, c = s.dataset.c;
    S[p] = S[p].includes(c) ? S[p].filter(x => x !== c) : [...S[p], c];
    s.classList.toggle('on'); savePrefs(); markDirty();
    /* 접이 머리말 요약도 바로 갱신 (예전엔 다시 그릴 때까지 `전체` 로 남았다) */
    const sm = box.querySelector('.acc[data-acc="color"] .acc-sum'); if (sm) sm.innerHTML = accSumHTML('color');
  });
  $$('[name=fp]', box).forEach(r => r.onchange = () => {
    S.period = r.dataset.p;
    const w = $('.date-wrap', box); if (w) w.hidden = S.period !== '날짜 지정';
    const sm = box.querySelector('.acc[data-acc="period"] .acc-sum'); if (sm) sm.innerHTML = accSumHTML('period');
    savePrefs(); markDirty();
  });
  ['dFrom', 'tFrom', 'dTo', 'tTo'].forEach(id => { const n = $('#' + id, box); if (n) n.onchange = () => { S[id] = n.value; markDirty(); }; });
}
...
/* 좌측 패널은 필터가 아니라 **검색 조건**이다. 값을 바꿔도 결과가 바로
   갈리지 않고, `검색` 을 다시 눌러야 반영된다 (2026-09-10 사양). */
function markDirty() {
  if (!S.searched) { syncSearchBtn(); return; }
  if (!S._dirty) {
    S._dirty = true;
    toast('검색 조건이 바뀌었습니다. 다시 검색해 주세요.');
  }
  syncSearchBtn();
}
...
function syncSearchBtn() {
  ...
  /* 검색 뒤에는 조건이 바뀌어야 다시 누를 수 있다 */
  btn.disabled = noCam || (S.searched && !S._dirty);
...
S.view = 'thumb';
/* 기본 카드 폭 — 시안 5295:84013 은 1507 폭에 208.43 카드 7열(간격 8).
   종전 264(5열)는 "7열까지 벌어진다"는 이유로 넓혀 둔 값이었는데,
   갱신된 시안이 7열이라 2026-09-15 구두로 되돌렸다. */
S.thumbW = 208;
function applyView() {
...
(function initViewCtl() {
  document.querySelectorAll('#viewSeg [data-view]').forEach(b => b.onclick = () => {
    ...
    /* 카드 이미지가 보기 모드에 따라 달라진다(크롭 ↔ 원본) —
       클래스만 바꾸면 이전 이미지가 남아 다음 조작 때야 반영됐다. */
...
/* 크기 슬라이더 양 끝의 산 아이콘 — 누르면 한 단계씩 움직인다.
   `label` 안에 있어 클릭이 입력까지 이어지지 않아 핸들로만 조작됐다.
   결과 목록(#thumbSize)과 영상 선택(#clipsSize) 모두 여기서 처리한다 —
   후자는 다시 그려지므로 개별 바인딩 대신 문서에 위임한다. */
document.addEventListener('click', e => {
  const ic = e.target.closest('.size-slider i'); if (!ic) return;
  const wrap = ic.closest('.size-slider');
  const r = wrap && wrap.querySelector('input[type=range]'); if (!r) return;
  e.preventDefault(); e.stopPropagation();
  const step = +r.step || 1, min = +r.min, max = +r.max;
  const dir = ic.classList.contains('ss-max') ? 1 : -1;
  const next = Math.min(max, Math.max(min, +r.value + dir * step));
  if (next === +r.value) return;
  r.value = next;
  r.dispatchEvent(new Event('input', { bubbles: true }));
});

/* 시안(5295:84026)의 트랙은 왼쪽(작은 쪽)이 밝게 채워져 있다.
   ::-webkit-slider-runnable-track 을 직접 그리면 accent-color 가 죽어서
   채움 비율을 CSS 변수(--pct)로 넘겨 그라디언트로 칠한다.
   팝업 슬라이더(#clipsSize 등)는 나중에 그려지므로 추가도 함께 본다. */
function syncSliderFill(r) {
  const min = +r.min || 0, max = +r.max || 100;
  const pct = max === min ? 0 : ((+r.value - min) / (max - min)) * 100;
  r.style.setProperty('--pct', pct.toFixed(2) + '%');
}
function syncAllSliderFills(root) {
  (root || document).querySelectorAll('.size-slider input[type=range]').forEach(syncSliderFill);
}
document.addEventListener('input', e => {
  const r = e.target.closest && e.target.closest('.size-slider input[type=range]');
  if (r) syncSliderFill(r);
});
new MutationObserver(ms => {
  for (const m of ms) for (const n of m.addedNodes) {
    if (n.nodeType !== 1) continue;
    if (n.matches && n.matches('.size-slider input[type=range]')) syncSliderFill(n);
    else syncAllSliderFills(n);
  }
}).observe(document.documentElement, { childList: true, subtree: true });
syncAllSliderFills();

/* ── 결과 목록 하단 : 끝에 닿으면 자동으로 더 불러온다 (2026-09-15 구두) ────────
   종전엔 머리 줄의 `결과 더보기 ⓘ` 를 눌러야 유사도 기준 아래(-15%p) 결과까지 불러왔다.
   버튼을 없애고 **목록 맨 아래**에 상태 줄을 두고, 스크롤이 끝에 닿으면 알아서 부른다.
   더 부를 게 남았는지는 종전과 같은 `S.moreLoaded` 한 단계뿐이다. */
function resMoreState() {
  if (S.aiMode || S.mode === 'aim') return 'none';
  if (!S.searched || !S.results.length) return 'none';
  if (S._moreBusy) return 'loading';
  return S.moreLoaded ? 'done' : 'idle';
}
function renderResMore() {
  const body = document.getElementById('resultsBody'); if (!body) return;
  const st = resMoreState();
  let f = body.querySelector('.res-foot');
  /* 아직 안 불러온 상태(idle)는 아무것도 보이지 않는다 —
     스피너는 **실제로 부르는 동안만**, 끝나면 다 불러왔다고 알린다 */
  if (st === 'none' || st === 'idle') { if (f) f.remove(); return; }
  if (!f) f = el('div', 'res-foot');
  body.appendChild(f);                     /* 다시 그려도 항상 목록 맨 아래로 */
  f.className = 'res-foot ' + st;
  f.innerHTML = st === 'loading'
    ? `<i class="rf-wait"></i><span>결과를 더 불러오는 중…</span>`
    : `<span>모든 결과를 불러왔습니다.</span>`;
}
let resMoreCheck = () => {};
{
  const near = b => b.scrollTop + b.clientHeight >= b.scrollHeight - 160;
  const tryMore = () => {
    const b = document.getElementById('resultsBody');
    if (!b || resMoreState() !== 'idle') return;
    /* 스크롤이 아예 생기지 않을 만큼 결과가 적으면 기다릴 것 없이 바로 채운다 */
    const scrollable = b.scrollHeight > b.clientHeight + 4;
    if (scrollable && !near(b)) return;
    S._moreBusy = true; renderResMore();
    /* 실제 서버라면 응답을 기다리는 구간 — 목업은 잠깐 뒤에 채운다 */
    setTimeout(() => {
      /* 다시 그리면 목록 innerHTML 이 통째로 바뀌어 스크롤이 맨 위로 튄다 — 보던 위치를 지킨다 */
      const keep = b.scrollTop;
      S._moreBusy = false; S.moreLoaded = true; runSearch(true);
      const b2 = document.getElementById('resultsBody'); if (b2) b2.scrollTop = keep;
    }, 400);
  };
  document.addEventListener('scroll', e => {
    if (e.target && e.target.id === 'resultsBody') tryMore();
  }, true);
  window.addEventListener('resize', tryMore);
  /* 결과를 새로 그린 직후에도 한 번 본다(첫 화면이 이미 끝까지 차 있는 경우) */
  resMoreCheck = tryMore;
}

/* ===================== 검색 실행 ===================== */
function matchFilters(o) {
  if (o.sim < S.sim - (S.moreLoaded ? 15 : 0)) return false;
  if (S.cams.length && S.cams.length < CAMERAS.length && !S.cams.includes(o.cam)) return false;
  if (FILTER_DEF[S.mode].includes('color') && S.mode !== 'car') {
...
function runSearch(keep) {
  ...
  S.searched = true;
  if (!keep) S.moreLoaded = false;   /* 새 검색이면 더보기 해제 */
  S._dirty = false;                 /* 검색을 눌렀으니 조건이 반영됐다 */
...
function sortOptions() {
  /* 와이어프레임 4862:108451 : 차량번호 = 최신순·위치순·번호순 / 지능형 = 이름순·최신순·위치순 /
     텍스트·인물 = 유사도순·최신순·위치순 (인물순 삭제) */
  if (S.mode === 'car')  return ['최신순', '위치순', '번호순'];
  if (S.mode === 'algo') return ['이름순', '최신순', '위치순'];
  return ['유사도순', '최신순', '위치순'];
}
function renderSortMenu() {
  const m = document.getElementById('sortMenu'); if (!m) return;
  const opts = sortOptions();
  /* 방식을 바꾸면 그 방식의 기본 정렬(첫 항목)로 */
  if (S._sortMode !== S.mode) { S._sortMode = S.mode; S.sort = opts[0]; }
  if (!opts.includes(S.sort)) S.sort = opts[0];
  m.innerHTML = opts.map(v => `<div data-v="${v}"${v === S.sort ? ' class="on"' : ''}>${v}</div>`).join('');
  const btn = document.querySelector('#sortSelect .select-btn');
  /* chevron 은 시안(5295:84026) 기준 24px */
  if (btn) btn.innerHTML = `${S.sort}<i class="i i-24 i-chevron i-down caret"></i>`;
...
function sortResults() {
  const c = {
    ...
    '번호순':   (a, b) => plateOf(a).localeCompare(plateOf(b)),
    '이름순':   (a, b) => cardTitle(a).localeCompare(cardTitle(b)) || b.t.localeCompare(a.t)
  };
...
function cardTitle(o) {
  if (S.mode === 'person') {
    ...
    const sels = S.selPersons || [];
    const p = sels.length ? (S.persons || []).find(x => x.id === sels[objHash(o) % sels.length]) : null;
    if (p) return p.name;
    /* 이미지 검색 : 고른 검출 인물 수만큼만 이름을 붙여 묶는다 (결과마다 따로 묶이던 문제) */
    const nSel = Math.max(1, (S.extSel || []).length);
    return autoPersonName(objHash(o) % nSel);
  }
  if (S.mode === 'algo') return (S.algos && S.algos.length) ? S.algos[objHash(o) % S.algos.length] : o.cam;
  if (S.mode === 'car') return plateOf(o);
  return o.cam;
}
/* 목업 : 결과마다 고정된 값을 뽑기 위한 해시 */
function objHash(o) { let h = 0; for (const ch of String(o.id)) h = (h * 31 + ch.charCodeAt(0)) >>> 0; return h; }
/* 차량번호 목업 — 입력한 번호 일부가 들어가도록 */
function plateOf(o) {
  const h = objHash(o), han = '가나다라마거너더러머버서어저고노도로모보소오조구누두루무부수우주하허호';
  const q = String(S.carQ || '').replace(/\s/g, '').replace(/[^0-9]/g, '').slice(0, 4);
  let back = String(1000 + (h % 9000));
  if (q) back = (q + back).slice(0, 4);
  return `${10 + (h % 290)}${han[h % han.length]} ${back}`;
}
...
function cardHTML(o, opt = {}) {
  ...
  return `<div class="card${S.selected === o.id ? ' on' : ''}" data-id="${o.id}">
    <div class="thumb">
      <!-- 원본 비율 보기에서는 잘라낸 카드가 아니라 **원본 영상**을 쓴다 -->
      <img src="${S.view === 'raw' ? srcVideo(o.cam) : o.img}" alt="">
      ...
      <!-- 썸네일 안 북마크 표시는 삭제 — 캡션의 북마크 버튼(.cbk)과 중복이었다.
           (2026-09-16 구두 : 북마크 누르면 위에 노란 게 또 생김 / 시안 5402:189712 도 캡션 한 곳뿐) -->
    </div>
    <div class="meta">
      <div><div class="loc">${cardTitle(o)}</div><div class="tm">${['car', 'algo', 'person'].includes(S.mode) ? `<span class="tm-cam">${o.cam}</span>` : ''}${fmtT(o.t)}</div></div>
      <!-- 와이어프레임 : 카드 하단에 북마크를 둔다 -->
      <button class="cbk${bm ? ' on' : ''}" data-cbk="${o.id}" title="북마크">${ICON.bmark}</button>
      <!-- ⋯ 는 지능형 알고리즘 검색 결과에만 (2026-09-15 구두).
           그 밖의 검색 유형은 북마크 아이콘만 둔다 -->
      ${S.mode === 'algo' ? `<button class="more" data-more="${o.id}">${ICON.more}</button>` : ''}
    </div>
  </div>`;
}

/* 확대 보기 : 5열 → 3열 로 줄여 카드를 키운다 */
function applyResultView(v) {
  const body = document.getElementById('resultsBody');
  if (body) body.classList.toggle('zoomed', v === 'raw');
}

/* 정렬 기준이 **묶을 수 있는 값**이면 구분선과 머리말로 나눠 보여준다.
   유사도순은 연속값이라 나눌 것이 없어 평면 목록 그대로 둔다. */
const RES_GROUP_BY = {
  '최신순': o => String(o.t).slice(0, 10),           /* 날짜 */
  '위치순': o => o.cam                                /* 카메라(위치) */
};
function groupedResultsHTML(list) {
  /* 와이어프레임 4862:108451 : 차량번호 = 날짜별 / 인물 = 인물별 / 지능형 = 알고리즘별로 묶고 머리말(▲ 이름 (N))로 접는다 */
  const byMode = { car: o => String(o.t).slice(0, 10), person: o => cardTitle(o), algo: o => cardTitle(o) }[S.mode];
  const keyOf = byMode || RES_GROUP_BY[S.sort];
  if (!keyOf) return `<div class="grid">${list.map(o => cardHTML(o)).join('')}</div>`;

  const groups = [];
  if (byMode && S.mode !== 'car') {
    /* 인물·알고리즘은 정렬과 무관하게 같은 이름끼리 한 묶음 (처음 나온 순서) */
    list.forEach(o => { const k = keyOf(o); let g = groups.find(x => x.k === k); if (!g) groups.push(g = { k, items: [] }); g.items.push(o); });
  } else {
    /* 정렬은 이미 끝난 상태라 순서를 지키며 이어지는 것끼리 묶는다 */
    list.forEach(o => {
      const k = keyOf(o), last = groups[groups.length - 1];
      if (last && last.k === k) last.items.push(o); else groups.push({ k, items: [o] });
    });
  }
  if (!S.resClosed) S.resClosed = new Set();
  return groups.map(g => {
    const closed = S.resClosed.has(S.mode + ':' + g.k);
    return `<section class="res-group${closed ? ' closed' : ''}">
      <header class="res-gh" data-rgk="${String(g.k).replace(/"/g, '&quot;')}"><i class="rg-cv">${closed ? '▼' : '▲'}</i><b>${resGroupLabel(g.k)}</b><span class="n">(${g.items.length})</span></header>
      <div class="grid">${g.items.map(o => cardHTML(o)).join('')}</div>
    </section>`; }).join('');
}
/* 날짜 묶음에는 요일을 함께 — 며칠치인지 훑기 쉽다 */
function resGroupLabel(k) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(k)) return k;
  const w = ['일', '월', '화', '수', '목', '금', '토'][new Date(k + 'T00:00:00').getDay()];
  return `${k} (${w})`;
}
...
function renderResults() {
  ...
  if (S.activeTab !== 'search') {
    ...
    $('#resCount').innerHTML = `총 <em class="rc-badge">-</em> 건`;
    return;
  }
  ...
  /* 2026-09-15 구두 : 머리 줄의 `결과 더보기 ⓘ` 버튼을 빼고, 목록 **하단**에서
     끝에 닿으면 자동으로 더 불러온다(renderResMore · #resultsBody 스크롤).
     버튼 CSS(.rh-more · .rh-info)는 되돌릴 때를 위해 남겨 둔다. */
  $('#resCount').innerHTML = `총 <em class="rc-badge">${(aiOn && S.aiStage === 'done' ? aiList.length : S.results.length)}</em> 건`;
  renderSortMenu();
  ...
  if (true) {   /* 항상 평면 목록 (유사 대상별 보기 삭제) */
    ...
    body.innerHTML = groupedResultsHTML(S.results);
    $$('[data-rgk]', body).forEach(h => h.onclick = () => {
      const k = S.mode + ':' + h.dataset.rgk;
      S.resClosed.has(k) ? S.resClosed.delete(k) : S.resClosed.add(k);
      renderResults();
    });
    bindCards(body); applyView(); renderResMore();
    /* 첫 화면이 이미 끝까지 차 있으면 스크롤 없이도 다음 것을 부른다 */
    requestAnimationFrame(resMoreCheck);
    return;
  }
...
function pickSideHTML(opt) {
  const o = findObj(REID.seed) || OBJECTS[0];
  /* 시안(영상선택_default) : 좌측은 기준 대상 사진과 이름뿐이다 */
  return `
    <div class="vs-obj">
      <img src="${o.img}" alt="">
      <button class="vs-add" title="사진 추가">＋</button>
    </div>
    <!-- 시안 5329:111961 : 안내 문구는 상시 노출이 아니라 ⓘ 에 걸리는 툴팁이다 -->
    <label class="vs-name">
      <span>이름 <i class="q" data-tip="검색 인물을 구분할 이름을 입력하세요.">?</i></span>
      <input id="pkName" value="${seedName()}" placeholder="인물A" maxlength="20">
    </label>
    <!-- 와이어프레임 4862:108462 영상선택 : 카메라 · 기간 조건을 바꿔 재검색 -->
    <div class="vs-filter">
      <!-- 스크롤은 좌측 열 전체가 아니라 **카메라·기간 영역 안**에만 생긴다 (2026-09-16 구두).
           사진·이름과 재검색 버튼은 제자리에 두고 가운데만 굴린다. -->
      <div class="vf-scroll">
        <div class="vf-lb">카메라</div>
        <label class="check sm"><input type="checkbox" id="vsAllCam" ${REID.allCam !== false ? 'checked' : ''}><i></i>전체 카메라에서 검색</label>
        <!-- 시안 5329:111961 : 기간 옆 값 배지는 없다 (고른 라디오가 곧 값) -->
        <div class="vf-lb">기간</div>
        ${['오늘', '최근 3일', '최근 7일', '날짜 지정'].map(v => `<label class="radio sm"><input type="radio" name="vsPeriod" data-vsp="${v}" ${(REID.period || '오늘') === v ? 'checked' : ''}><i></i>${v}</label>`).join('')}
        <!-- 날짜 지정 을 고르면 시작·종료 입력을 편다 (2026-09-16 구두) -->
        <div class="vs-date"${(REID.period === '날짜 지정') ? '' : ' hidden'}>
          <label><span>시작일</span><input type="date" id="vsFrom" value="${REID.from || ''}"></label>
          <label><span>종료일</span><input type="date" id="vsTo" value="${REID.to || ''}"></label>
        </div>
      </div>
      <button type="button" class="btn-ghost sm vf-go" id="vsResearch"${vsDirty() ? '' : ' disabled'}>재검색</button>
    </div>`;
}
...
/* 재검색은 **기간(날짜) 이나 `전체 카메라에서 검색` 이 바뀌었을 때만** 활성 (2026-09-16 구두).
   이름 입력·정렬·크기 같은 다른 조작으로는 켜지지 않는다.
   `applied*` = 지금 화면에 걸려 있는 검색 조건(팝업을 열 때와 재검색 직후에 갱신). */
function vsDirty() {
  return (REID.allCam !== false) !== (REID.appliedAllCam !== false)
    || (REID.period || '오늘') !== (REID.appliedPeriod || '오늘');
}
function vsMarkApplied() { REID.appliedAllCam = REID.allCam; REID.appliedPeriod = REID.period || '오늘'; }
function syncVsResearch() { const b = document.getElementById('vsResearch'); if (b) b.disabled = !vsDirty(); }

function bindPickName() {
  const n = document.getElementById('pkName');
  if (n) n.oninput = e => { REID.name = e.target.value; };   /* 이름은 재검색 조건이 아니다 */
  const ac = document.getElementById('vsAllCam');
  if (ac) ac.onchange = () => { REID.allCam = ac.checked; syncVsResearch(); };
  document.querySelectorAll('[data-vsp]').forEach(r => r.onchange = () => {
    REID.period = r.dataset.vsp; const b = document.querySelector('.vs-filter .vf-badge'); if (b) b.textContent = REID.period;
    /* 날짜 지정 일 때만 시작·종료 입력을 편다 (2026-09-16 구두) */
    const d = document.querySelector('.vs-filter .vs-date'); if (d) d.hidden = REID.period !== '날짜 지정';
    syncVsResearch();
  });
  { const f = document.getElementById('vsFrom'), t = document.getElementById('vsTo');
    if (f) f.onchange = () => { REID.from = f.value; syncVsResearch(); };
    if (t) t.onchange = () => { REID.to = t.value; syncVsResearch(); }; }
  const go = document.getElementById('vsResearch');
  if (go) go.onclick = () => {
    if (go.disabled) return;
    REID.clipShown = Math.min(REID.clipTotal || 0, REID.clipMax || 64);
    if (typeof renderClips === 'function' && REID.clipAll) renderClips(REID.clipAll);
    toast(`${REID.allCam === false ? '선택한 카메라' : '전체 카메라'} · ${REID.period || '오늘'} 조건으로 다시 검색했습니다.`);
    vsMarkApplied(); syncVsResearch();   /* 반영했으니 다시 비활성 */
  };
  syncVsResearch();
}

/* 필터에 따라 카드에서 어떤 정보를 앞세울지 결정한다 */
function pickCardHTML(o, checked, sort, n) {
  /* 시안 5402:189952 : 유사도는 **썸네일 우상단 배지**, 캡션은 카메라명 + 일시 (2026-09-16 구두).
     종전에는 정렬에 따라 캡션 앞머리가 바뀌고 유사도순일 때 배지를 숨겼다.
     `n` 이 오면 체크박스 옆에 **하단 타임라인과 같은 번호**를 단다 (2026-09-18 구두).
     번호를 넘겨주지 않는 화면(RE-ID 선별)에서는 종전대로 배지가 없다. */
  return `<label class="reid-card${checked ? ' on' : ''}">
    ...
    <div class="rc-th"><img src="${o.img}" alt="">
      ${n ? `<span class="rc-n">${n}</span>` : ''}
      <span class="sim ${simCls(o.sim)}">${o.sim}%</span></div>
    <div class="rc-meta">
      <div class="rc-lead">${o.cam}</div>
      <div class="rc-sub">${fmtT(o.t)}</div>
    </div>
  </label>`;
}
...
/* 그룹 구분자 + 카드 */
/* `nums` = 대상 id → 번호 Map (넘겨주면 카드에 번호 배지를 단다. 없으면 종전대로) */
function pickGridHTML(list, selSet, sort, w, byDayHalf, nums) {
  const numOf = o => nums && nums.get(o.id);
  const sorted = list.slice().sort(pickSortFn(sort));
  /* 유사도순은 **묶지 않는다** — 연속값이라 구간을 나눌 자리가 없다.
     디바이더 없이 유사도 높은 순서로만 죽 뿌린다 (2026-09-16 구두).
     날짜(최신순)·위치순은 묶음 구분자를 그대로 쓴다. */
  if (sort === '유사도순') {
    return `<div class="reid-grid" style="--rw:${w}px">${sorted.map(o =>
      pickCardHTML(o, selSet.has(o.id), sort, numOf(o))).join('')}</div>`;
  }
  const groups = [];
  sorted.forEach(o => {
...
function pickToolsHTML(sort, allOn, id) {
  /* 시안 : 좌측 `n/64` 배지 · 우측 정렬 · 크기 슬라이더 · 보기 전환 */
  const sel = id === 'clips' ? REID.clipSel.size : REID.sel.size;
  const tot = id === 'clips' ? (REID.clipTotal || 0) : reidPool().length;
  return `
    <span class="vs-count">총 ${tot}건</span>
    <div class="vs-right">
      <!-- 시안 5402:189767 : 오른쪽 묶음은 **슬라이더 → 정렬 → 썸네일/원본** 순이다
           (구분선 5402:189763 은 시안에서 숨김) — 종전에는 정렬이 앞에 있었다 -->
      <label class="size-slider" title="썸네일 크기">
        <i class="i i-18 i-image ss-min"></i>
        <input type="range" id="${id}Size" min="110" max="240" step="10" value="${REID.clipW || 180}">
        <i class="i i-24 i-image ss-max"></i>
      </label>
      ...
      <!-- 시안 5329:111961 : 보기 전환은 아이콘이 아니라 썸네일 / 원본 글자 세그먼트 -->
      <div class="seg vs-seg">
        <button class="on" data-vsview="썸네일">썸네일</button>
        <button data-vsview="원본">원본</button>
      </div>
    </div>`;
}
...
function openReid(seedId) {
  ...
  REID.clipMode = ''; REID.clipDone = null;
  /* 팝업을 열면 지금 조건이 곧 적용된 조건 — 재검색은 비활성으로 시작한다 */
  vsMarkApplied();
...
function renderClipPreview() {
  ...
  if (!picked.length) {
    /* 시안 5402:189801 Timeline : 제목도 점선 상자도 없이 **가운데 한 줄**뿐이다
       (2026-09-16 구두 — `이동 경로 미리보기` 타이틀이 남아 있다는 지적) */
    host.innerHTML = `<div class="cp-empty">영상을 선택해 주세요.</div>`;
    return;
  }

  /* 2026-09-18 구두 : 하단을 **상세 화면 타임라인과 같은 구조**로 바꿨다.
     눈금(날짜 + 시각) → 레인 → 썸네일. **인물 칩(.tl-obj)은 그리지 않는다.**
     이 팝업엔 클립 길이 정보(to)가 없어 폭으로 길이를 나타낼 수 없으므로
     **시작 시각 위치에 고정 폭 썸네일**을 놓는다(폭은 CSS `#cpTl .tl-th` = 101px,
     필름 타일 1장). 위에 얹는 8px 막대도 같은 고정 폭 — 길이가 아니라 시작 지점 표시다.
     썸네일 디자인(필름 반복·흰 테두리 원형 번호·위치명)은 타임라인 것을 그대로 쓴다:
     style.css 의 `:is(#dtTl,#cmpTl)` 규칙에 `#cpTl` 을 함께 묶어 두었다.
     재생 커서(.tl-cursor)는 없다 — 이 팝업은 재생을 하지 않는다.
     (종전 `.cp-scroll/.cp-inner/.cp-node` 배치와 fitClipPreview 의 √간격 압축은 폐기.
      되돌릴 때를 위해 fitClipPreview 함수와 .cp-* 스타일은 남겨 둔다) */
  /* ★눈금 범위는 **고른 클립 전체를 감싸야** 한다 (2026-09-18 지적).
     상세 화면의 tlDomain 은 한 사람의 하루를 보는 화면이라 늘 `하루`(08:00~다음날 08:00)만
     돌려준다. 그런데 이 팝업은 여러 날에 걸친 영상을 함께 고를 수 있어서(목록에 06-28·
     06-29·06-30 이 섞여 있다) 하루로 잘라 두면 범위를 벗어난 클립의 퍼센트가 100% 를
     넘어 **눈금 오른쪽 밖**에 놓였다. 그래서 여기서는 고른 것의 처음~끝을 덮는
     **날 수만큼** 넓힌다(하루 경계는 상세와 같은 08:00 기준). */
  const CP_DAY = 864e5;
  const cpDayStart = ms => {
    const d = new Date(ms); d.setHours(TL_DAY_START, 0, 0, 0);
    if (ms < +d) d.setDate(d.getDate() - 1);
    return +d;
  };
  const cpTs = picked.map(o => +tlTime(o.t));
  const d0 = cpDayStart(Math.min(...cpTs));
  const d1 = cpDayStart(Math.max(...cpTs)) + CP_DAY;
  ...
  host.innerHTML = `
    ...
    <!-- 시안 경로 편집(5184:99829) : 미리보기 아래 확대/축소 -->
    <div class="cp-zoom">
      <button type="button" class="btn-icon" data-cpz="-1" title="축소"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="7" cy="7" r="4.6" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M10.4 10.4L14 14M5 7h4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></button>
      <input type="range" class="cp-zr" min="1" max="8" step="1" value="${CP_ZOOM}" aria-label="미리보기 확대">
  ...
  /* 미리보기에서 바로 빼기 — 목록의 체크와 선택 수도 함께 갱신된다 */
  host.querySelectorAll('[data-cprm]').forEach(b => b.onclick = e => {
    e.preventDefault(); e.stopPropagation();
    const id = b.dataset.cprm;
    REID.clipSel.delete(id);
    const o = findObj(id);
  ...
  /* 미리보기 썸네일을 누르면 **위 목록에서 그 클립을 포커스**한다 (2026-09-16 구두).
     ...
     강조는 타임라인의 `.tl-th.on` 을 그대로 쓴다(종전 `.cp-node.on`). */
  host.querySelectorAll('[data-cpn]').forEach(n => n.onclick = e => {
    ...
    host.querySelectorAll('.tl-th.on').forEach(x => x.classList.remove('on'));
    n.classList.add('on');
  ...
  /* 확대하면 가상 폭(.tl-inner width)이 넓어져 눈금 간격이 벌어지고 가로 스크롤로 넘겨 본다
     — 상세 타임라인의 TL.zoom 과 같은 방식. 눈금 촘촘함도 tlStep 이 다시 계산하므로 재렌더한다. */
  /* 배율 상한은 상세 타임라인(.tl-zr)과 같은 8 — 종전 4 였다.
     고른 클립이 1~2시간에 몰리면 24시간 눈금에서 고정 폭 썸네일이 서로 덮는데,
     8배까지 늘릴 수 있으면 그 경우에도 다 펼쳐 볼 수 있다 (2026-09-18). */
  const zr = host.querySelector('.cp-zr');
  const setZ = v => {
    const z = Math.max(1, Math.min(8, v));
    if (z === CP_ZOOM) return;
    CP_ZOOM = z; renderClipPreview();
  };
  if (zr) zr.oninput = e => setZ(+e.target.value);
  host.querySelectorAll('[data-cpz]').forEach(b => b.onclick = e => { e.preventDefault(); setZ(CP_ZOOM + +b.dataset.cpz); });
...
function fitClipPreview() {
  ...
  /* 확대하면 썸네일도 같이 커진다 (2026-09-16 구두) — CSS 가 `--cpz` 로 크기를 잡는다.
     간격(SLOT)도 썸네일 폭을 따라 벌려야 카드끼리 겹치지 않는다. */
...
/* ── 영상 선택 팝업 : 목록 끝에 닿으면 자동으로 이어 붙인다 (2026-09-15 구두) ──
   검색 결과 목록(renderResMore)과 같은 방식. 종전엔 하단 `더보기 (N개 남음)` 버튼이었다. */
function clipMoreFootHTML(total) {
  if (REID.clipShown >= total) {
    /* 처음부터 한 화면에 다 들어오면 굳이 알리지 않는다 */
    return total > (REID.clipMax || 12)
      ? `<div class="res-foot done"><span>모든 영상을 불러왔습니다.</span></div>` : '';
  }
  return REID._moreBusy
    ? `<div class="res-foot loading"><i class="rf-wait"></i><span>영상을 더 불러오는 중…</span></div>` : '';
}
function bindClipAutoMore(all) {
  const b = document.getElementById('clipsBody'); if (!b) return;
  const tryMore = () => {
    if (REID._moreBusy || REID.clipShown >= all.length) return;
    const scrollable = b.scrollHeight > b.clientHeight + 4;
    if (scrollable && b.scrollTop + b.clientHeight < b.scrollHeight - 160) return;
    REID._moreBusy = true;
    const foot = b.querySelector('.res-foot');
    const html = `<div class="res-foot loading"><i class="rf-wait"></i><span>영상을 더 불러오는 중…</span></div>`;
    if (foot) foot.outerHTML = html; else b.insertAdjacentHTML('beforeend', html);
    setTimeout(() => {
      /* 다시 그리면 innerHTML 이 통째로 바뀌어 스크롤이 맨 위로 튄다 — 보던 위치를 지킨다 */
      const keep = b.scrollTop;
      REID._moreBusy = false; REID.clipShown += (REID.clipMax || 12); renderClips(all);
      const b2 = document.getElementById('clipsBody'); if (b2) b2.scrollTop = keep;
    }, 400);
  };
  b.onscroll = tryMore;
  requestAnimationFrame(tryMore);   /* 첫 화면이 이미 꽉 차 있으면 바로 이어 붙인다 */
}

function renderClips(all) {
  if (REID.clipMode === 'path') return renderClipsPath(all);
  { const md = document.getElementById('mdClips'); if (md) md.classList.remove('pe', 'orig'); }
  REID.clipAll = all;
  REID.clipTotal = all.length;
  /* 클립 선택은 **날짜순이 기본** — 언제 찍힌 영상인지가 먼저 보여야 한다 */
  REID.clipSort = REID.clipSort || '유사도순';   /* 시안 기본값 (2026-09-16 구두) */
  REID.clipW = REID.clipW || 180;   /* 시안 5329:111961 : 한 줄 5개 (팝업 폭에서 카드 약 188) */
  ...
  /* 고른 영상은 카드에도 **하단 타임라인과 같은 번호**를 단다 (2026-09-18 구두).
     번호 기준은 타임라인과 똑같이 촬영 시각 순이므로, 목록 정렬을 바꿔도 번호는 그대로다. */
  const cpNums = new Map([...REID.clipSel].map(id => findObj(id)).filter(Boolean)
    .sort((a, b) => String(a.t).localeCompare(String(b.t)))
    .map((o, i) => [o.id, i + 1]));
  $('#clipsBody').innerHTML =
    /* 시안 5402:189781 : 목록 맨 위에 `전체 선택` 체크박스가 있다 (2026-09-16 구두) */
    `<label class="check sm vs-all"><input type="checkbox" id="clipsAll" ${allOn ? 'checked' : ''}><i></i>전체 선택</label>`
    + pickGridHTML(shown, REID.clipSel, sort, REID.clipW, true, cpNums)
    /* 2026-09-15 구두 : `더보기` 버튼을 누르지 않고 **끝에 닿으면 자동으로** 이어 붙인다.
       버튼 마크업은 아카이브(아래 주석) — CSS `.pk-more` 도 그대로 둔다.
       <div class="pk-more"><button class="btn-ghost" id="clipMore">더보기 (N개 남음)</button>
         <p class="hintline">한 번에 N개까지 불러옵니다.</p></div> */
    + clipMoreFootHTML(all.length);
  ...
  bindPickName();
  /* 시안 도구줄에는 `전체 선택` 체크박스가 없다 — 날짜 머리말의 체크로 고른다 */
  const allCb = $('#clipsAll');
  ...
  const szEl = $('#clipsSize');
  if (szEl) szEl.oninput = e => { REID.clipW = +e.target.value; renderClips(all); };
  /* 아카이브 — 종전 `더보기` 버튼. 지금은 스크롤이 끝에 닿으면 자동으로 이어 붙인다. */
  const more = $('#clipMore');
  if (more) more.onclick = () => { REID.clipShown += REID.clipMax; renderClips(all); };
  bindClipAutoMore(all);
  $$('#mdClips [data-close]').forEach(b => b.onclick = () => closeModal('#mdClips'));
  const goEl = $('#clipsGo');
  if (goEl) goEl.onclick = () => { closeModal('#mdClips'); finishReid([...REID.clipSel]); };
  renderClipPreview();
}

/* ── 경로 편집 · 영상 선택 (GUI 260914 · 5254:70521) ──────────────────────────
   왼쪽 : 기준 인물 사진 200 + 이름 / 오른쪽 : 총 N건 · 크기 · 정렬 · 썸네일/원본 + 카드 5열(누르면 선택)
   아래 : 고른 영상이 시간 순서로 놓이는 띠(번호 · ⊗ 빼기 · 위치 · 시각). 완료 동작은 REID.clipDone */
function renderClipsPath(all) {
  REID.clipAll = all; REID.clipTotal = all.length;
  const ui = REID.peUi || (REID.peUi = { cols: 5, view: '썸네일', sort: '유사도순' });
  const list = all.map(id => findObj(id)).filter(Boolean).sort(pickSortFn(ui.sort));
  const md = $('#mdClips'); md.classList.add('pe'); md.classList.toggle('orig', ui.view === '원본');
  const seed = findObj(REID.seed) || OBJECTS[0];
  $('#clipsSide').innerHTML = `<div class="pe-obj"><img src="${seed.img}" alt=""><i class="i i-g-info" title="경로를 편집할 인물"></i></div>
    <input class="pe-name" id="pkName" value="${seedName()}" maxlength="20" aria-label="이름" placeholder="인물A">`;
  $('#clipsTools').innerHTML = `<b class="pe-total">총 ${list.length}건</b>
    <div class="pe-right">
      <span class="pe-size" title="썸네일 크기"><i class="i i-16 i-image"></i><input type="range" min="3" max="6" step="1" value="${8 - ui.cols}" id="peCols"><i class="i i-image"></i></span>
      <div class="select sm pe-sort" id="clipsSort" data-value="${ui.sort}"><button class="select-btn">${ui.sort}<i class="i i-16 i-chevron i-down caret"></i></button>
        <div class="select-menu">${PICK_SORTS.map(v => `<div data-v="${v}">${v}</div>`).join('')}</div></div>
      <span class="pe-view">${['썸네일', '원본'].map(v => `<button type="button" class="${ui.view === v ? 'on' : ''}" data-peview="${v}">${v}</button>`).join('')}</span>
    </div>`;
  $('#clipsBody').innerHTML = `<div class="pe-grid" style="--pe-cols:${ui.cols}">${list.map(x => `
    <button type="button" class="pe-card${REID.clipSel.has(x.id) ? ' on' : ''}" data-pecard="${x.id}">
      <span class="pe-img"><img src="${srcVideo(x.cam) || x.img}" alt=""><span class="sim ${simCls(x.sim)}">${x.sim}%</span></span>
      <span class="pe-wrap"><b>${x.cam}</b><i>${String(x.t).slice(0, 16)}</i></span></button>`).join('')}</div>`;
  $('#clipsFoot').innerHTML = `<button class="btn-ghost" data-close>취소</button>
    <button class="btn-primary" id="clipsGo" ${REID.clipSel.size ? '' : 'disabled'}>완료</button>`;
  bindPickName();
  const rr = () => renderClipsPath(all);
  $$('#clipsBody [data-pecard]').forEach(b => b.onclick = () => { const id = b.dataset.pecard; REID.clipSel.has(id) ? REID.clipSel.delete(id) : REID.clipSel.add(id); rr(); });
  const pc = $('#peCols'); if (pc) pc.oninput = () => { ui.cols = 8 - +pc.value; const g = $('#clipsBody .pe-grid'); if (g) g.style.setProperty('--pe-cols', ui.cols); };
  $$('#clipsTools [data-peview]').forEach(b => b.onclick = () => { ui.view = b.dataset.peview; rr(); });
  bindSelect('#clipsSort', v => { ui.sort = v; rr(); });
  $$('#mdClips [data-close]').forEach(b => b.onclick = () => closeModal('#mdClips'));
  const go = $('#clipsGo'); if (go) go.onclick = () => { if (typeof REID.clipDone === 'function') REID.clipDone(); };
  renderClipStrip();
}
function renderClipStrip() {
  const host = $('#clipsPreview'); if (!host) return;
  const picked = [...REID.clipSel].map(id => findObj(id)).filter(Boolean).sort((a, b) => String(a.t).localeCompare(String(b.t)));
  if (!picked.length) { host.innerHTML = `<div class="pe-strip empty"><span>선택한 영상이 시간 순서대로 여기에 놓입니다.</span></div>`; return; }
  const W = Math.max(host.clientWidth || 0, 1280);
  const ts = picked.map(o => +new Date(String(o.t).replace(' ', 'T')));
  const t0 = Math.min(...ts), span = Math.max(1, Math.max(...ts) - t0), usable = Math.max(200, W - 40 - 180);
  let last = -1e9;
  const xs = ts.map(t => { let x = 40 + (t - t0) / span * usable; if (x < last + 182) x = last + 182; last = x; return Math.round(x); });
  const days = [];
  picked.forEach((o, i) => { const d = String(o.t).slice(0, 10); if (!days.some(x => x.d === d)) days.push({ d, x: days.length ? xs[i] - 12 : 17 }); });
  host.innerHTML = `<div class="pe-strip">
      <div class="pe-dates">${days.map(x => `<span style="left:${x.x}px">${x.d}</span>`).join('')}</div>
      <div class="pe-track" style="--pe-w:${Math.max(W, last + 200)}px"><i class="pe-axis"></i>${days.map(x => `<i class="pe-day" style="left:${x.x + 1}px"></i>`).join('')}
        ${picked.map((o, i) => `<div class="pe-clip" style="left:${xs[i]}px">
          <span class="pe-th"><img src="${srcVideo(o.cam) || o.img}" alt="">
            <span class="pe-info"><em class="pe-n">${i + 1}</em><button type="button" class="pe-rm" data-perm="${o.id}" title="이 영상 빼기"><i class="i i-g-remove"></i></button></span></span>
          <b>${o.cam}</b><i>${String(o.t).slice(11, 19)}</i></div>`).join('')}</div>
      <div class="pe-zoom"><i class="i i-24 i-g-zoomout"></i><input type="range" min="1" max="4" value="1" aria-label="타임라인 확대"><i class="i i-24 i-g-zoomin"></i></div>
    </div>`;
  host.querySelectorAll('[data-perm]').forEach(b => b.onclick = e => { e.stopPropagation(); REID.clipSel.delete(b.dataset.perm); renderClips(REID.clipAll); });
}
...
function finishReid(ids) {
  ...
  DT.tools = ['obj', 'single'];
  newTab(`${name} ${o.cam}`, o, { kind: 'group', label: name });
}
...
function openCardDetail(id, cardEl) {
  ...
  newTab(target === o.cam ? `${o.cam}` : `${target} ${o.cam}`, o, isRep ? { kind: 'group' } : undefined);
}

/* 한 번 클릭 = 미리보기 / 두 번 클릭 = 상세 (2026-09-15 구두 : 상황에 따라 안 먹던 것)
   종전에는 220ms 타이머가 끝나면 곧바로 `selectCard → renderResults` 로 결과 카드를 전부
   다시 그렸다. 그래서 두 번째 클릭이 **새로 만들어진 다른 노드**에 떨어져 브라우저
   `dblclick` 이 발생하지 않았다(조금 느리게 두 번 누르면 미리보기만 두 번 뜨는 증상).
   게다가 첫 미리보기가 열릴 때 오른쪽 패널이 생기며 결과 영역이 좁아져 카드가 밀린다 —
   검색 후 **첫 더블클릭**이 특히 자주 실패한 이유다.
   그래서 네이티브 dblclick 에 기대지 않고 클릭 수를 직접 센다. 판정이 끝나기 전에는
   다시 그리지 않으므로 카드가 제자리에 남고 레이아웃도 흔들리지 않는다. */
const CARD_DBL = 350;
...
function bindCards(root) {
  $$('.card', root).forEach(c => {
    ...
    c.onclick = e => {
      ...
      clearTimeout(cardClick.t);
      if (cardClick.n >= 2) {
        /* 두 번째 클릭 → 상세 (클립 선택을 거쳐 열린다) */
        cardClickReset();
        selectCard(id); openReid(id);
        return;
      }
      cardClick.t = setTimeout(() => { cardClickReset(); selectCard(id); }, CARD_DBL);
    };
    /* 네이티브 dblclick 은 위 셈과 겹쳐 상세가 두 번 열리므로 쓰지 않는다.
       (글자 선택만 막는다) */
    c.ondblclick = e => e.preventDefault();
  });
...
function openCtx(e, id) {
  ...
  /* 카드 하단에 북마크 버튼(.cbk)이 따로 있으므로 더보기에는 북마크를 넣지 않는다.
     ⋯ 자체가 **지능형 알고리즘 결과에만** 붙으므로(cardHTML) 항목은 알고리즘 기준 2종이다.
     (2026-09-15 구두 — 썸네일 북마크와 ⋯ 안 북마크가 중복이던 것을 정리) */
  m.innerHTML = `<div data-act="pfind">인물 검색</div><div data-act="report">오탐지 신고</div>`;
  ...
  $$('div', m).forEach(d => d.onclick = () => {
    ...
    else if (act === 'pfind') { const o = findObj(id); if (o) openPersonFromFrame(o); return; }
...
/* 미리보기 상자에 실제로 붙어 있는 <video> (vidSync 가 스틸 뒤에 겹쳐 둔다).
   메타데이터가 오기 전에는 duration 이 NaN 이라 그때는 스틸 루프(10초)로 돈다. */
function pvVid() {
  const img = document.querySelector('#pvVideo > img');
  const v = img && img._vid;
  return (v && v.duration && !isNaN(v.duration)) ? v : null;
}
const pvMMSS = s => `${String(Math.floor((s || 0) / 60)).padStart(2, '0')}:${String(Math.floor((s || 0) % 60)).padStart(2, '0')}`;
...
function pvClock() {
  ...
  PV.timer = setInterval(() => {
    ...
    if (du) du.textContent = '/ ' + pvMMSS(PV.dur);
  }, 100);
}
...
function bindPv() {
  ...
  /* 재생 상태를 HUD 와 **실제 <video>** 양쪽에 적용한다.
     종전에는 .paused 클래스(스틸 애니메이션)만 바뀌어 영상은 계속 돌았다. */
  const apply = () => {
    v.classList.toggle('paused', !PV.playing);
    if (b) { b.innerHTML = PV.playing ? ICON.pause : ICON.play; b.title = PV.playing ? '일시정지' : '재생'; }
    const vid = pvVid();
    if (vid) { if (PV.playing) vid.play().catch(() => {}); else vid.pause(); }
  };
  PV.apply = apply;
  if (b) b.onclick = e => { e.stopPropagation(); PV.playing = !PV.playing; apply(); };

  /* 진행 바를 눌러/끌어 그 지점으로 이동 (종전에는 표시만 하고 조작이 안 됐다) */
  const bar = v.querySelector('.pv-bar');
  if (bar) {
    const seek = clientX => {
      const r = bar.getBoundingClientRect();
      const ratio = Math.max(0, Math.min(1, (clientX - r.left) / r.width));
      const vid = pvVid();
      PV.dur = vid ? vid.duration : PV.dur;
      PV.t = ratio * PV.dur;
      if (vid) { try { vid.currentTime = PV.t; } catch (_) {} }
      const fill = document.getElementById('pvBar'); if (fill) fill.style.width = (ratio * 100) + '%';
      const tc = document.getElementById('pvTime'); if (tc) tc.textContent = pvMMSS(PV.t);
    };
    bar.onmousedown = e => {
      e.preventDefault(); e.stopPropagation();
      PV.seeking = true; seek(e.clientX);
...
/* 미리보기는 **원본 영상**을 보여준다. 결과 카드 이미지는 인물만 잘라낸
   것이라 원본으로 쓰면 화면이 확대돼 보인다. */
/* 카메라 → 영상 샘플 (v01 1층 로비 · v02 1층 게이트 · v03 10층 EV홀 · v04 9층 EV홀 · v05 2층 외곽 ·
   v06 2층 로비 2 · v07 2층 로비 · v08 2층 회전문 우측 · v09 2층 회전문 좌측) */
const SRC_VIDEO = { '1F 메인 복도': 'v01', '1층 로비': 'v02', '1F 로비': 'v02', '1F 계단': 'v02',
  '1F 엘리베이터 홀': 'v03', 'B1 엘리베이터': 'v04', 'B1 주차장': 'v05', 'B1 입구': 'v09',
  '2층 통로': 'v06', '2F 통로': 'v06', '3층 매장': 'v07', '3F 매장': 'v07', '1F 업무공간': 'v07',
  '외부 CCTV': 'v08', '외부 본관 별동': 'v09', '외부 주차장 1구역': 'v05', '외부 사업장 정문': 'v08' };
function srcVideo(cam) { return IMG(SRC_VIDEO[cam] || 'v01'); }

function renderPreview() {
  const box = $('#previewList'); box.innerHTML = '';
  /* 검색 전에는 미리 볼 것이 없다 — 패널을 접어 결과 영역에 자리를 내준다 */
  /* 사양 : 미리보기는 **고른 대상이 있을 때만** 나온다 (검색만으로는 안 뜬다) */
  const pv = document.getElementById('preview');
  /* 원본 영상 미리보기 패널은 **검색 홈 전용**이다 (2026-09-16 구두 오류).
     상세 화면에서 이 함수가 불리면(어느 경로로든) 패널이 우측에 튀어나왔다.
     여기서 한 번 막아 두면 호출 경로를 모두 뒤지지 않아도 된다. */
  const inDetail = document.body.classList.contains('in-detail');
  if (pv) pv.hidden = inDetail || !(S.preview && S.preview.length);
    ...
    if (p.uid === openUid) {
      ...
      const n = el('div', 'pv', `
        <div class="pv-video${PV.playing ? '' : ' paused'}" id="pvVideo">
          <img src="${srcVideo(p.cam)}" alt="">
          <div class="pv-hud"><span class="tc" id="pvTime">00:00</span><span class="dur" id="pvDur">/ ${pvMMSS(PV.dur)}</span></div>
...
/* 입력창 안의 돋보기 — 아래 `검색` 버튼과 같은 동작 (시안 5295:84013) */
{ const g = $('#qTextGo'); if (g) g.onclick = () => $('#btnSearchGo').click(); }

/* select 컴포넌트 */
function bindSelect(id, onChange) {
  const s = $(id); if (!s) return;
  const btn = $('.select-btn', s); if (!btn) return;
  btn.onclick = e => { e.stopPropagation(); $$('.select').forEach(x => x !== s && x.classList.remove('open')); s.classList.toggle('open'); };
  /* 항목은 컨테이너에 **위임**한다 — #sortMenu 처럼 비어 있다가 나중에
     innerHTML 로 채워지는 메뉴는 개별 바인딩이 남아나지 않는다. */
  const menu = $('.select-menu', s); if (!menu) return;
  menu.onclick = e => {
    const d = e.target.closest('div[data-v]');
    if (!d || !menu.contains(d)) return;
    e.stopPropagation();
    s.dataset.value = d.dataset.v;
    const b = $('.select-btn', s);
    if (b.childNodes[0] && b.childNodes[0].nodeType === 3) b.childNodes[0].nodeValue = d.dataset.v;
    else b.insertBefore(document.createTextNode(d.dataset.v), b.firstChild);
...
/* 와이어프레임 4862:109536 : 지능형 결과 영상 안에서 인물을 골라 → 인물 검색(이미지 탭)으로 재검색 */
function openPersonFromFrame(o) {
  S._imgFrom = 'algo';
  loadImage(srcVideo(o.cam) || o.img);
}
function setImgModalCopy() {
  const fromAlgo = S._imgFrom === 'algo' || S._imgFrom === 'fence';
  const h = $('#mdImage .md-head h3'), d = $('#mdImage .md-head p');
  if (h) h.textContent = fromAlgo ? '인물 선택' : '이미지 검색';
  if (d) d.textContent = fromAlgo ? '영상에서 검출된 인물을 검색할 수 있습니다.' : '이미지에서 자동 추출된 얼굴과 대상을 검색할 수 있습니다. (복수 선택 가능)';
  $('#btnRepick').hidden = fromAlgo;
  $('#imgCancel').textContent = S._imgFrom === 'fence' ? '이전' : '취소';
}
function loadImage(url) {
  if (S._imgFrom !== 'algo' && S._imgFrom !== 'fence') S._imgFrom = null;
  setImgModalCopy();
  S.uploaded = url;
  $('#mdImagePreview').src = url;
  $('#ieGrid').innerHTML = `<div class="ie-loading">${S._imgFrom === 'algo' ? '인물 추출 중' : '대상 추출 중'}</div>`;
  $('#bboxLayer').innerHTML = '';
...
function finishExtract() {
  ...
  $('#bboxLayer').innerHTML = S.extracted.map(x =>
    `<div class="bbox ${x.type}" style="left:${x.box.x}%;top:${x.box.y}%;width:${x.box.w}%;height:${x.box.h}%"><span>${x.type === 'face' ? '얼굴' : '외형'}</span></div>`).join('');
...
function renderExtractGrid(t) {
  ...
  if (!list.length) { $('#ieGrid').innerHTML = `<div class="ie-loading" style="color:#6b7785">${S._imgFrom ? '검출된 인물이 없습니다.' : '이미지에서 추출된 대상이 없습니다.<br>다시 추출하거나 다른 이미지를 선택해 주세요.'}</div>`; return; }
  $('#ieGrid').innerHTML = list.map(x =>
    `<div class="ext-item${S.extSel.includes(x.id) ? ' on' : ''}" data-x="${x.id}"><img src="${x.img}"><span class="tag">${x.type === 'face' ? '얼굴' : '외형'}</span></div>`).join('');
...
$('#imgCancel').onclick = () => {
  closeModal('#mdImage');
  /* 가상펜스 : `이전` = 직전 가상펜스 검색 팝업으로(선택 유지) */
  if (S._imgFrom === 'fence' && S._fence) { S._imgFrom = null; openAreaSearch(S._fence.a); return; }
  S._imgFrom = null;
};
$('#imgSearch').onclick = () => {
  closeModal('#mdImage');
  /* 지능형 결과에서 왔으면 인물 검색 > 이미지 탭으로 옮겨 재검색 */
  if (S._imgFrom === 'fence') {
    /* 가상펜스 : 검색 결과 탭(검색 홈)의 `인물` 검색으로 이동해 이미지 검색과 같은 과정 수행 */
    S._imgFrom = null; S._fence = null; DT.area = null;
    S.activeTab = 'search'; renderTabs(); syncPanels();
    switchMode('person');
    const tb = document.querySelector('#pfWay [data-way="img"]'); if (tb) tb.click();
    ...
    renderSideExtract(); buildFilters('person'); runSearch(false);
    toast('인물 검색으로 이동해 선택한 인물을 검색합니다.');
    return;
  }
  if (S._imgFrom === 'algo') {
    S._imgFrom = null;
    switchMode('person');
    const tb = document.querySelector('#pfWay [data-way="img"]'); if (tb) tb.click();
    toast('인물 검색으로 전환해 선택한 인물을 다시 검색합니다.');
  }
  $('#dropzone').hidden = true; $('#uploadedWrap').hidden = false; $('#uploadedImg').src = S.uploaded;
  const acc = $('#extractedAcc'); acc.classList.add('open'); $('.acc-head', acc).disabled = false;
  renderSideExtract();
...
function renderSideExtract() {
  $('#extGrid').innerHTML = S.extracted.map(x =>
    `<div class="ext-item${S.extSel.includes(x.id) ? ' on' : ''}" data-sx="${x.id}"><img src="${x.img}"><span class="tag">${x.type === 'face' ? '얼굴' : '외형'}</span><i class="ext-add">${S.extSel.includes(x.id) ? '✓' : '+'}</i></div>`).join('');
  { const n = $('#extCount'); if (n) { n.hidden = !S.extracted.length; n.textContent = S.extracted.length; } }
...
function renderPersonGrid() {
  ...
  box.innerHTML = list.map(p => `<div class="pcard${S.selPersons.includes(p.id) ? ' on' : ''}" data-p="${p.id}">
    <div class="thumb"><img src="${p.imgs[0]}"><button type="button" class="pc-more" data-pmore="${p.id}" title="더보기">⋮</button></div><div class="nm">${p.name}</div><div class="ds">${p.desc}</div></div>`).join('');
  /* 와이어프레임 4862:109341 : 카드 ⋮ → 수정 · 삭제 */
  $$('[data-pmore]', box).forEach(b => b.onclick = e => {
    e.stopPropagation();
    const id = b.dataset.pmore, m = $('#ctxMenu');
    m.innerHTML = `<div data-act="pedit">수정</div><div data-act="pdel">삭제</div>`;
    const r = b.getBoundingClientRect();
    m.style.left = Math.min(r.left, innerWidth - 120) + 'px'; m.style.top = (r.bottom + 4) + 'px'; m.hidden = false;
    $$('div', m).forEach(d => d.onclick = ev => {
      ev.stopPropagation(); m.hidden = true; m.innerHTML = CTX_DEFAULT;
      const p = S.persons.find(x => x.id === id); if (!p) return;
      if (d.dataset.act === 'pedit') { openPersonMgr(); pmTarget = p; pmView = 'detail'; renderPM(); }
      else alertSpec('personDel', () => {
        S.persons = S.persons.filter(x => x.id !== id); S.selPersons = S.selPersons.filter(x => x !== id);
...
{ const b = $('#btnNewPersonInline'); if (b) b.onclick = () => { openPersonMgr(); pmNew(); }; }
...
const IMG_POOL = [PIMG('v01', 0), PIMG('v02', 4), PIMG('v03', 0), PIMG('v06', 2), PIMG('v09', 5)];
...
function renderHistory() {
  /* 와이어프레임 4862:108625 : 날짜별 묶음, 행 = 썸네일 · 인물명 · 카메라 위치(카메라명) · 📍지점 수 · 일시 · ✕.
     (사건 등록 · 북마크 · 연관 검색 펼침 삭제) — 행을 누르면 그 인물의 상세(동선)를 연다 */
  let k = 0;
  $('#historyBody').innerHTML = HISTORY.map((d, di) => `
    <div class="hs-day"><div class="hs-date">${d.date}</div>
      ${d.items.map((it, i) => {
        const o = OBJECTS[(di * 3 + i) % OBJECTS.length] || OBJECTS[0];
        const nm = `인물 ${String.fromCharCode(65 + (k++ % 26))}`;
        return `<div class="hs-item hs-row" data-hs="${di}-${i}" data-hsobj="${o.id}" data-hsnm="${nm}">
          <img class="hs-th" src="${o.img}" alt="">
          <span class="hs-bd"><span class="hs-l1"><b class="hs-q">${nm}</b><span class="hs-cam">${o.cam}</span>
            <span class="hs-pin" title="출현 지점 수"><svg viewBox="0 0 12 12" aria-hidden="true"><path d="M6 11s3.6-3.4 3.6-6.2A3.6 3.6 0 0 0 2.4 4.8C2.4 7.6 6 11 6 11z" fill="none" stroke="currentColor" stroke-width="1.1"/><circle cx="6" cy="4.8" r="1.2" fill="currentColor"/></svg>${Math.max(2, Math.min(12, it.n % 11 + 2))}</span></span>
            <em class="hs-t">${d.date} 14:52</em></span>
          <button class="btn-icon hs-x" data-hsrm="${di}-${i}" title="기록 삭제">${ICON.x}</button>
        </div>`; }).join('')}
    </div>`).join('');
  $$('#historyBody .hs-row').forEach(r => r.onclick = e => {
    if (e.target.closest('[data-hsrm]')) return;
    const o = findObj(r.dataset.hsobj); if (!o) return;
    closeHistoryPanel();
    newTab(`${r.dataset.hsnm} ${o.cam}`, o);
  });
  $$('#historyBody [data-hsrm]').forEach(b => b.onclick = e => {
    e.stopPropagation();
    const row = b.closest('.hs-item'), day = row.parentElement;
    row.remove(); if (!day.querySelector('.hs-item')) day.remove();
    toast('검색 기록을 삭제했습니다.');
  });
  return;
...
/* 히스토리는 가운데 모달이 아니라 **히스토리 버튼 아래에 붙는 패널**이다
   (2026-09-16 구두 · 시안 5391:119198 — 440×564, 탭 바 바로 아래 우측 정렬).
   딤(overlay) 없이 열고, 바깥을 누르거나 Esc 로 닫는다. */
function placeHistory() {
  const md = document.getElementById('mdHistory'), btn = document.getElementById('btnHistory');
  if (!md || !btn || md.hidden) return;
  const r = btn.getBoundingClientRect();
  const w = md.offsetWidth || 440;
  md.style.left = Math.max(8, Math.min(r.right - w, innerWidth - w - 8)) + 'px';
  md.style.top = Math.round(r.bottom + 6) + 'px';
  md.style.maxHeight = Math.max(200, innerHeight - r.bottom - 20) + 'px';
}
function openHistoryPanel() {
  renderHistory();
  const md = document.getElementById('mdHistory');
  md.hidden = false;
  md.classList.add('as-pop');
  placeHistory();
  setTimeout(() => document.addEventListener('mousedown', histOut, true), 0);
  document.addEventListener('keydown', histEsc, true);
  addEventListener('resize', placeHistory);
}
function closeHistoryPanel() {
  const md = document.getElementById('mdHistory'); if (!md) return;
  md.hidden = true;
  document.removeEventListener('mousedown', histOut, true);
  document.removeEventListener('keydown', histEsc, true);
  removeEventListener('resize', placeHistory);
}
function histOut(e) { if (!e.target.closest('#mdHistory') && !e.target.closest('#btnHistory')) closeHistoryPanel(); }
function histEsc(e) { if (e.key === 'Escape') closeHistoryPanel(); }
$('#btnHistory').onclick = () => {
  const md = document.getElementById('mdHistory');
  if (md && !md.hidden) { closeHistoryPanel(); return; }   /* 같은 버튼을 다시 누르면 닫힌다 */
  openHistoryPanel();
};
...
function setCollapsed(v) {
  ...
  document.body.classList.toggle('side-folded', !!v);
  /* 시안(4965:32320) : 접으면 패널이 통째로 사라지고,
     **탭바 첫 자리에 `검색` 탭 + 펼침 버튼**이 대신 선다. */
  $('#sidePanel').hidden = v || S.aiMode;
  $('#aiPanel').hidden = v || !S.aiMode;
  renderTabs();
  const fold = document.getElementById('btnFold');
  if (fold) fold.onclick = () => setCollapsed(true);
}
...
/* ============================================================
   ...
   ============================================================ */
const DT = {
  /* 와이어 기본 상세는 평면도 한 장이지만, 3D 적층이 층간 이동을 훨씬 잘 보여준다.
     사용자 판단으로 3D 적층을 기본으로 유지한다 (2026-09-09). */
  clip: 0, map: 'floor', layers: false, flatFloor: null, zone: 'A동', zoneAuto: true, zone2: null, split: false, tools: ['obj', 'multi'], bookmarks: new Set(), nearQ: '',   /* 참고안 : 2x2 가 기본 */
...
const ICON2 = {
  folder: '<i class="i i-20 i-g-folder"></i>',
...
const TL = {
  ...
  show: null,
  cmp: [],
  /* 경로 비교 : 칩을 눌러 끈(비활성) 인물의 슬롯 키 — 사양 4759:35111 */
  off: new Set()
};
...
function tlStep(spanMs, zoom, px) {
  ...
  const maxTicks = Math.max(Math.floor(w / 48), 4);   /* GUI 260914 : 1280 폭에서 1시간 눈금(약 50px 간격) */
...
function openClipReselect(host, tracks, opt, ti = 0) {
  const pool = OBJECTS.slice(0, 13);
  const cur = new Set(tracks[ti].clips.map(c => c.cam + c.from));
  REID.clipSel = new Set(pool.filter(o => cur.has(o.cam + o.t)).map(o => o.id));
  /* 목업 데이터는 타임라인 구간과 검색 결과가 1:1 로 이어져 있지 않다 — 못 찾으면 구간 수만큼 앞에서 골라 둔다 */
  if (!REID.clipSel.size) pool.slice(0, Math.min(5, Math.max(1, tracks[ti].clips.length))).forEach(o => REID.clipSel.add(o.id));
  const ids = pool.map(o => o.id);
  REID.clipShown = Math.max(REID.clipMax, 12);
  REID.clipMode = 'path';
  /* 완료 시 타임라인 구간을 다시 구성한다 — 카드를 고를 때마다 다시 그려져도 완료 동작이 바뀌지 않게 콜백으로 둔다 */
  REID.clipDone = () => {
    closeModal('#mdClips');
    const picked = [...REID.clipSel].map(id => findObj(id)).filter(Boolean);
    if (!picked.length) { toast('선택한 클립이 없습니다.'); return; }
    /* 시안 띠의 00:20:00 — 구간마다 20분, 다음 구간 시작을 넘지 않게 자른다 */
    const ms = t => +new Date(String(t).replace(' ', 'T'));
    const fmt = v => { const d = new Date(v), z = n => String(n).padStart(2, '0'); return `${d.getFullYear()}-${z(d.getMonth() + 1)}-${z(d.getDate())} ${z(d.getHours())}:${z(d.getMinutes())}:${z(d.getSeconds())}`; };
    picked.sort((a, b) => String(a.t).localeCompare(String(b.t)));
    tracks[ti].clips = picked.map((o, i) => {
      const next = picked[i + 1] ? ms(picked[i + 1].t) - 60000 : Infinity;
      return { n: i + 1, cam: o.cam, img: o.img, from: fmt(ms(o.t)), to: fmt(Math.max(ms(o.t) + 60000, Math.min(ms(o.t) + 20 * 60000, next))) };
    });
    TL.sel = { t: 0, c: 0 };
  ...
  renderClips(ids);
  openModal('#mdClips');
  /* 2026-09-18 : 종전엔 여기서 renderClipStrip() 으로 하단을 옛 `.pe-strip` 으로 덮어썼다.
     renderClips 가 이미 renderClipPreview(= 상세와 같은 구조의 새 타임라인)를 그리므로
     덮어쓰지 않고, 모달이 열려 실제 폭이 잡힌 뒤 한 번 더 그려 눈금 간격을 맞춘다.
     (renderClipStrip 은 경로 편집 팝업 renderClipsPath 전용으로 남는다) */
  requestAnimationFrame(() => renderClipPreview());
}

/* 타임라인 인물 칩 상세 정보 (2026-09-15 구두)
   칩을 누르면 뜨고 ✕ · 바깥 클릭 · Esc 로 닫는다. 종전의 '눌러서 활성/비활성' 은 삭제.
   호버 툴팁과 같은 상자(#tlTip)를 쓰되 dataset.pin 으로 구분해 호버가 덮어쓰지 못하게 한다. */
function openChipInfo(chip, tr) {
  let tip = document.getElementById('tlTip');
  if (!tip) { tip = el('div', 'tl-tip'); tip.id = 'tlTip'; document.body.appendChild(tip); }
  const a = tr.clips[0], z = tr.clips[tr.clips.length - 1];
  const cams = [...new Set(tr.clips.map(c => c.cam))];
  /* 유사도는 트랙에 없다 — 비교 중이면 CMP.objs, 단일이면 탭이 들고 있는 대상에서 가져온다 */
  const cur = S.tabs.find(x => x.id === S.activeTab) || {};
  const o = ((typeof CMP !== 'undefined' && CMP.objs) ? CMP.objs.find(x => x.label === tr.label) : null)
    || (cur.kind !== 'compare' ? cur.obj : null)
    || ((typeof OBJECTS !== 'undefined') ? OBJECTS.find(x => x.cam === a.cam) : null);
  const color = typeof slotColor === 'function' ? slotColor(tr.slot) : 'var(--primary)';
  tip.className = 'tl-tip pin';
  tip.dataset.pin = '1';
  tip.innerHTML = `<img src="${a.img}" alt="">
    <span class="tt-i">
      <b><i style="background:${color}"></i>${tr.label}</b>
      <span class="tt-f"><span>유사도</span><em>${o ? o.sim + '%' : '-'}</em></span>
      <span class="tt-f"><span>출현</span><em>${a.from} ~<br>${z.to}</em></span>
      <span class="tt-f"><span>구간</span><em>${tr.clips.length}개 · 카메라 ${cams.length}대</em></span>
      <span class="tt-cam">${cams.join(' · ')}</span>
      <button type="button" class="tt-reg">인물등록<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M6 3.5L10.5 8 6 12.5" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
    </span>
    <button type="button" class="tt-x" title="닫기">${ICON.x}</button>`;
  tip.hidden = false;
  ...
  const close = () => {
    tip.hidden = true; delete tip.dataset.pin; tip.className = 'tl-tip';
    document.removeEventListener('mousedown', onOut, true);
    document.removeEventListener('keydown', onKey, true);
  };
  const onOut = e => { if (!e.target.closest('#tlTip') && !e.target.closest('.tl-obj')) close(); };
  const onKey = e => { if (e.key === 'Escape') close(); };
  tip.querySelector('.tt-x').onclick = close;
  tip.querySelector('.tt-reg').onclick = () => { close(); if (typeof openWatch === 'function') openWatch(); };
...
function renderTimeline(host, tracks, opt = {}) {
  if (!host) return;
  { const pv = document.getElementById('tlCurPv'); if (pv) pv.hidden = true; }   /* 다시 그리면 커서 미리보기는 닫는다 */
  TL.tracks = tracks;
  ...
  const span = d1 - d0;
  /* 커서 기본 위치를 첫 클립에 맞춘다 — 기본값(0.34)이면 클립에서 멀리 떨어져
     미리보기도 안 뜨고 무엇을 보는 커서인지 알 수 없다. */
  if (!TL._cursorSet && tracks[0] && tracks[0].clips[0]) {
    TL.cursor = Math.max(0, Math.min(1, (+tlTime(tracks[0].clips[0].from) - d0) / span));
    TL._cursorSet = true;
  }
  ...
  for (let t = d0; t <= d1; t += step) {
    ...
    ticks += `<span class="tk" style="left:${p}%"></span>`;
    /* 맨 끝(다음날 08:00) 라벨은 오른쪽으로 삐져나와 가로 스크롤을 만든다 — 시안도 07:00 까지 */
    /* GUI 260914 : 자정 눈금은 `24:00` 으로 적는다 */
    if (p < 99.5) ticks += `<span class="tl-lb" style="left:${p}%">${isNewDay && lastDay && tlHM(d) === '00:00' ? '24:00' : tlHM(d)}</span>`;
  ...
  const rows = tracks.map((tr, ti) => `
    <div class="tl-row${TL.off.has(tr.slot) ? ' off' : ''}" data-tr="${ti}">
      <div class="tl-obj" style="--slot:${typeof slotColor === 'function' ? slotColor(tr.slot) : 'var(--primary)'}">
        <img src="${(tr.clips[0] || {}).img || PIMG('v01', 0)}" alt="">
        <span class="nm"><i></i><em>${tr.label}</em>${ti > 0 ? `<button class="rm" data-tlrm="${ti}" title="${tr.label} 빼기"><i class="i i-g-remove"></i></button>` : ''}</span>
      </div>
      <div class="tl-lane">
        ${(() => {
          /* 시안 5126:58116 : 클립 창의 **폭이 곧 구간 길이**다.
             그 안에 100x56 썸네일이 필름처럼 반복되며 창 밖은 잘린다.
             짧은 구간은 시안(8px 조각)처럼 얇은 조각으로 남고,
             위의 8px 막대가 그 구간을 눈에 띄게 해 준다. */
          return tr.clips.map((c, ci) => {
            /* 위치·폭 모두 **실제 시각** 그대로. 밀어내면 눈금·커서와 어긋난다. */
            const l = pct(c.from);
            const w = wid(c);
            /* 테두리(1px x2) 때문에 시작점이 미세하게 왼쪽으로 밀린다 */
            const extra = (c.extra || []).length;
            const on = TL.sel.t === ti && TL.sel.c === ci;
            /* 클립 길이 편집 중 : 줄여서 잘려 나간 원래 구간도 흐린 썸네일로 계속 보여준다 (2026-09-14 구두) */
            const ghost = (opt.edit && c._f0 && (c._f0 !== c.from || c._t0 !== c.to))
              ? `<div class="tl-ghost" style="left:${pct(c._f0)}%;width:${Math.max(0.6, (+tlTime(c._t0) - +tlTime(c._f0)) / span * 100)}%;--bc:${slotColor(tr.slot)}"><span class="tl-film" style="background-image:url('${(typeof srcVideo === 'function' && srcVideo(c.cam)) || c.img}')"></span></div>`
              : '';
            return ghost + `<div class="tl-bar${on ? ' on' : ''}" style="left:${l}%;width:${w}%;--bc:${typeof slotColor === 'function' ? slotColor(tr.slot) : 'var(--primary)'}" data-tr="${ti}" data-c="${ci}"></div>
  ...
  host.innerHTML = `
    ...
    <div class="tl-foot">
      ...
      <div class="tf-zoom" title="타임라인 확대/축소">
        <button class="btn-icon zb" data-tlz="out" title="축소"><i class="i i-24 i-g-zoomout"></i></button>
        <input type="range" class="tl-zr" min="1" max="8" step="1" value="${TL.zoom}">
        <button class="btn-icon zb" data-tlz="in" title="확대"><i class="i i-24 i-g-zoomin"></i></button>
      </div>
      <div class="tf-r">${opt.edit ? `
        <button class="btn-ghost sm" data-tledcancel>취소</button>
        <button class="btn-primary sm" data-tledsave ${tlEditDirty() ? '' : 'disabled'}>저장</button>` : `
  ...
  /* 윗선 2 포함. 확대해 가로 스크롤이 생기면 스크롤바 자리(10)를 따로 둬서 마지막 트랙을 가리지 않는다 */
  /* 가로 스크롤바는 자리를 차지하지 않는 오버레이라 확대해도 높이가 변하지 않는다(화면 흔들림 없음) */
  host.style.height = (Math.max(206, 2 + 48 + 8 + tracks.length * 65 + (tracks.length - 1) * 2 + 2 + 51) + topH) + 'px';
  host.classList.toggle('tl-zoomed', TL.zoom > 1);
  /* 비교 화면에는 인물 탭 줄(.tl-top)이 하나 더 있다 — 높이 계산에 넣지 않으면
     마지막 트랙이 잘린다. 실측해서 변수로 넘긴다. */
  requestAnimationFrame(() => {
    const top = host.querySelector('.tl-top');
    const h = top ? Math.round(top.getBoundingClientRect().height) : 0;
    host.style.setProperty('--tl-toph', h + 'px');
  });
  ...
  host.querySelectorAll('[data-tlrm]').forEach(b => b.onclick = e => {
    ...
    if (i === 0) { toast('기준 대상은 삭제할 수 없습니다.'); return; }
    const tr = tracks[i];
    /* §14 공통 Alert 문구 그대로 */
    alertSpec('personDel', () => {
      TL.show.delete([...TL.show].sort((a, z) => a - z)[i]);
      const cur = S.tabs.find(x => x.id === S.activeTab);
      if (cur && cur.kind === 'compare') {
        const k = CMP.objs.findIndex(o => tr && o.label === tr.label);
        if (k > 0) { CMP.objs.splice(k, 1); if (CMP.trackIdx) CMP.trackIdx.splice(k, 1); }
        TL.show = new Set(CMP.objs.map((_, n) => n));
        if (CMP.objs.length <= 1) {
          /* 한 명만 남으면 같은 탭이 단일 뷰(원래 상세)로 */
          if (cur.baseKind) cur.kind = cur.baseKind; else delete cur.kind;
          TL.show = new Set([0]); TL.off = new Set();
          renderTabs(); syncPanels(); toast('단일 뷰로 전환했습니다.'); return;
        }
        syncPanels(); return;
      }
      renderTimeline(host, allTracks, opt);
  ...
  /* 사양 2-2) `경로 비교` → 경로 비교 추가 팝업 */
  if (addObj) addObj.onclick = () => {
    /* 비교 화면의 트랙은 부분집합이라 인덱스를 TL_TRACKS 기준으로 되돌린다 */
    const cur = S.tabs.find(x => x.id === S.activeTab);
    const base = cur && cur.kind === 'compare' && CMP.trackIdx
      ? CMP.trackIdx.slice() : [...TL.show].map(i => TL_TRACKS.indexOf(allTracks[i])).filter(i => i >= 0);
    openCompareAdd(TL_TRACKS, base.sort((a, b) => a - b));
  };
  ...
  host.querySelectorAll('[data-tlz]').forEach(b => b.onclick = () => {
    TL.zoom = b.dataset.tlz === 'in' ? Math.min(8, TL.zoom + 1) : Math.max(1, TL.zoom - 1);
    renderTimeline(host, allTracks, opt);
  });
  ...
  const syncCursor = () => {
    ...
    if (host.id === 'dtTl' && typeof cmpNowStr === 'function') {
      const now = cmpNowStr(), rg = document.getElementById('dtRange'); if (rg) rg.textContent = now;
      document.querySelectorAll('#dtMulti .mv-head .tm').forEach(t => { t.textContent = now; });   /* 멀티 뷰 칸도 현재 재생 시각 */
    }
    ...
    if (typeof vidSeekAll === 'function') vidSeekAll();   /* 영상 재생 위치도 커서 시각으로 */
    if (typeof cmpSyncHeads === 'function') cmpSyncHeads();
    /* 커서 미리보기가 떠 있으면(끄는 중 포함) 커서를 따라 갱신 */
    const pv = document.getElementById('tlCurPv');
    if (pv && !pv.hidden && typeof host._pv === 'function') host._pv(TL._pvY);
    /* 멀티 뷰 : 재생 구간이 바뀌면 다시 채운다 (이동 경로는 다음 세트로 자동 페이지 전환) */
    if (((host.id === 'dtTl' && DT.tools && DT.tools.includes('multi')) || (host.id === 'cmpTl' && CMP.multi)) && typeof mvCurClip === 'function') {
      const k = (DT.mvType || '이동 경로') + (DT.mvSplit || 4) + ':' + mvCurClip().ci;
  ...
  const seekTo = clientX => {
    ...
    /* 좌측 라벨 자리(gutter)를 뺀 **레인 폭** 기준으로 센다 —
       그래야 커서가 눈금·클립과 같은 위치를 가리킨다. */
    const gut = parseFloat(getComputedStyle(inner0).paddingLeft) || 0;
    TL.cursor = Math.max(0, Math.min(1, (clientX - r.left - gut) / (r.width - gut)));
    syncCursor();
  };
  const startSeek = e => {
    e.preventDefault();
    ...
    document.body.classList.add('tl-seeking');   /* 끄는 동안만 시각 배지를 보인다 */
  ...
  /* 가로 스크롤바 : 브라우저 막대는 숨기고 반투명 막대를 트랙 위에 겹쳐 띄운다 (2026-09-11 구두).
     자리를 차지하지 않으니 생기거나 사라져도 화면이 흔들리지 않는다. 끌기·클릭 이동 지원 */
  {
    const sc = host.querySelector('.tl-scroll');
    const bar = el('div', 'tl-sb', '<i></i>'); host.appendChild(bar);
    const thumb = bar.firstChild;
    const sync = () => {
      const max = sc.scrollWidth - sc.clientWidth;
      bar.hidden = max <= 1; if (bar.hidden) return;
      const tw = Math.max(40, sc.clientWidth * sc.clientWidth / sc.scrollWidth);
      thumb.style.width = tw + 'px';
      thumb.style.transform = `translateX(${(sc.clientWidth - tw) * (sc.scrollLeft / max)}px)`;
    };
    const flash = () => { bar.classList.add('act'); clearTimeout(bar._t); bar._t = setTimeout(() => bar.classList.remove('act'), 900); };
    sc.addEventListener('scroll', () => { sync(); flash(); });
    thumb.addEventListener('mousedown', e => {
      e.preventDefault(); e.stopPropagation();
      const x0 = e.clientX, s0 = sc.scrollLeft, k = sc.scrollWidth / sc.clientWidth;
      bar.classList.add('drag');
      const mv = ev => { sc.scrollLeft = s0 + (ev.clientX - x0) * k; };
      const up = () => { bar.classList.remove('drag'); removeEventListener('mousemove', mv); removeEventListener('mouseup', up); };
      addEventListener('mousemove', mv); addEventListener('mouseup', up);
    });
    bar.addEventListener('mousedown', e => {
      if (e.target === thumb) return;
      const r = bar.getBoundingClientRect();
      sc.scrollLeft = (e.clientX - r.left) / r.width * sc.scrollWidth - sc.clientWidth / 2;
    });
    requestAnimationFrame(sync); setTimeout(sync, 60);
  }

  /* 커서 호버 미리보기 (시안 5223:57371 Thumbnail_Hover) :
     인물 정보가 아니라 **커서가 가리키는 영상 클립의 썸네일** + 시각.
     마우스가 올라간 트랙의 클립을 우선, 그 트랙에 없으면 커서 시각에 클립이 있는 첫 트랙. */
  host._pv = cy => {
    let pv = document.getElementById('tlCurPv');
    if (!pv) { pv = el('div', 'tl-cur-pop'); pv.id = 'tlCurPv'; pv.hidden = true; document.body.appendChild(pv); }
    const t = d0 + TL.cursor * span;
    const at = k => (tracks[k] ? tracks[k].clips.find(c => +tlTime(c.from) <= t && t <= +tlTime(c.to)) : null);
    const rows = [...host.querySelectorAll('.tl-row')];
    let ti = rows.findIndex(r => { const b = r.getBoundingClientRect(); return cy >= b.top && cy <= b.bottom; });
    if (ti < 0 || !at(ti)) ti = tracks.findIndex((_, k) => at(k));
    const c = ti >= 0 ? at(ti) : null;
    pv.innerHTML = (c ? `<span class="cp-img" style="border-color:${slotColor(tracks[ti].slot)}"><img src="${(typeof srcVideo === 'function' && srcVideo(c.cam)) || c.img}" alt=""></span>` : '')
      + `<b class="cp-t">${tlHMS(new Date(t)).replace(/:/g, ' : ')}</b>`;
    pv.dataset.tr = ti; pv.hidden = false;
    const cur = host.querySelector('.tl-cursor'); if (!cur) return;
    const cr = cur.getBoundingClientRect(), x = cr.left + cr.width / 2;
    pv.style.left = Math.max(8, Math.min(x - pv.offsetWidth / 2, innerWidth - pv.offsetWidth - 8)) + 'px';
    pv.style.top = Math.max(8, (cy || cr.top) - pv.offsetHeight - 10) + 'px';
  };
  if (curEl) {
    const onPv = e => { TL._pvY = e.clientY; host._pv(e.clientY); };
    curEl.addEventListener('mouseenter', onPv);
    curEl.addEventListener('mousemove', onPv);
    curEl.addEventListener('mouseleave', () => {
      if (document.body.classList.contains('tl-seeking')) return;   /* 끄는 중엔 유지 */
      const pv = document.getElementById('tlCurPv'); if (pv) pv.hidden = true;
    });
  }
  ...
  host.querySelectorAll('[data-tlg]').forEach(g => g.onmousedown = e => {
    ...
    document.body.style.cursor = 'ew-resize';
    /* 클립 길이는 **줄이기만** 된다 — 처음 탐지된 구간(_f0~_t0) 밖으로 늘릴 수 없다 (2026-09-11 구두) */
    if (c._f0 == null) { c._f0 = c.from; c._t0 = c.to; }
    const F0 = +tlTime(c._f0), T0 = +tlTime(c._t0);
    const mv = ev => {
      const d = (ev.clientX - sx) / pxPerMs;
      if (side === 's') {
        const v = Math.min(from0 + d, to0 - 5e3);       /* 최소 5초 (v0.8) */
        c.from = iso(Math.max(F0, d0, v));
      } else {
        const v = Math.max(to0 + d, from0 + 5e3);
        c.to = iso(Math.min(T0, d1, v));
      }
      renderTimeline(host, tracks, opt);
      trimPreview(ev.clientX, host.querySelector(`.tl-th[data-tr="${ti}"][data-c="${ci}"]`), c, side === 's' ? c.from : c.to, tracks[+ti]);
    };
    const up = () => {
      document.removeEventListener('mousemove', mv);
      document.removeEventListener('mouseup', up);
    ...
    trimPreview(sx, g.closest('.tl-th'), c, side === 's' ? c.from : c.to, tracks[+ti]);
    document.addEventListener('mousemove', mv);
    document.addEventListener('mouseup', up);
  });
  ...
  /* 사양 3-2) 타임라인 영역에서 마우스 휠로 시간 축척 확대/축소 (가로 스크롤은 그대로) */
  const scr = host.querySelector('.tl-scroll');
  if (scr) scr.addEventListener('wheel', e => {
    if (Math.abs(e.deltaY) <= Math.abs(e.deltaX)) return;
    e.preventDefault();
    const z = Math.max(1, Math.min(8, TL.zoom + (e.deltaY < 0 ? 1 : -1)));
  ...
  if (ed) ed.onclick = () => {
    ...
    openPersonPick(tracks, k => openClipReselect(host, allTracks, opt, allTracks.indexOf(tracks[k])));
  };
  const ve = host.querySelector('[data-tlvedit]');
  if (ve) ve.onclick = () => { tlEditOn() ? tlEditCancel() : tlEditStart(); };
  const ec = host.querySelector('[data-tledcancel]'), es = host.querySelector('[data-tledsave]');
  if (ec) ec.onclick = () => tlEditCancel();
  if (es) es.onclick = () => tlEditSave();
  /* v0.8 3-4 : 구간 너비가 좁으면 내부 썸네일·위치명 생략 */
  host.querySelectorAll('.tl-th').forEach(n => { const w = n.offsetWidth; n.classList.toggle('narrow', w < 64); n.classList.toggle('tiny', w < 22); });
  /* v0.8 3-3 : 탐지 구간 호버 → 그 지점의 영상 썸네일 + HH:MM:SS 미리보기 (편집 중엔 끄는 지점 미리보기가 대신) */
  if (!opt.edit) host.querySelectorAll('.tl-th').forEach(n => {
    const tr = tracks[+n.dataset.tr], c = tr && tr.clips[+n.dataset.c]; if (!c) return;
    const show = e => {
      const inner = host.querySelector('.tl-inner'), r = inner.getBoundingClientRect();
      const gut = parseFloat(getComputedStyle(inner).paddingLeft) || 0;
      const f = Math.max(0, Math.min(1, (e.clientX - r.left - gut) / (r.width - gut)));
      const t = new Date(d0 + f * span);
      trimPreview(e.clientX, n, c, `0000-00-00 ${tlHMS(t)}`, tr);
    };
    n.addEventListener('mousemove', show);
    n.addEventListener('mouseleave', () => { const pv = document.getElementById('tlCurPv'); if (pv) pv.hidden = true; });
  });
}
...
const _cb = (t, cls, extra = '') => `<button class="btn-icon" title="${t}" ${extra}><i class="i ${/^i-g-/.test(cls) ? 'i-24' : 'i-18'} ${cls}"></i></button>`;
/* 재생 버튼은 **지금 재생 상태**를 보고 그린다 (2026-09-16 구두).
   종전에는 늘 ▶ 로 그려서, 다시 그릴 때마다(멀티 뷰·팝업 등) 재생 중인데도 ▶ 가 떴다. */
function playBtnHTML() {
  const on = (typeof VID !== 'undefined') && VID.playing;
  return `<button class="btn-icon play${on ? ' playing' : ''}" data-playtg title="${on ? '일시정지' : '재생'}">` +
    `<i class="i i-24 i-g-${on ? 'pause' : 'play'}"></i></button>`;
}
/* 화면에 있는 재생 버튼을 한꺼번에 상태에 맞춘다 */
function syncPlayBtns() {
  const on = (typeof VID !== 'undefined') && VID.playing;
  document.querySelectorAll('#dtPlay, [data-playtg]').forEach(x => {
    x.classList.toggle('playing', !!on);
    x.title = on ? '일시정지' : '재생';
    x.innerHTML = `<i class="i i-24 i-g-${on ? 'pause' : 'play'}"></i>`;
  });
}
function playCtrlHTML(opt = {}) {
  ...
  /* 양끝 버튼은 **구간 건너뛰기** — 상세 전용 id 가 아니라 공통 속성으로 붙여
     비교·팝업 조작 바에서도 같은 동작을 하게 한다 (2026-09-16 구두) */
  return `${clips ? b('이전 영상', 'i-g-prev', 'data-clipjump="-1"') : ''}
    ${b('배속 뒤로', 'i-g-rewind')}
    ${b('10초 뒤로', 'i-g-back10')}
    ${playBtnHTML()}
    ${b('10초 앞으로', 'i-g-fwd10')}
    ${b('배속 앞으로', 'i-g-forward')}
    ${clips ? b('다음 영상', 'i-g-next', 'data-clipjump="1"') : ''}`;
}
function ctrlHTML(opt = {}) {
  const b = _cb;
  /* 시안 5184:100217 (경로 비교) : 상세와 같은 배치. 비교 중엔 영역·선 검색과
     히트맵·주변 카메라를 쓰지 않아 20% 로 흐리게, 재생·설정 · 보기 전환(단일/멀티 뷰)은 살린다
     (2026-09-14 구두 : 경로 비교에서도 카메라 멀티 뷰가 동작하는 사양) */
  ...
  /* 새 시안 5501:163780(경로비교 4명) 실측 : 오른쪽은
     ...
     2026-09-17 구두 : 위 시안 실측은 `분할 버튼 → 콤보박스` 순이었으나, 경로 비교에서
     멀티 뷰로 바꾸면 상세 화면과 좌우가 뒤바뀌어 보인다는 지적. **상세 화면(#dtMvType →
     #dtSplitBtn, index.html)과 같은 `콤보박스 → 분할 버튼` 순으로 통일한다.** */
...
function renderPip(clip) {
  ...
  let box = document.getElementById('dtPipc');
  if (!clip) { if (box) box.remove(); return; }
  /* 클립 데이터는 바꾸지 않는다 — 예전엔 clip.cam·img 를 바꿔 써서 멀티 뷰·타임라인까지 카메라가 뒤섞였다.
     칸 안 스위칭 상태는 PIP_MAIN(클립별 키)에 따로 둔다 */
  const key = 'dt-' + (clip.from || clip.cam);
  const pr = pipResolve(key, { cam: clip.cam, img: (typeof srcVideo === 'function' && srcVideo(clip.cam)) || clip.img }, simulCams(clip));
  if (!pr.list.length) { if (box) box.remove(); return; }
  if (!box) { box = el('div', 'pip-host'); box.id = 'dtPipc'; stage.appendChild(box); }
  box.innerHTML = pipHTML(key, pr.list);
  if (pr.main.cam !== clip.cam) {
    const im = document.getElementById('dtVideoImg'); if (im) im.src = pr.main.img;
    const lb = document.getElementById('dtCam'); if (lb) lb.textContent = pr.main.cam;
  }
  /* PIP 카메라 → 메인 영상 칸 안에서 스위칭 */
  box.querySelectorAll('[data-pipcam]').forEach(b => b.onclick = e => { e.stopPropagation(); pipSwap(b); });
  requestAnimationFrame(() => pipAutoFit(box));
}
...
/* ============================================================
   ...
   ============================================================ */
/* 사양 2-6) 단일 영상 뷰(1X1)가 기본 — 옛 참고안의 2x2 기본은 폐기 */
...
=======
      <img src="assets/img/floor.png?v=202609211033" alt="맵뷰">
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
  ...
  const tileHead = k => {
    ...
    return `<span class="pv-head">
      ...
      <b>${t ? t.label : '대상'}</b>
      <em>${c.cam || '-'}</em>
      <span class="rg">${c.from ? String(c.from).slice(0, 19) : ''} ~ ${c.to ? String(c.to).slice(11, 19) : ''}</span>
      <span class="ic"><button class="tb mv-more" data-vmore title="더보기"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><circle cx="3.5" cy="8" r="1.3" fill="currentColor"/><circle cx="8" cy="8" r="1.3" fill="currentColor"/><circle cx="12.5" cy="8" r="1.3" fill="currentColor"/></svg></button></span>
    </span>`;
  };
  ...
  if ((PANE.vmode[i] || 'multi') === 'multi') {
    return `<div class="pn-vid pn-quad">
      <div class="pq">${[0, 1, 2, 3].map(k =>
        `<span><img src="${(clips[k] || clips[0] || {}).img || 'assets/video/v01.jpg'}" alt="">
  ...
  return `<div class="pn-vid">
    <img src="${(clips[Math.min(i, 3)] || {}).img || 'assets/video/v01.jpg'}" alt="">
...
function renderDetail(tab) {
  ...
  renderTimeline($('#dtTl'), TL_TRACKS, {
    ...
    onSelect: (tr, c) => { const im = $('#dtVideoImg'); if (im) im.src = srcVideo(c.cam) || c.img; renderPip(c); },
  ...
  /* 양끝 버튼은 **타임라인 구간**을 건너뛴다 (2026-09-16 구두) — 구간이 2개 이상이면 쓴다.
     종전에는 DT.clip 기준이라 첫 구간에서 `이전`이 늘 꺼져 있었다. */
  /* 보이는 트랙 전부의 구간 수 — 2명 이상이면 합쳐서 센다 (2026-09-16 구두) */
  const nSeg = ((TL.tracks || []).flatMap(t => (t && t.clips) || []).length) || nClip;
  if ($('#dtPrevClip')) $('#dtPrevClip').disabled = nSeg <= 1;
  if ($('#dtNextClip')) $('#dtNextClip').disabled = nSeg <= 1;
  ...
  $$('#dtClips .dt-clip').forEach(n => n.onclick = e => {
    ...
    $('#dtVideoImg').src = srcVideo(DT.clips[DT.clip].cam) || DT.clips[DT.clip].img;
    renderDetail(tab);
  });
  ...
  /* 비율 버튼 · 합친 분할 버튼 · 재생 버튼을 실제 화면 상태에 맞춘다 (아이콘이 상태를 나타내므로) */
  if (typeof syncVRatioAll === 'function') requestAnimationFrame(syncVRatioAll);
  if (typeof syncSplitBtn === 'function') syncSplitBtn();
  if (typeof syncPlayBtns === 'function') requestAnimationFrame(syncPlayBtns);
...
function openNearDetail(x) {
  ...
  $('#dtVideoImg').src = srcVideo(x.cam) || x.img;
}
...
function applyTools() {
  ...
  cmpMultiHome();
  $$('#dtTools [data-tool], #dtAreaTools [data-tool]').forEach(b => b.classList.toggle('on', on(b.dataset.tool)));
  ...
  /* 2026-09-16 구두 : 멀티 뷰에서도 영역·가상펜스를 비활성으로 두지 않는다.
     (멀티에서 누르면 단일 뷰로 바꾼 뒤 그리기 모드로 들어간다 — 아래 클릭 처리) */
  $$('#dtAreaTools [data-area]').forEach(b => { b.disabled = false; });
  $$('#dtTools [data-tool], #dtAreaTools [data-tool]').forEach(b => {
    if (['heat', 'path', 'cctv'].includes(b.dataset.tool)) b.disabled = multiOn;
  });
  $('#dtCtrl').classList.toggle('multi-mode', multiOn);
  const vsel = $('#dtViewSel'); if (vsel) vsel.classList.toggle('on', multiOn);
...
function renderCctvPins() {
  $('#ovCctv').innerHTML = VIDEO_CCTV.map((c, i) =>
    `<span class="cctv-pin" data-vc="${i}" title="${c.cam}" style="left:${c.x}%;top:${c.y}%"><i class="i i-g-boxcam"></i></span>`).join('');
  $$('#ovCctv .cctv-pin').forEach(n => {
    n.onmouseenter = () => {
      const c = VIDEO_CCTV[+n.dataset.vc];
      /* GUI 260914 (5254:70255) : 핀 오른쪽 아래 160×90 미리보기 · 아래 어둡게 · 카메라명 10M */
      const pop = el('div', 'cctv-pop', `<img src="${srcVideo(c.cam) || c.img}" alt=""><span class="cap">${c.cam}</span>`);
      pop.style.left = `calc(${c.x}% + 2px)`; pop.style.top = `calc(${c.y}% + 14px)`;
      $('#ovCctv').appendChild(pop); n._pop = pop;
    };
    n.onmouseleave = () => { if (n._pop) { n._pop.remove(); n._pop = null; } };
    n.onclick = () => adjSwitch(VIDEO_CCTV[+n.dataset.vc]);
  });
}

/* ---- 멀티 뷰 (사양 PTS_DTL_004_1) ----------------------------------------------
   추적 중인 **한 인물**의 영상만 2×2 로 본다 (여러 인물 동시 재생이 아니다).
   - 좌측 상단 : 지금 재생 중인 메인 영상 유지
   - 나머지 3칸 : [영상 유형] 에 따라 최대 3개 자동 배치
     · 동시 포착 : 메인과 같은 시간대·인접 구역(같은 층·구역)에서 동시에 포착된 타 카메라, 메인 재생 시점과 동기화
     · 이동 경로 : 타임라인 동선 순서대로 4개씩 1세트(#1~#4, #5~#8 …). 재생이 다음 세트로 넘어가면 자동 페이지 전환.
                   4개 미만이면 탐지된 클립만 두고 나머지 칸은 빈 화면
   - 각 칸 머리말 : 카메라명 · 촬영 일시 (+ 북마크 · 영상 비율 · 전체 보기) */
const VH_BM = `<button class="tb" data-vbm title="북마크"><i class="i i-24 i-g-bookmark"></i></button>`;
/* 기본 상태는 `원본 비율` 이므로 아이콘도 그 상태 것으로 시작한다 (5318:87771) */
const VH_RATIO = `<button class="tb" data-vratio data-ratio-state="원본 비율" title="화면 비율 변경 (원본 비율)"><i class="i i-24 i-g-ratio-original"></i></button>`;
/* 사양 v0.8 : 영상 슬롯 머리말 = 북마크 · 영상 비율 · 영상 확대 */
const VH_ZOOM = `<button class="tb" data-vzoom title="영상 확대"><i class="i i-24 i-g-zoomin"></i></button>`;
const VH_FULL = `<button class="tb" data-vfull title="전체보기"><svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><path d="M2.5 6V2.5H6M10 2.5h3.5V6M13.5 10v3.5H10M6 13.5H2.5V10" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button>`;

/* 단일(1X1) / 멀티(2X2) 는 택1. 멀티에선 영상 검색·분석 도구를 쓰지 않는다 */
function setViewMode(k) {
  DT.tools = DT.tools.filter(x => x !== 'single' && x !== 'multi').concat(k);
  PANE.vmode[0] = k;   /* 패널 상태도 같이 — 안 그러면 다음 렌더에서 되돌아간다 */
  /* 단일로 돌아올 때도 표시를 지운다 — 남아 있으면 영상 이미지가 숨겨진 채 조작 바가 위로 올라붙는다 */
  { const vv = document.getElementById('dtVideo'); if (vv) vv.classList.toggle('has-multi', k === 'multi'); }
  if (k === 'multi') {
    DT.tools = DT.tools.filter(x => !['heat', 'path', 'cctv'].includes(x)); DT.tracks = false; DT.area = null;
    renderMulti();
  }
  applyTools();
  if (typeof syncSplitBtn === 'function') syncSplitBtn();   /* 합친 분할 버튼도 같이 */
}
/* ── 경로 비교 멀티 뷰 (2026-09-14 구두) ──
   비교 툴바의 [단일] [멀티 뷰 ▾] 로 전환. 멀티 뷰면 비교 칸 대신 **기준 인물(첫 트랙)** 의
   멀티 뷰(이동 경로 / 주변 카메라 × 2·3·4분할)를 같은 자리에 띄운다. 상세의 멀티 뷰 부품(#dtMulti)을 옮겨 쓴다. */
function cmpMultiHome() {
  const mv = document.getElementById('dtMulti'), vv = document.getElementById('dtVideo');
  if (!mv || !vv || mv.parentElement === vv || vidIsCompare()) return;
  mv.classList.remove('in-cmp'); vv.insertBefore(mv, document.getElementById('dtVhead'));
}
function cmpApplyMulti() {
  const grid = document.getElementById('cmpGrid'), mv = document.getElementById('dtMulti'); if (!grid || !mv) return;
  const stage = grid.parentElement;
  if (CMP.multi) {
    if (mv.parentElement !== stage) stage.insertBefore(mv, grid);
    mv.classList.add('in-cmp'); mv.hidden = false; grid.hidden = true;
    renderMulti();
  } else {
    grid.hidden = false;
    if (mv.parentElement === stage) { mv.hidden = true; mv.classList.remove('in-cmp'); const vv = document.getElementById('dtVideo'); if (vv) vv.insertBefore(mv, document.getElementById('dtVhead')); }
...
function mvCurClip() {
  const tr = (TL.tracks && TL.tracks[0]) || (typeof TL_TRACKS !== 'undefined' ? TL_TRACKS[0] : null);
  const clips = tr ? tr.clips : [];
  const t = (TL.d0 != null && TL.span) ? TL.d0 + TL.cursor * TL.span : null;
  let ci = t == null ? -1 : clips.findIndex(c => +tlTime(c.from) <= t && t <= +tlTime(c.to));
  if (ci < 0) ci = t == null ? 0 : clips.reduce((k, c, i) => (+tlTime(c.from) <= t ? i : k), 0);
  return { tr, clips, ci };
}
function renderMulti() {
  const vv = document.getElementById('dtVideo');
  if (vv) vv.classList.toggle('has-multi', DT.tools.includes('multi'));
  const host = document.getElementById('dtMulti'); if (!host) return;
  /* 사양 v0.8 PTS_DTL_004_1 : 재생 영상(이동 경로 · 주변 카메라, 기본 이동 경로) × 화면 분할(2·3·4, 기본 4) */
  const type = DT.mvType || '이동 경로', N = DT.mvSplit || 4;
  const { tr, clips, ci } = mvCurClip();
  if (!clips.length) { host.innerHTML = ''; return; }
  const rng = c => `${c.from.slice(0, 19)} ~ ${c.to.slice(11, 19)}`;
  const frame = c => (typeof srcVideo === 'function' && srcVideo(c.cam)) || c.img;
  let slots;
  if (type === '이동 경로') {
    /* 분할 수만큼 1세트 — 세트 마지막 구간이 끝나면 다음 세트로 자동 페이지 전환 */
    const s0 = Math.floor(ci / N) * N;
    slots = [...Array(N).keys()].map(k => { const c = clips[s0 + k]; return c ? { cam: c.cam, img: frame(c), range: rng(c), n: s0 + k + 1, ci: s0 + k, now: s0 + k === ci } : null; });
  } else {
    const main = clips[ci];
    /* 주변 카메라 : 같은 시간대 · 인접 구역 카메라를 인물 포착 여부와 무관하게, 포착된 카메라 우선.
       좌측 상단은 메인 영상 유지, 메인 재생 시점과 동기화(일시 동일) */
    const extra = (main.extra || []).map(x => ({ cam: x.cam, img: x.img }));
    const pool = (typeof MULTI_TILES !== 'undefined' ? MULTI_TILES : []).map(x => ({ cam: x.cam, img: x.img }))
      .filter(x => x.cam !== main.cam && !extra.some(e => e.cam === x.cam));
    const same = x => m3FloorOf(x.cam) === m3FloorOf(main.cam) && zoneOfCam(x.cam) === zoneOfCam(main.cam);
    slots = [{ cam: main.cam, img: frame(main), range: rng(main), ci }]
  ...
  host.innerHTML = slots.map((s, i) => s ? `
    <div class="mv-tile${s.now ? ' now' : ''}" data-mv="${i}"${s.ci != null ? ` data-ci="${s.ci}"` : ''}>
      <img src="${s.img}" alt="">
      <div class="mv-head"><span class="mv-obj"><i style="background:${col}"></i><b>${(tr && tr.label) || '인물 A'}</b></span>${s.n ? `<span class="mv-hn" style="background:${col}">${s.n}</span>` : ''}<span class="nm">${s.cam}</span><span class="tm">${cmpNowStr()}</span><span class="mv-ic"><span class="mv-sw">${VH_BM}</span><button class="tb mv-more" data-vmore title="더보기"><i class="i i-24 i-g-vmore"></i></button></span></div>
  ...
  if (!DT.mvView) DT.mvView = {};
  $$('#dtMulti .mv-tile[data-mv]').forEach(n => {
    const im = n.querySelector(':scope > img'), v = DT.mvView[n.dataset.mv]; if (!im) return;
    applyVRatio(im, (v && v.ratio) || '원본 비율');
    if (v && v.zoom && v.zoom !== '1.0') { im.dataset.zx = v.zx || 0; im.dataset.zy = v.zy || 0; applyVZoom(im, v.zoom); }
  });
  requestAnimationFrame(() => pipAutoFit(host));
  /* 이동 경로 : 칸을 누르면 그 구간으로 이동 */
  $$('#dtMulti .mv-tile[data-ci]').forEach(n => n.onclick = e => {
    if (e.target.closest('.mv-ic') || e.target.closest('.v-zoom-mini') || type !== '이동 경로') return;
    if (n.querySelector(':scope > img.v-zoomed')) return;   /* 확대 중 드래그 이동은 구간 이동으로 치지 않는다 */
    const c = clips[+n.dataset.ci]; if (!c || TL.d0 == null) return;
    TL.cursor = Math.max(0, Math.min(1, (+tlTime(c.from) - TL.d0) / TL.span));
    renderTimeline(document.getElementById('dtTl'), TL.tracks, TL._opt);
    renderMulti();
  });
}
/* 멀티 뷰에서 `전체 보기` → 영상 전체보기 팝업에 2×2 그대로 (사양 PTS_DTL_001_3 [멀티뷰 선택 시]) */
function openVideoGrid() {
  const slots = DT._mvSlots || [];
  $('#vwBody').innerHTML = `<div class="vw">
    <div class="vw-stage vw-grid">${slots.map(s => s ? `<div class="vg-tile"><img src="${s.img}" alt="">
      <div class="vw-head"><span class="nm">${s.cam}</span><span class="tm">${s.range}</span><span class="vh-ic">${VH_BM}</span></div></div>`
      : '<div class="vg-tile empty"></div>').join('')}</div>
    <div class="vw-prog" title="재생 위치"><i class="buf" style="width:70%"></i><i class="cur" style="width:27%"></i></div>
    <div class="dt-ctrl vw-ctrl"><div class="grp l"></div><div class="grp c">${playCtrlHTML()}</div><div class="grp r"></div></div>
  </div>`;
  const pg = $('#vwBody .vw-prog');
  if (pg) pg.onclick = e => { const r = pg.getBoundingClientRect(); pg.querySelector('.cur').style.width = Math.max(0, Math.min(100, (e.clientX - r.left) / r.width * 100)) + '%'; };
  openModal('#mdVideo');
}
...
function setAreaTool(mode) {
  if (DT.area && DT.area.mode === mode) { DT.area = null; }
    ...
    DT.area = { mode, done: false, rect: null, line: null };
    /* v0.8 : 검색 모드 진입 시 영상 자동 정지 */
    const pl = document.getElementById('dtPlay'); if (pl && pl.classList.contains('playing')) pl.click();
  }
...
=======
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609211033" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
  ...
  if (dir) dir.onclick = () => { a.dirOn = !a.dirOn; a.dir = a.dir || 1; redraw(); };
  const arr = $('[data-abarrow]', box);
  if (arr) arr.onclick = e => { e.stopPropagation(); a.dir = -(a.dir || 1); redraw(); };
  const reset = () => { DT.area = { mode: a.mode, done: false, rect: null, line: null, dir: a.dir, dirOn: false }; redraw(); };
  const rs = $('[data-abreset]', box); if (rs) rs.onclick = reset;
  const cc = $('[data-abcancel]', box); if (cc) cc.onclick = reset;
  /* 사양 v0.8 영역 검색 팝업 / 가상펜스 검색 팝업 */
  const go = $('[data-abgo]', box); if (go) go.onclick = () => openAreaSearch(a);
  const ok = $('[data-abok]', box); if (ok) ok.onclick = () => openAreaSearch(a);
}
/* 영상 위 드로잉 */
(function bindAreaDraw() {
  ...
  v.addEventListener('mousedown', e => {
    ...
    const up = () => {
      ...
      if (DT.area && DT.area.rect && DT.area.rect[2] > 3 && DT.area.rect[3] > 3) { DT.area.done = true; DT.area.single = DT.area.rect[2] * DT.area.rect[3] < 220; renderArea(); renderNear(DT._tab); }
  ...
  v.addEventListener('click', e => {
    ...
    if (!DT.area.line) { DT.area.line = [x, y, x, y]; renderArea(); }
      ...
      DT.area.done = true; renderArea(); renderNear(DT._tab);
    }
  });
...
/* 커서 시각에 대상이 있던 층 — 첫 지점 전이면 첫 지점의 층 */
function flatFloorAt(now) {
  const pts = ((typeof MAP_PATHS_CACHE !== 'undefined' && MAP_PATHS_CACHE && MAP_PATHS_CACHE[0])
    || (typeof MOVE_PATHS !== 'undefined' && MOVE_PATHS[0]) || { pts: [] }).pts.filter(t => zoneOfCam(t.cam) !== '외부')
    .sort((a, b) => a.hh - b.hh);
  if (!pts.length) return '1F';
  let at = pts[0];
  pts.forEach(t => { if (t.hh <= now) at = t; });
  return m3FloorOf(at.cam);
}
const camOf = w => (w && (w.dataset.cam || (w.title || '').split(' ·')[0])) || '';
function syncMapToCursor() {
  const now = cursorHours();
  /* 커서가 다른 구역·층으로 넘어가면 그 구역 지도 / 그 층 도면으로 바꾼다 */
  if (typeof MAP_PATHS_CACHE !== 'undefined' && MAP_PATHS_CACHE) {
    const z = zoneAt(now);
    if (DT.zoneAuto && z !== DT.zone) { DT.zone = z; renderMapLayers(MAP_PATHS_CACHE); return; }
    if (!DT.layers && DT.zone !== '외부') {
      const f = flatFloorAt(now);
      if (DT.zoneAuto !== false && f !== DT.flatFloor) { DT.flatFloor = f; renderMapLayers(MAP_PATHS_CACHE); return; }
    }
  }
  document.querySelectorAll('.dt-map, .pn-map').forEach(map => {
    /* 숨어 있는 평면 오버레이의 지점까지 잡히면 좌표가 0 이라 마커가 엉뚱한 데로
       간다 — 화면에 실제로 그려진 지점만 센다. */
    /* 경로 비교 : 인물(data-slot)마다 지점을 나눠 **각자의 현재 위치 마커**를 움직인다.
       예전에는 모든 인물의 지점을 시각순 한 줄로 섞어 마커 하나만 오갔다. */
    const all = [...map.querySelectorAll('.map-wp[data-hh]')].filter(w => w.getBoundingClientRect().width > 0);
    const bySlot = new Map();
    all.forEach(w => { const k = w.dataset.slot || ''; if (!bySlot.has(k)) bySlot.set(k, []); bySlot.get(k).push(w); });
    map.querySelectorAll('.map-now').forEach(m => { if (!bySlot.has(m.dataset.slot || '')) m.remove(); });
    if (!bySlot.size) return;
    const solid = map.querySelector('#dtMap3d:not([hidden]), #dtMapGL:not([hidden]), .zone-map:not([hidden]), .flat-stage.scaled, body.dt-b #dtMap > .flat-stage') || (document.body.classList.contains('dt-b') && map.querySelector(':scope > .flat-stage'));   /* 레터박스 판은 영역과 좌표계가 달라 화면 좌표로 */
    const box = map.getBoundingClientRect();
    const mid = w => { const r = w.getBoundingClientRect();
      return { x: r.left + r.width / 2 - box.left, y: r.top + r.height / 2 - box.top }; };
    const multiSlot = bySlot.size > 1;
    let pos = null, px = false;
    bySlot.forEach((grp, slot) => {
      const wps = grp.sort((a, b) => +a.dataset.hh - +b.dataset.hh);
      ...
      let mk = [...map.querySelectorAll('.map-now')].find(m => (m.dataset.slot || '') === slot);
      if (!mk) { mk = el('span', 'map-now'); mk.dataset.slot = slot; map.appendChild(mk); }
      mk.title = slot && multiSlot ? `${(TL_TRACKS.find(t => t.slot === slot) || {}).label || slot} 현재 위치` : '현재 위치';
      if (slot && multiSlot) mk.style.setProperty('--now', slotColor(slot)); else mk.style.removeProperty('--now');
      mk.classList.toggle('off', !!(wps[0] && wps[0].dataset.off));
      const ratio = (prev && next)
        ? (() => { const a = +prev.dataset.hh, b = +next.dataset.hh; return b > a ? (now - a) / (b - a) : 0; })()
        : 0;
      let p = null; px = false;
      /* 층별(3D)에서는 지점이 회전·기울어진 층 안에 있어 컨테이너 기준 %(data-x/y)와
         화면 위치가 다르다. 실제 화면 좌표를 재서 그 사이를 잇는다. */
      if (solid && box.width) {
        px = true;
        if (prev && next) { const A = mid(prev), B = mid(next);
          p = { x: A.x + (B.x - A.x) * ratio, y: A.y + (B.y - A.y) * ratio }; }
        else if (prev) p = mid(prev);
      } else {
        const at = (a, b, r) => ({
          x: +a.dataset.x + (+b.dataset.x - +a.dataset.x) * r,
          y: +a.dataset.y + (+b.dataset.y - +a.dataset.y) * r
        });
        if (prev && next) p = at(prev, next, ratio);
        else if (prev) p = { x: +prev.dataset.x, y: +prev.dataset.y };
      }
      if (p) {
        mk.hidden = false;
    ...
    /* 시안(5126:58130) : 구역이 바뀌는 구간에서는 현재 위치에 점선으로
       `구역 이동` 안내가 매달린다. 지금 어디에서 어디로 넘어가는 중인지 알린다. */
    let zone = map.querySelector('.map-zone');
    if (!zone) { zone = el('div', 'map-zone'); map.appendChild(zone); }
    const seqAll = ((MAP_PATHS_CACHE && MAP_PATHS_CACHE[0]) || { pts: [] }).pts.slice().sort((a, b) => a.hh - b.hh);
    let fp = null, fn = null;
    seqAll.forEach(t => { if (t.hh <= now) fp = t; else if (!fn) fn = t; });
    const mz = map.dataset.zone || 'A동';
    /* v0.8 : 구역 이동은 지도 머리말 이전/다음 버튼이 맡는다 — 지도 위 떠 있는 칩은 쓰지 않음 */
    const crossing = false && pos && fp && fn && zoneOfCam(fp.cam) !== zoneOfCam(fn.cam) && zoneOfCam(fp.cam) === mz;
    if (typeof renderZoneNav === 'function') renderZoneNav();
    zone.hidden = !crossing;
    if (crossing) {
      zone.innerHTML = `<span class="mz-line"></span>
        <button class="mz-box" type="button" data-zgo="${zoneOfCam(fn.cam)}" title="${zoneOfCam(fn.cam)} 지도로 이동"><b>구역 이동</b><em>${zoneOfCam(fp.cam)} → ${zoneOfCam(fn.cam)}</em><i class="mz-go">›</i></button>`;
      zone.style.left = pos.x + (px ? 'px' : '%');
      zone.style.top  = pos.y + (px ? 'px' : '%');
    }
  });
}

/* 지도 지점 → 타임라인 커서 */
/* 그날 0시 기준 시각(시)으로 타임라인 커서를 옮긴다 — 지도 지점·이동 경로 리스트 공용 */
function seekToHour(hh) {
  if (!TL.span) return;
  DT.zoneAuto = true;   /* 지점·목록으로 옮기면 지도가 다시 커서를 따라간다 */
  const mid = new Date(TL.d0); mid.setHours(0, 0, 0, 0);
  let t = +mid + hh * 36e5;
  if (t < TL.d0) t += 864e5;           /* 타임라인이 08:00 에 시작하므로 새벽은 다음 날 */
  TL.cursor = Math.max(0, Math.min(1, (t - TL.d0) / TL.span));
...
function bindMapSeek() {
  document.querySelectorAll('.map-wp[data-hh]').forEach(w => {
    ...
    w.onclick = e => { e.stopPropagation(); seekToHour(+w.dataset.hh); };
  });
}
...
/* 지점 이름 앞머리로 어느 층인지 가른다 (3F·2F·1F·B1).
   B1 을 1층에 합쳐 두었더니 지하 지점까지 한 층에 몰려 라벨이 겹쳤다. */
function m3FloorOf(cam) {
  const t = String(cam || '');
  if (/^3F|3층/.test(t)) return '3F';
  if (/^2F|2층/.test(t)) return '2F';
  if (/^B1|지하/.test(t)) return 'B1';
  return '1F';
}
...
function renderMap3d(paths) {
  ...
  if (!host) return;
  /* 사양 4-3) : 기본은 평면(단일 층). `다층 뷰` 버튼을 켜면 층을 쌓아 본다.
     WebGL 이 되면 three.js 3D(드래그 360° 회전), 안 되면 CSS 적층으로 대신한다. */
  const layers = !!DT.layers && DT.zone !== '외부';
  const src0 = (paths && paths.length) ? paths
             : (typeof MOVE_PATHS !== 'undefined' ? MOVE_PATHS.slice(0, 1) : []);
  const gl = document.getElementById('dtMapGL');
  const useGL = layers && !!gl && typeof M3D !== 'undefined' && M3D.supported();
  if (gl) gl.hidden = !useGL;
  const srcIn = src0.map(p => ({ ...p, pts: p.pts.filter(t => zoneOfCam(t.cam) !== '외부') }));
  if (useGL) M3D.show(gl, srcIn); else if (typeof M3D !== 'undefined') M3D.hide();
  const on = layers && !useGL;
  host.hidden = !on;
  const img = document.getElementById('dtMapImg');
  if (img) img.style.visibility = (on || useGL) ? 'hidden' : '';
  /* 층 배지는 층마다 붙으므로 평면용 정적 배지는 적층일 때 감춘다 */
  const fb = document.getElementById('dtFloor');
  if (fb) fb.hidden = on || useGL;
  if (!on) return;

  /* 경로가 아직 없을 때도 층별 뷰 자체는 보여야 한다 (시안 기본 상태) */
  const src = (paths && paths.length) ? paths
            : (typeof MOVE_PATHS !== 'undefined' ? MOVE_PATHS.slice(0, 1) : []);
  paths = src;
  /* 경로 비교 : 인물마다 자기 색 경로 (WebGL 이 없을 때의 적층 뷰도 같게) */
  const pl = src.map(p => ({ p, pts: p.pts.filter(t => zoneOfCam(t.cam) !== '외부') }));
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    const onFl = pts => pts.filter(t => m3FloorOf(t.cam) === f.key).sort((a, b) => (+a.n || 0) - (+b.n || 0));
    /* 같은 층에서 이어지는 지점은 선으로 잇는다. 층이 바뀌는 구간은 잇지 않는다 */
    const polys = pl.map(({ p, pts }) => { const mine = onFl(pts); return mine.length > 1
      ? `<polyline points="${mine.map(t => `${t.x},${t.y}`).join(' ')}" fill="none" stroke="${slotColor(p.slot)}" stroke-width="1.5"
           stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>` : ''; }).join('');
...
=======
      <img src="assets/img/floor.png?v=202609211033" alt="">
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
      ${poly}
      ${pl.map(({ p, pts }) => onFl(pts).map(t => `<span class="map-wp" data-pt="${f.key}-${p.slot}-${t.n}" data-cam="${t.cam}"
          ...
          style="left:${t.x}%;top:${t.y}%;background:${slotColor(p.slot)}">${t.n}</span>`).join('')).join('')}
    </div>`;
  }).join('');   /* 가운데 관통선 제거 — 실제 층 전환 지점과 무관해 의미가 없었다 */

  bindMapSeek();
  syncMapToCursor();
  /* 층마다 공간 라벨이 서로 겹쳐 묻혔다 — 실측해 뒤엣것을 감춘다.
     3D 변형은 이미지 로드 뒤에야 자리가 잡혀 한 번 더 확인한다. */
  const relabel = () => hideOverlappedLabels(host, '.m3-sp');
  requestAnimationFrame(relabel);
  setTimeout(relabel, 120);
  setTimeout(relabel, 400);
  host.querySelectorAll('img').forEach(im => { if (!im.complete) im.addEventListener('load', relabel, { once: true }); });
}

let MAP_PATHS_CACHE = null;

function renderMapLayers(paths, camName) {
  /* 경로가 없어도 맵 구성(3D 층·이동 칩)은 보여야 한다 — 기본 경로로 채운다 */
  if (!paths || !paths.length) {
    paths = (typeof MOVE_PATHS !== 'undefined') ? MOVE_PATHS.slice(0, 1) : paths;
  }
  const cctvOn = DT.mapTools.includes('cctv');
  /* 주변 카메라 원뿔은 평면 도면 좌표라 다층 뷰(3D)에서는 층판과 어긋난다 — 평면에서만 */
  $('#dtMapCones').hidden = !cctvOn || !!DT.layers;
  if (cctvOn) {
    $('#dtMapCones').innerHTML = MAP_CCTV.map((c, i) =>
      `<span class="map-cone ${coneType(c.cam)}" data-mc="${i}" title="${c.cam} · ${({ play: '현재 재생 카메라', hit: '대상 포착 카메라', miss: '대상 미포착 카메라' })[coneType(c.cam)]}" style="left:${flatU(c.x)}%;top:${c.y}%;rotate:${c.deg - 90}deg"><i></i><b></b></span>`).join('');
  ...
  renderMap3d(paths);            /* 층별 뷰는 경로 유무와 무관하게 그린다 */
  ...
  /* 사양 (2) 외부 구역 뷰 — 외곽 지도 · 클러스터 · 레터박스. 다층 뷰는 비활성 */
  const hostM = $('#dtMap');
  hostM.dataset.zone = DT.zone;
  const outdoor = DT.zone === '외부';
  const lyb = $('#dtMapTools [data-map="layers"]');
  if (lyb) { lyb.disabled = outdoor; lyb.classList.toggle('on', !!DT.layers && !outdoor); }
  const zmMain = hostM.querySelector(':scope > .zone-map');
  if (outdoor) {
    ['#dtMapPath', '#dtMapWps', '#dtMarker', '#dtMap3d', '#dtMapGL', '#dtFloor'].forEach(q => { const e = $(q); if (e) e.hidden = true; });
    if (typeof M3D !== 'undefined') M3D.hide();
    $('#dtMapImg').style.visibility = 'hidden';
    hostM.querySelectorAll(':scope > .m3-labels').forEach(n => n.remove());
    renderZoneMap(hostM, paths.map(p => ({ ...p, pts: p.pts.filter(t => zoneOfCam(t.cam) === '외부') })), slotColor(paths[0].slot));
    setBldg('#dtBldg', '외부', DT.split ? DT.zone2 : null);
    renderSplitPane(paths);
    requestAnimationFrame(() => { bindMapSeek(); syncMapToCursor(); });
    return;
  }
  if (zmMain) zmMain.hidden = true;

  /* 층별(3D) 일 때 평면 지도용 오버레이까지 그리면 지점·이름이 이중으로 겹친다 —
     평면 오버레이는 `지도` 모드에서만 쓴다. */
  const flat = !DT.layers;
  $('#dtMapPath').hidden = !flat; $('#dtMapWps').hidden = !flat;
  $('#dtMarker').hidden = !flat || !!multi;
  /* 평면(단일 층) : 커서가 지금 있는 층의 도면·지점만 보인다 (시안 기본 — 1층 도면).
     커서가 다른 층으로 넘어가면 syncMapToCursor 가 층을 바꿔 다시 그린다. */
  if (!DT.flatFloor) DT.flatFloor = flatFloorAt(cursorHours());
  const fl = DT.flatFloor;
  const fpaths = paths.map(p => ({ ...p, pts: p.pts.filter(t => m3FloorOf(t.cam) === fl && zoneOfCam(t.cam) !== '외부') }));
  const flb = (M3_FLOORS.find(f => f.key === fl) || {}).label || fl;
  if ($('#dtFloor')) $('#dtFloor').textContent = flb;
  /* 사양 4-4) : 같은 층·같은 구역은 실선, 외부/타 공간으로 나가는 구간은 점선 */
  $('#dtMapPath').innerHTML = `<svg viewBox="0 0 100 100" preserveAspectRatio="none">${fpaths.map(p => {
    const q = p.pts; let d = '';
    for (let i = 1; i < q.length; i++) {
      const out = /외부/.test(q[i - 1].cam) !== /외부/.test(q[i].cam);
      d += `<path d="M${flatU(q[i - 1].x)},${q[i - 1].y} L${flatU(q[i].x)},${q[i].y}" stroke="${out ? 'var(--exit)' : slotColor(p.slot)}"
        ${out ? 'stroke-dasharray="2.6 2.2"' : ''} ${p.off ? 'opacity="0.25"' : ''} vector-effect="non-scaling-stroke"/>`;
    }
    return d;
  }).join('')}</svg>`;

  const one = paths.length === 1;
  $('#dtMapWps').innerHTML = fpaths.map((p, pi) => p.pts.map(t0 => ({ ...t0, x: flatU(t0.x) })).map((t, ti) => {
    const last = ti === p.pts.length - 1;
    return `<span class="map-wp" data-hh="${t.hh}" data-x="${t.x}" data-y="${t.y}" data-slot="${p.slot}"${p.off ? ' data-off="1"' : ''}
        title="${t.cam} · 이 지점으로 이동" style="left:${t.x}%;top:${t.y}%;background:${slotColor(p.slot)}${p.off ? ';opacity:.3' : ''}">${t.n}</span>
  ...
  if (one) {
    ...
    /* 시안(맵뷰어) : 하단에 **외부 이동 / 실내 이동** 두 칩.
       인접 동 목록이 아니라 '어디로 나갔고 어디서 들어왔는지' 를 알린다. */
    const bl = el('div', 'pn-bldg', `
      <button class="mv-out" title="이 지점에서 건물 밖으로 나갑니다">
        <i>→</i><span><b>외부 이동</b><em>A동 → 사업장명</em></span></button>
      <button class="mv-in" title="밖에서 이 건물로 들어옵니다">
        <i>←</i><span><b>실내 이동</b><em>사업장명 → A동</em></span></button>`);
    host.append(bl);
  }
  ...
  setBldg('#dtBldg', 'A동', DT.split ? DT.zone2 : null);
  renderSplitPane(paths);
...
/* 층 라벨 배치 — 층 안에 두면 **위층 평면에 가려진다**.
   층은 transform 으로 스택 컨텍스트를 만들어 z-index 로 빠져나올 수 없다.
   그래서 라벨만 층 위 평면 레이어에 따로 그리고, 배지 위치를 실측해 얹는다.
   겹치면 감추기 전에 위·아래·오른쪽·왼쪽 순으로 자리를 옮겨 본다. */
function hideOverlappedLabels(host, sel) {
  if (!host) return;
  const box = host.getBoundingClientRect();
  if (!box.width) return;
  let layer = host.querySelector('.m3-labels');
  if (!layer) { layer = el('div', 'm3-labels'); host.appendChild(layer); }

  const badges = [...host.querySelectorAll('.map-wp')];
  const floors = [...host.querySelectorAll('.m3-floor')];
  layer.innerHTML =
    floors.map(f => `<span class="m3-fl">${f.dataset.fl || ''}</span>`).join('') +
    badges.map(b =>
      `<span class="m3-sp" data-pt="${b.dataset.pt || ''}">${b.dataset.cam || ''}</span>`).join('');

  /* 층 배지는 각 층의 왼쪽 위 모서리에 — 층끼리 겹치지 않는 자리다 */
  const fls = [...layer.querySelectorAll('.m3-fl')];
  fls.forEach((n, i) => {
    const fr = floors[i].getBoundingClientRect();
    n.style.left = Math.round(fr.left - box.left + 6) + 'px';
    n.style.top  = Math.round(fr.top - box.top + Math.max(fr.height * 0.30, 8)) + 'px';
  });

  /* 층이 바뀌는 구간은 점선으로 잇는다 — 시안(비교-대상4개)의 층간 이동선.
     예전에는 가운데에 고정선을 그어 실제 전환 지점과 무관했다. */
  const seq = badges.slice().sort((a, b) => (+a.textContent || 0) - (+b.textContent || 0));
  let lines = '';
  for (let i = 1; i < seq.length; i++) {
    const A = seq[i - 1], B = seq[i];
    if ((A.dataset.pt || '').split('-')[0] === (B.dataset.pt || '').split('-')[0]) continue;
    const ra = A.getBoundingClientRect(), rb = B.getBoundingClientRect();
    lines += `<line x1="${ra.left + ra.width / 2 - box.left}" y1="${ra.top + ra.height / 2 - box.top}"
                    x2="${rb.left + rb.width / 2 - box.left}" y2="${rb.top + rb.height / 2 - box.top}" />`;
  }
  if (lines) {
    layer.insertAdjacentHTML('afterbegin',
      `<svg class="m3-riser" width="${box.width}" height="${box.height}" aria-hidden="true">${lines}</svg>`);
  }

  const labs = [...layer.querySelectorAll('.m3-sp')];
  const brs = badges.map(b => b.getBoundingClientRect());
  ...
  labs.forEach((l, i) => {
    const br = brs[i];
    l.hidden = false;
    /* 3D 에서 위층 판에 가려진 지점은 이름도 감춘다 */
    if (badges[i] && badges[i].classList.contains('occ')) { l.hidden = true; return; }
    const cx = br.left + br.width / 2 - box.left;
    const cy = br.top + br.height / 2 - box.top;
    const lw = l.offsetWidth, lh = l.offsetHeight, g = 4;
    const spots = [
      [cx - lw / 2,                cy - br.height / 2 - lh - g],   /* 위 */
      [cx - lw / 2,                cy + br.height / 2 + g],        /* 아래 */
      [cx + br.width / 2 + g,      cy - lh / 2],                   /* 오른쪽 */
      [cx - br.width / 2 - lw - g, cy - lh / 2]                    /* 왼쪽 */
    ];
    let ok = false;
    for (const [x, y] of spots) {
      /* 컨테이너 밖으로 나가는 자리는 쓰지 않는다 */
      if (x < 0 || y < 0 || x + lw > box.width || y + lh > box.height) continue;
      const r = { left: box.left + x, top: box.top + y, right: box.left + x + lw, bottom: box.top + y + lh };
      const onBadge = brs.some((b, j) => j !== i && hit(r, b));
      const onLabel = placed.some(pr => hit(r, pr));
      if (!onBadge && !onLabel) {
...
=======
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609211033' : 'assets/img/floor.png?v=202609211033';
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
...
$$('#dtMapTools [data-map]').forEach(b => b.onclick = () => {
  ...
  /* 사양 4-3) 다층 뷰 : 층별 도면을 레이어로 쌓아 층간 이동을 한눈에 본다 */
  if (k === 'layers') {
    DT.layers = !DT.layers;
    b.classList.toggle('on', DT.layers);
    if (DT.layers && typeof M3D !== 'undefined' && M3D.reset) M3D.reset();   /* v0.8 : 다시 진입하면 시점 초기화 */
    renderMapLayers(MAP_PATHS_CACHE);
    return;
  }
  if (k === 'full') { openMapView(); return; }
...
/* 영상 도구 토글 */
$$('#dtTools [data-tool], #dtAreaTools [data-tool]').forEach(b => b.onclick = () => {
  const k = b.dataset.tool;
  /* 사양서 2-5) 단일(1X1) / 다중(2X2) view 는 택1 — 토글이 아니라 라디오처럼 동작 */
  if (k === 'single' || k === 'multi') { setViewMode(k); return; }   /* 멀티 뷰 버튼 : 2X2 즉시 전환 */
...
$$('#dtAreaTools [data-area]').forEach(b => b.onclick = () => {
  /* 영역·가상펜스는 영상 한 화면 위에 그린다 — 멀티 뷰면 단일로 먼저 바꾼다 */
  if ((DT.tools || []).includes('multi')) setViewMode('single');
  setAreaTool(b.dataset.area);
});
const SET = {
...
/* 사양 2-7) 0.25 / 0.5 / 1.0 / 1.25 / 1.5 / 2.0 · 기본 1.0 (시안의 0.75 는 사양에 없어 뺐다) */
const SET_SPEEDS = ['0.25', '0.5', '1.0', '1.25', '1.5', '2.0'];
...
function syncVm4() {
  ...
  v.querySelector('[data-cur="type"]').textContent = t;
  v.querySelector('[data-cur="split"]').textContent = n + '분할';
  v.querySelectorAll('[data-mt]').forEach(d => d.classList.toggle('on', d.dataset.mt === t));
  v.querySelectorAll('[data-ms]').forEach(d => d.classList.toggle('on', +d.dataset.ms === n));
}
/* GUI 260914 (5254:69796) 멀티 뷰 팝오버 : 아이콘을 누르면 멀티 뷰로 바꾸고 팝오버를 연다.
   `재생영상 · 화면 분할` 행을 누르면 그 목록으로 들어가고(‹ 머리로 복귀), 고르면 적용 후 닫힌다.
   상세 · 경로 비교 툴바 공용 */
function vm4Pane(menu, k) { menu.querySelectorAll('.vm4-pane').forEach(p => { p.hidden = p.dataset.pane !== k; }); }
function vm4Toggle(menu, open) { vm4Pane(menu, 'main'); menu.hidden = open === undefined ? !menu.hidden : !open; }
function bindVm4Menu(menu, apply) {
  menu.onclick = e => {
    e.stopPropagation();
    const row = e.target.closest('.vm4-row'); if (row) { vm4Pane(menu, row.dataset.sub); return; }
    if (e.target.closest('[data-back]')) { vm4Pane(menu, 'main'); return; }
    const mt = e.target.closest('[data-mt]'), ms = e.target.closest('[data-ms]'); if (!mt && !ms) return;
    if (mt) DT.mvType = mt.dataset.mt;
    if (ms) DT.mvSplit = +ms.dataset.ms;
    vm4Toggle(menu, false);
    apply();
  };
}
(() => {
  ...
  /* 아이콘의 멀티 뷰 전환은 [data-tool] 공통 처리가 맡고, 여기서는 팝오버만 연다 */
  v.querySelector('.vm4-go').addEventListener('click', e => { e.stopPropagation(); setTimeout(() => { syncVm4(); vm4Toggle(menu); }, 0); });
  bindVm4Menu(menu, () => { syncVm4(); setViewMode('multi'); });
  document.addEventListener('click', e => { if (!e.target.closest('#dtViewSel')) vm4Toggle(menu, false); });
})();

/* ── 화면 분할 버튼 하나로 합치기 (2026-09-16 구두) ─────────────────────────────
   종전 `[1분할] [4분할 ▾(팝오버)]` 두 버튼 → **버튼 하나**. 누를 때마다 1 → 2 → 3 → 4 → 1.
   팝오버 안에 있던 `재생 영상(이동 경로 / 주변 카메라)` 은 **옆 콤보박스**로 뺐다.
   (종전 `.vm1` · `#dtViewSel` 마크업과 코드는 되돌릴 때를 위해 남기고 CSS 로만 감춘다) */
const MV_SPLITS = [1, 2, 3, 4];
function curSplit() { return (DT.tools || []).includes('multi') ? (DT.mvSplit || 4) : 1; }
function syncSplitBtn() {
  const b = document.getElementById('dtSplitBtn');
  if (b) {
    const n = curSplit();
    b.dataset.split = n;
    b.title = `화면 분할 (${n}분할)`;
    b.classList.toggle('on', n > 1);
    const ic = b.querySelector('i'); if (ic) ic.className = 'i i-24 ' + (n === 1 ? 'i-g-view1' : 'i-g-view4');
    const nm = b.querySelector('.vm-n'); if (nm) nm.textContent = n;
  }
  /* 재생 영상 콤보박스도 현재 값으로 */
  const s = document.getElementById('dtMvType');
  if (s) {
    const t = DT.mvType || '이동 경로';
    s.dataset.value = t;
    const sb = s.querySelector('.select-btn');
    if (sb && sb.childNodes[0] && sb.childNodes[0].nodeType === 3) sb.childNodes[0].nodeValue = t;
    s.querySelectorAll('.select-menu div').forEach(x => x.classList.toggle('on', x.dataset.v === t));
    /* 1분할에서는 고를 의미가 없다 — 멀티 뷰일 때만 쓴다 */
    s.classList.toggle('off', curSplit() === 1);
  }
}
(() => {
  const b = document.getElementById('dtSplitBtn'); if (!b) return;
  b.onclick = e => {
    e.stopPropagation();
    const n = MV_SPLITS[(MV_SPLITS.indexOf(curSplit()) + 1) % MV_SPLITS.length];
    if (n === 1) setViewMode('single');
    else { DT.mvSplit = n; setViewMode('multi'); }
    if (typeof syncVm4 === 'function') syncVm4();
    syncSplitBtn();
  };
  bindSelect('#dtMvType', v => {
    DT.mvType = v;
    if (curSplit() > 1 && typeof renderMulti === 'function') renderMulti();
    if (typeof syncVm4 === 'function') syncVm4();
    syncSplitBtn();
  });
  syncSplitBtn();
...
/* 2026-09-16 구두 : 공통 제어바의 확대는 **지금 보고 있는 화면 그대로**(단일/멀티) 크게 보기 */
$('#dtFull').onclick = () => openCurrentViewPopup($('#dtFull'));
/* ── 조작 바 양끝 = 클립(구간) 건너뛰기 (2026-09-16 구두) ─────────────────────────
   지금 커서가 있는 자리를 기준으로 앞/뒤 구간의 **시작 지점**으로 옮긴다.
   커서를 옮기면 영상·카메라명·PIP·지도는 기존 동기화(`_sync`)가 알아서 따라온다. */
function tlClipJump(dir) {
  /* 2026-09-16 구두 : 인물이 2명 이상이면 **보이는 트랙 전부**의 구간을 모아
     그중 가장 가까운 시작 지점으로 옮긴다 (종전에는 첫 트랙만 봐서 1명일 때만 됐다). */
  const all = (typeof vidTracks === 'function' ? vidTracks() : (TL.tracks || []));
  const trs = (all && all.length) ? all : (TL.tracks || []);
  const clips = trs.flatMap(t => (t && t.clips) || []);
  if (!clips.length || TL.d0 == null || !TL.span) return;
  /* 여러 트랙의 시작 지점을 한 줄로 합치고 같은 시각은 하나로 */
  const starts = [...new Set(clips.map(c => +tlTime(c.from)))].sort((a, z) => a - z);
  const now = TL.d0 + TL.cursor * TL.span;
  let t;
  if (dir > 0) t = starts.find(s => s > now + 500);
  else { const before = starts.filter(s => s < now - 500); t = before.length ? before[before.length - 1] : null; }
  if (t == null) { toast(dir > 0 ? '마지막 구간입니다.' : '첫 구간입니다.'); return; }
  TL.cursor = Math.max(0, Math.min(1, (t - TL.d0) / TL.span));
  const h = (typeof vidHosts === 'function' && vidHosts()[0]) || document.getElementById('dtTl');
  if (h && h._sync) h._sync();
  else if (typeof vidSeekAll === 'function') vidSeekAll();
}
if ($('#dtPrevClip')) $('#dtPrevClip').onclick = () => tlClipJump(-1);
if ($('#dtNextClip')) $('#dtNextClip').onclick = () => tlClipJump(1);
/* 비교·팝업 조작 바의 양끝 버튼도 같은 동작 (공통 속성으로 위임) */
document.addEventListener('click', e => {
  const b = e.target.closest('[data-clipjump]'); if (!b || b.disabled) return;
  e.stopPropagation();
  tlClipJump(+b.dataset.clipjump);
}, true);
/* 재생 버튼은 토글 — 시안에 별도 일시정지 버튼이 없다 */
$('#dtPlay').onclick = () => {
  const b = $('#dtPlay'), on = b.classList.toggle('playing');
  b.title = on ? '일시정지' : '재생';
  b.innerHTML = on
    ? `<i class="i i-24 i-g-pause"></i>`
    : `<i class="i i-24 i-g-play"></i>`;
};
...
/* ============================================================
   ...
   ============================================================ */
function openCompareTab(ids, trackIdx) {
  /* trackIdx 가 오면 그 트랙(인물)의 슬롯·이름을 쓴다 — A·D 를 고르면 A·D 가 나와야 한다 */
  const objs = ids.slice(0, 4).map((id, i) => {
    const o = findObj(id);
    const t = trackIdx && TL_TRACKS[trackIdx[i]];
    const sl = (t && CMP_SLOTS.find(x => x.k === t.slot)) || CMP_SLOTS[i];
    return { ...o, slot: sl.k, label: t ? t.label : sl.label };
  });
  CMP.objs = objs; CMP.tab = '전체'; CMP.openLane = null;
  CMP.trackIdx = trackIdx ? trackIdx.slice(0, 4) : null;
  TL.off = new Set();
  /* 사양 v0.8 PTS_DTL_CMP_000_1 : 경로 비교는 **지금 보던 상세 탭 안에서** 전환된다(새 탭 아님, 탭 제목 유지).
     한 명만 남으면 같은 탭이 다시 단일 상세로 돌아간다 */
  const cur = S.tabs.find(x => x.id === S.activeTab && x.obj);
  const first = objs[0];
  if (cur) {
    if (cur.kind !== 'compare') { cur.baseKind = cur.kind || null; cur.kind = 'compare'; }
    renderTabs(); syncPanels(); return;
  }
  newTab(`${first.label} ${first.cam}`, first, { kind: 'compare' });
}

/* 타일을 16:9로 유지하며 스테이지에 맞춘다 (n2 = 1×2, n4 = 2×2) */
function fitCmpGrid() {
  /* 영역 전환 시 영상 묶음이 오른쪽 칸으로 옮겨가므로 #cmpMain 안에서만 찾지 않는다 */
  const stage = document.querySelector('.cmp-stage'), grid = $('#cmpGrid');
  ...
  pipAutoFit(grid);   /* 칸 크기가 정해진 뒤 PIP 기본 펼침/접힘 */
}
...
/* 경로 비교 추가로 고른 인물이면 타일 i → 그 인물의 트랙 */
const cmpTi = i => (CMP.trackIdx && CMP.trackIdx[i] != null ? CMP.trackIdx[i] : i);
function cmpSlotTrack(i) { return (typeof TL_TRACKS !== 'undefined' && TL_TRACKS[cmpTi(i)]) || null; }
/* 사양 v0.8 경로 비교 1-1) 머리말 = 인물명 · 위치명 · **현재 재생 시각**.
   타임라인 현재 시점에 그 인물의 영상이 없으면 위치명·일시를 '-' 로 */
function cmpClipAt(i) {
  const t = cmpSlotTrack(i); if (!t || !t.clips || TL.d0 == null || !TL.span) return null;
  const now = TL.d0 + TL.cursor * TL.span;
  return t.clips.find(c => +tlTime(c.from) <= now && now <= +tlTime(c.to)) || null;
}
function cmpNowStr() {
  if (TL.d0 == null || !TL.span) return '';
  const d = new Date(TL.d0 + TL.cursor * TL.span), z = n => String(n).padStart(2, '0');
  return `${d.getFullYear()}-${z(d.getMonth() + 1)}-${z(d.getDate())} ${z(d.getHours())}:${z(d.getMinutes())}:${z(d.getSeconds())}`;
}
function cmpSlotCam(i, o) { const c = cmpClipAt(i); return c ? c.cam : '-'; }
function cmpSlotRange(i) { return cmpClipAt(i) ? cmpNowStr() : '-'; }
/* 커서를 옮기면 비교 칸 머리말·빈 구간 표시만 갱신 */
function cmpSyncHeads() {
  const t = S.tabs.find(x => x.id === S.activeTab); if (!t || t.kind !== 'compare') return;
  document.querySelectorAll('#cmpGrid .cmp-tile[data-tile]').forEach(n => {
    const i = +n.dataset.tile, has = !!cmpClipAt(i), off = n.classList.contains('off');
    n.classList.toggle('noclip', !has);
    const nm = n.querySelector('.cmp-head .nm'), tm = n.querySelector('.cmp-head .tm');
    if (nm && !n.dataset.pipSwapped) nm.textContent = off ? '-' : cmpSlotCam(i);
    if (tm) tm.textContent = off ? '' : cmpSlotRange(i);
  });
}
...
function renderCmpView(tab) {
  ...
  for (let i = 0; i < slots; i++) {
    ...
    if (o) {
      ...
      h += `<div class="cmp-tile${TL.off.has(o.slot) ? ' off' : ''}" data-tile="${i}">
        ...
        <div class="cmp-vid"><img src="${pr.main.img}" alt=""></div>
        <!-- 동시 포착은 영상 상자(16:9 레터박스)가 아니라 **칸 하단**에 둔다 — 영상 아래를 가리지 않게 (2026-09-15 구두) -->
        ${i === 0 ? pipHTML('cmp-' + o.slot, pr.list) : ''}
        <!-- 시안 5126:59281 : ■ 인물 A · 위치 · 일시 … 우측 더보기(⋯) -->
        <div class="cmp-head">
          <span class="cmp-lb"><i style="background:${s.color}"></i>${s.label}</span>
          <span class="nm">${pr.main.cam}</span>
          <span class="tm">${cmpSlotRange(i)}</span>
          <!-- 2026-09-15 구두 : 칸이 작아도(3·4명) 아이콘을 모두 보여 준다.
               종전엔 slots >= 3 이면 더보기(⋯ mv-more · data-vmore) 하나로 접었다. -->
          <span class="vh-ic">${VH_BM}</span>
        </div>
      </div>`;
    } else {
  ...
  grid.innerHTML = h;
  /* PIP 카메라 → 그 칸 안에서 메인 영상과 스위칭 (전체 보기 팝업 아님) */
  grid.querySelectorAll('[data-pipcam]').forEach(b => b.onclick = e => { e.stopPropagation(); pipSwap(b); });
  requestAnimationFrame(() => { pipAutoFit(grid); cmpSyncHeads(); });
  ...
  grid.querySelectorAll('[data-cmpsim]').forEach(b => b.onclick = e => {
    e.stopPropagation();
    const tile = b.closest('.cmp-tile');
    const had = tile.querySelector('.cmp-sim-pop');
    grid.querySelectorAll('.cmp-sim-pop').forEach(x => x.remove());
    if (had) return;
    const list = MULTI_TILES.slice(0, 3);
    const pop = el('div', 'cmp-sim-pop', `<div class="cs-head"><b>동시 포착 (${list.length})</b><button class="cs-x" title="닫기">&#10005;</button></div>
      <div class="cs-grid">${list.map((t, k) => `<button data-cs="${k}"><img src="${t.img}" alt=""><em>${t.cam}</em></button>`).join('')}</div>`);
    tile.appendChild(pop);
    pop.onclick = ev => ev.stopPropagation();
    pop.querySelector('.cs-x').onclick = () => pop.remove();
    pop.querySelectorAll('[data-cs]').forEach(x => x.onclick = () => { const t = list[+x.dataset.cs]; pop.remove(); openVideoView({ cam: t.cam, img: t.img }); });
  });
  $('#cmpCtrl').innerHTML = ctrlHTML({ dimRight: true });
  bindCmpView();
  fitCmpGrid();
  watchCmpStage();
  $$('#cmpGrid [data-slotmore]').forEach(b => b.onclick = e => {
    e.stopPropagation();
    const i = +b.dataset.slotmore;
    if (i === 0) { toast('기준 대상 타일은 비울 수 없습니다.'); return; }
    CMP.objs = CMP.objs.filter((_, k) => k !== i);
    if (CMP.trackIdx) CMP.trackIdx = CMP.trackIdx.filter((_, k) => k !== i);
    TL.show = new Set(CMP.objs.map((_, k) => k));
    renderCmpView(tab);
  });
  $$('#cmpGrid [data-rmslot]').forEach(b => b.onclick = e => {
    e.stopPropagation();
    const r = +b.dataset.rmslot;
    CMP.objs = CMP.objs.filter((_, i) => i !== r);
    if (CMP.trackIdx) CMP.trackIdx = CMP.trackIdx.filter((_, i) => i !== r);
    TL.show = new Set(CMP.objs.map((_, k) => k));
    renderCmpView(tab);
  });
  ...
  const paths = objs.map(o => MOVE_PATHS.find(p => p.slot === o.slot)).filter(Boolean)
    .map(p => ({ ...p, off: TL.off.has(p.slot) }));   /* 비활성 인물은 지도에서 흐리게 */
...
function renderSetMenu() {
  ...
  if (SET.view === 'main') {
    if (!SET_SPEEDS.includes(SET.speed)) SET.speed = '1.0';
    box.innerHTML = `
      <button class="sm-row" data-setv="speed">${SET_IC.speed}<span class="t">재생 속도</span>
        <span class="v on">${SET.speed}</span>${SET_IC.next}</button>
      <div class="sm-row">${SET_IC.loop}<span class="t">반복 재생</span>
        <button class="sm-sw${SET.loop ? ' on' : ''}" id="setLoop" title="반복 재생"><i></i></button></div>
      <!-- 음량 : 슬라이더 0~100 + 아이콘을 누르면 음소거 토글 (사양 2-7) -->
      <div class="sm-row sm-vol"><button class="sm-mute" id="setMute" type="button"></button><span class="t">음량</span>
        <input type="range" id="setVol" min="0" max="100" value="${SET.muted ? 0 : SET.vol}"></div>`;
    ...
    /* 음량을 끄는 동안 메뉴 전체를 다시 그리면 슬라이더가 교체돼 드래그가 끊겼다 — 아이콘만 갱신 */
    const sv = $('#setVol'), mu = $('#setMute');
    const paintVol = () => {
      const off = SET.muted || SET.vol === 0;
      mu.title = off ? '음소거 해제' : '음소거';
      mu.innerHTML = off
        ? '<svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><path d="M2.5 6h2.3L8 3.4v9.2L4.8 10H2.5z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/><path d="M10.6 6.2l3.4 3.6M14 6.2l-3.4 3.6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>'
        : '<svg viewBox="0 0 16 16" class="ic" aria-hidden="true"><path d="M2.5 6h2.3L8 3.4v9.2L4.8 10H2.5z" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/><path d="M10.4 5.6a3.4 3.4 0 010 4.8M12.2 3.9a5.8 5.8 0 010 8.2" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>';
      sv.value = off ? 0 : SET.vol;
      sv.style.setProperty('--fill', (off ? 0 : SET.vol) + '%');
    };
    sv.oninput = e => { const v = +e.target.value; SET.muted = v === 0; if (v) SET.vol = v; paintVol(); };
    mu.onclick = () => { SET.muted = !SET.muted; if (!SET.muted && !SET.vol) SET.vol = 50; paintVol(); };
    paintVol();
  } else {
    ...
    box.innerHTML = `
      <button class="sm-head" id="setBack">${SET_IC.back}
...
=======
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609211033' : 'assets/img/floor.png?v=202609211033';
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
  ...
  const tools = `<div class="tools">
      <button class="btn-icon${DT.mapTools.includes('cctv') ? ' on' : ''}" data-mvt="cctv" title="주변 카메라 표시"><i class="i i-16 i-tool-cctv"></i></button>
  ...
  const conesHTML = DT.mapTools.includes('cctv') ? MAP_CCTV.map((c, i) =>
    `<span class="map-cone" data-mvc="${i}" style="left:${c.x}%;top:${c.y}%;rotate:${c.deg - 90}deg"><i></i><b></b></span>`).join('') : '';
...
function toggleMovePath(anchor) {
  ...
  /* 사양 : 시간 순 행 · 탐지 순서 / 위치명 / 출현 일시 / 카메라명,
     이전 지점과 공간 속성이 바뀌면 `층 이동` · `구역 이동` 표시,
     행을 누르면 해당 지점 지도로 전환·마커 포커스·타임라인 재생 위치 이동 */
  const paths = mvwPaths();
  const day = (typeof TL_TRACKS !== 'undefined' && TL_TRACKS[0] && TL_TRACKS[0].clips[0]) ? TL_TRACKS[0].clips[0].from.slice(0, 10) : '2026-06-29';
  const zone = c => /외부/.test(c) ? '외부' : 'A동';
  /* 시안 5223:58721 : 인물마다 칸(160) — 경로 비교면 칸을 옆으로 나란히.
     행(48) = 번호 · 위치명 · `시각 • 카메라 코드`, 층·구역이 바뀌는 곳에 회색 띠.
     예전엔 행에 지도 전체 보기의 절대 배치 클래스(mvw-wp)를 써서 행이 한자리에 겹쳤다 */
  $('#movePathBody').innerHTML = `<div class="mpl-cols">${paths.map(x => `
    <div class="mpl-col" data-slot="${x.slot}">
      <div class="mpl-who"><i style="background:${slotColor(x.slot)}"></i>${x.label}</div>
      <div class="mpl-list">${x.pts.slice().sort((a, b) => a.n - b.n).map((t, i, arr) => {
        const pv = arr[i - 1];
        const sep = !pv ? '' : zone(pv.cam) !== zone(t.cam) ? '구역 이동' : m3FloorOf(pv.cam) !== m3FloorOf(t.cam) ? '층 이동' : '';
        return `${sep ? `<div class="mpl-sep">${sep}</div>` : ''}
          <button type="button" class="mpl-row" data-mphh="${t.hh}" title="${day} ${t.t} · ${t.cam} · ${t.code}">
            <span class="n" style="background:${slotColor(x.slot)}">${t.n}</span>
            <span class="bd"><span class="nm">${t.cam}</span><span class="mt">${t.t}<i></i>${t.code}</span></span>
          </button>`;
      }).join('')}</div>
    </div>`).join('')}</div>`;
  p.hidden = false;
  /* 버튼 왼쪽에 붙이고, 자리가 없으면 오른쪽 · 화면 밖으로 나가지 않게 */
  const r = anchor.getBoundingClientRect(), pw = p.offsetWidth, ph = p.offsetHeight;
  let left = r.left - pw - 8; if (left < 8) left = Math.min(r.right + 8, innerWidth - pw - 8);
  p.style.left = Math.max(8, left) + 'px';
  p.style.top = Math.max(8, Math.min(r.top, innerHeight - ph - 8)) + 'px';
  /* 인물 칸마다 지금 커서 시각의 지점을 표시 */
  const cols = [...$('#movePathBody').querySelectorAll('.mpl-col')];
  const mark = () => { const now = cursorHours();
    cols.forEach(col => { const rows = [...col.querySelectorAll('[data-mphh]')]; let cur = null;
      rows.forEach(n => { if (+n.dataset.mphh <= now) cur = n; });
      rows.forEach(n => n.classList.toggle('on', n === (cur || rows[0]))); }); };
  $('#movePathBody').querySelectorAll('[data-mphh]').forEach(n => n.onclick = () => { seekToHour(+n.dataset.mphh); mark(); });
  mark();
}
...
function openVideoView(src) {
  ...
  $('#vwBody').innerHTML = `<div class="vw">
    <div class="vw-stage">
      <img src="${img}" alt="">
      <div class="vw-head">
        <span class="nm">${cam}</span>
        <span class="tm">${(() => { const c = (typeof TL_TRACKS !== 'undefined' && TL_TRACKS[0] && TL_TRACKS[0].clips[(TL.sel && TL.sel.c) || 0]) || null;
          return c ? `${c.from.slice(0, 19)} ~ ${c.to.slice(11, 19)}` : '2026-06-29 00:52:03 ~ 02:10:11'; })()}</span>
        <span class="vh-ic">${VH_BM}</span>
      </div>
    </div>
    <!-- 시안 5223:57487 : 영상 아래 6px 재생 막대(버퍼 · 재생 위치) + 가운데 재생 조작만 -->
    <div class="vw-prog" title="재생 위치"><i class="buf" style="width:70%"></i><i class="cur" style="width:27%"></i></div>
    <!-- 단일 전체보기에서도 화면 보기 모드(원본/맞추기/자르기)를 우측 하단에 (2026-09-16 구두) -->
    <div class="dt-ctrl vw-ctrl"><div class="grp l"></div><div class="grp c">${playCtrlHTML()}</div>
      <div class="grp r"><button class="btn-icon" data-vratio-all data-ratio-state="원본 비율" title="화면 비율 변경 (원본 비율)"><i class="i i-24 i-g-ratio-original"></i></button></div></div>
  </div>`;
  const pg = $('#vwBody .vw-prog');
  if (pg) pg.onclick = e => {
    const r = pg.getBoundingClientRect();
    pg.querySelector('.cur').style.width = Math.max(0, Math.min(100, (e.clientX - r.left) / r.width * 100)) + '%';
  };
  openModal('#mdVideo');
}
...
function applyDemo() {
  ...
  /* 여러 날에 걸쳐 고른 경우 (2026-09-18) — 눈금을 하루로 자르면 뒤쪽 클립이 100% 를 넘어
     눈금 오른쪽 밖으로 나갔다. 06-28 · 06-29 · 06-30 에서 골라 3일치 눈금을 만든다. */
  if (d === 'clipselmulti') {
    setMode('text'); S.q = '검정색 모자를 쓴 배송기사'; qText.value = S.q; buildFilters('text'); runSearch(false);
    openReid(OBJECTS[0].id);
    REID.clipSel = new Set(['o25', 'o26', 'o13', 'o14', 'o01', 'o02'].filter(id => findObj(id)));
    renderClips([...REID.sel]);
  }
  if (d === 'personmgr') { switchMode('person'); renderPersonGrid(); openPersonMgr(); }
  if (d === 'history')   { HIST.open.add('0-0'); $('#btnHistory').click(); }
  if (d === 'imagemodal'){ setMode('person'); setPfWay('img'); loadImage(PIMG('v01', 0)); }
...
function openImgSearch(src) {
  IMS.src = src || (OBJECTS[0] || {}).img || 'assets/video/v01.jpg';
...
/* ── 맵 상단 버튼 동작 연결 (2026-09-09) ────────────────────────────────
   시안에만 있고 눌러도 아무 일이 없던 것들을 이어준다. */
(function bindMapHead() {
  /* 레이아웃 토글 : 한 화면 / 위아래 둘로 */
  /* 사양 (3) 멀티 뷰 : 둘째 창에 다음 탐지 구역 (없으면 직전 구역) */
  document.querySelectorAll('#mapLay [data-ml]').forEach(b => b.onclick = () => {
    const two = b.dataset.ml === '2';
    if (two && zonesInPath().length < 2) { toast('탐지 이력이 한 구역에만 있어 멀티 뷰를 쓸 수 없습니다.'); return; }
    DT.split = two;
    renderMapLayers(MAP_PATHS_CACHE);
    toast(two ? '맵을 두 구역으로 나눠 봅니다.' : '맵을 한 화면으로 봅니다.');
  });
  /* 자리 바꾸기 : 좌측 영상과 우측 맵을 맞바꾼다 */
  const swap = document.getElementById('mapSwap');
  if (swap) swap.onclick = () => swapAreas();
  /* 동 선택 */
  /* 사양 4-1) 층 이동 드롭다운 : 항목 선택 = 그 층(구역) 지도로 즉시 전환 · 커서 자동 추종 중지.
     다층 뷰에서 층을 고르면 그 층 단독 도면으로 바꾼다(다층 뷰에선 층 선택이 보이지 않아 반응이 없어 보였다).
     버튼 글자는 setBldg 가 이름/탐지 수로 나눠 그리므로 공용 bindSelect 대신 따로 묶는다 */
  const bl = document.getElementById('dtBldg');
  if (bl) {
    bl.querySelector('.select-btn').onclick = e => {
      e.stopPropagation();
      document.querySelectorAll('.select').forEach(x => x !== bl && x.classList.remove('open'));
      bl.classList.toggle('open');
    };
    bl.querySelector('.select-menu').onclick = e => {
      const it = e.target.closest('div[data-v]'); if (!it || it.classList.contains('disabled')) return;
      e.stopPropagation(); bl.classList.remove('open');
      const z = it.dataset.zone, fl = it.dataset.fl;
      if (z === DT.zone && (!fl || fl === DT.flatFloor) && !DT.layers) return;
      if (DT.layers && z === 'A동') { DT.layers = false; const lb = document.querySelector('#dtMapTools [data-map="layers"]'); if (lb) lb.classList.remove('on'); }
      DT.zone = z; DT.zoneAuto = false; if (fl) DT.flatFloor = fl;
      renderMapLayers(MAP_PATHS_CACHE);
    };
  }
  /* 인접 동 칩 */
  document.addEventListener('click', e => {
    const b = e.target.closest('.pn-bldg button');
    if (b) toast(b.innerText.trim() + ' 으로 이동합니다.');
  });
})();
...
function renderSettings() {
  ...
  if (setNav === 'path') {
    ...
    body = row('결과 보기 모드',
      ...
      + row('3D 지도 조작 안내',
        `<div class="seg" id="st3d">${[['1', '호버 툴팁'], ['2', '하단 안내'], ['3', '3D 버튼']].map(([k, t]) =>
          `<button class="${String(S.m3dHint || '1') === k ? 'on' : ''}" data-st3d="${k}">${t}</button>`).join('')}</div>`,
        '다층 뷰에서 드래그로 돌려 볼 수 있음을 알리는 방식입니다. 호버 툴팁은 3초 뒤 사라지고, 3D 버튼은 눌러야 회전됩니다.');
  } else {
...
=======
          <img src="assets/img/map.png?v=202609211033" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
          ${c.path.map((p, i) => {
...
/* 차량번호 최근 검색도 **텍스트 검색과 같은 방식**으로 (2026-09-16 구두).
   종전에는 입력창 아래 별도 블록이라 아래 필터를 밀어냈다 →
   텍스트와 같이 입력창 안에서 펼쳐지는 드롭다운(`.recent-blk` + `rc-open`)으로 통일한다.
   (옛 `#carRecent` 자리는 비워 두기만 한다) */
function renderCarRecent() {
  const old = $('#carRecent'); if (old) old.innerHTML = '';
  const blk = $('#carRecentBlk'); if (!blk) return;
  const open = !S.searched && S.carRecent.length && S.carRecentOpen;
  blk.hidden = !open;
  ...
  blk.innerHTML = `<div class="rb-head"><span class="rb-lb">최근 검색</span>
      <button type="button" class="rb-clear" data-carrall>전체 삭제</button></div>
    <div class="rb-list">${S.carRecent.map((v, i) =>
      `<div class="rb-li" data-carrc="${i}"><span class="t">${v}</span>
         <button class="x" data-carrx="${i}" title="삭제"><i class="i i-12 i-close"></i></button></div>`).join('')}</div>`;
  const clr = blk.querySelector('[data-carrall]');
  if (clr) clr.onmousedown = e => { e.preventDefault(); S.carRecent = []; renderCarRecent(); };
  $$('#carRecentBlk [data-carrc]').forEach(r => r.onclick = e => {
  ...
  $$('#carRecentBlk [data-carrx]').forEach(b => b.onclick = e => {
...
(function initCar() {
  ...
  /* 텍스트 검색과 같은 드롭다운 자리를 입력창 안에 만든다 (2026-09-16 구두) */
  const wrap = t.closest('.textarea-wrap');
  if (wrap && !document.getElementById('carRecentBlk')) {
    const blk = document.createElement('div');
    blk.className = 'recent-blk'; blk.id = 'carRecentBlk'; blk.hidden = true;
...
(function initTextA() {
  ...
  /* 입력창 안 검색 버튼 삭제 — 엔터로 실행한다 (2026-09-10 사양) */
...
(function initSearchGo() {
  ...
  if (rs) rs.onclick = () => {
    ...
    S.sim = 80; S.allResults = false; S.period = '오늘';
    S.top = []; S.bottom = []; S.carTypes = []; S.cams = CAMERAS.slice();
    /* 입력한 검색어도 비운다 — 조건만 돌아가고 글자는 남아 있었다 (2026-09-15 구두) */
    S.q = '';
    { const t = document.getElementById('qText'); if (t) t.value = ''; const x = document.getElementById('qTextClear'); if (x) x.hidden = true; }
...
/* 초기 렌더가 끝났으니 화면을 드러낸다 (첫 깜빡임 방지).
   이 스크립트는 body 끝에서 돌므로 이 시점이면 DOM 은 이미 그려져 있다.
   rAF 에만 맡기면 그게 지연될 때 화면이 영영 안 보인다 — 즉시 떼고,
   혹시 위에서 예외가 났을 때를 대비해 타이머로 한 번 더 보장한다. */
document.body.classList.remove('booting');
setTimeout(() => document.body.classList.remove('booting'), 800);

/* 맵 확대/축소 — 다층 뷰(3D)에서는 카메라 거리로 */
(function bindMapZoom() {
  const z = document.querySelector('#dtMap .dt-zoom'); if (!z) return;
  z.addEventListener('click', e => {
    const b = e.target.closest('button'); if (!b) return;
    const inn = b === z.firstElementChild;
    if (DT.zone === '외부') { e.stopPropagation(); zoneMapZoom(document.getElementById('dtMap'), inn ? 1 : -1); return; }
    if (!DT.layers) { e.stopPropagation(); flatZoom(inn ? 1 : -1); return; }
...
/* 설정 > 화면 표시 : 3D 지도 조작 안내 방식 (시안 제안 1·2·3) */
document.addEventListener('click', e => {
  const b = e.target.closest('[data-st3d]'); if (!b) return;
  S.m3dHint = b.dataset.st3d;
  try { localStorage.setItem('svms.m3dHint', S.m3dHint); } catch (_) {}
  b.parentElement.querySelectorAll('[data-st3d]').forEach(x => x.classList.toggle('on', x === b));
  if (typeof M3D !== 'undefined') M3D.applyVariant();
});
try {
  const q = new URLSearchParams(location.search).get('m3d');
  S.m3dHint = (q && /^[123]$/.test(q)) ? q : (localStorage.getItem('svms.m3dHint') || '1');
} catch (_) { S.m3dHint = '1'; }

/* 사양 2-5) 배속 재생 : 정방향/역방향 x1 → x2 → x4 → x8 순환 · 10초 앞/뒤 이동 */
(function bindPlayRate() {
  const RATES = [1, 2, 4, 8];
  DT.rate = 1; DT.rateDir = 1;
  const ctrl = document.getElementById('dtCtrl'); if (!ctrl) return;
  const fwd = ctrl.querySelector('[title="배속 앞으로"]'), back = ctrl.querySelector('[title="배속 뒤로"]');
  const paint = () => {
    [fwd, back].forEach(b => { if (b) { b.removeAttribute('data-rate'); b.classList.remove('on'); } });
    if (DT.rate > 1) { const b = DT.rateDir > 0 ? fwd : back; if (b) b.classList.add('on'); }
    const bd = document.getElementById('dtRateBadge');
    if (bd) { bd.hidden = DT.rate <= 1; bd.innerHTML = DT.rateDir > 0 ? `x${DT.rate}<i>▶▶</i>` : `<i>◀◀</i>x${DT.rate}`; }
  };
  const step = dir => {
    if (dir !== DT.rateDir) { DT.rateDir = dir; DT.rate = 2; }
    else DT.rate = RATES[(RATES.indexOf(DT.rate) + 1) % RATES.length];
    paint();
  };
  if (fwd) fwd.onclick = () => step(1);
  ...
  if (b10) b10.onclick = () => nudge(-10000);
  if (f10) f10.onclick = () => nudge(10000);
})();
...
/* 상태별 아이콘 — 시안 `변경된 아이콘`(5318:87771) 에서 받은 SVG 3종.
   ic_원본비율 → g-ratio-original · ic_맞추기 → g-ratio-fit · ic_자르기 → g-ratio-crop */
const V_RATIO_IC = { '원본 비율': 'i-g-ratio-original', '맞추기': 'i-g-ratio-fit', '자르기': 'i-g-ratio-crop' };
function syncVRatioBtn(btn, v) {
  if (!btn) return;
  btn.dataset.ratioState = v;
  btn.title = '화면 비율 변경 (' + v + ')';
  /* 상태마다 아이콘이 다르므로(5318:87771) 활성 강조는 쓰지 않는다.
     종전 규칙(`기본값이 아닐 때만 on`)은 기본 표시가 실제로는 자르기(cover)라
     처음부터 버튼이 켜진 것처럼 보였다. */
  btn.classList.remove('on');
  const ic = btn.querySelector('i');
  if (ic) ic.className = 'i i-24 ' + (V_RATIO_IC[v] || V_RATIO_IC[V_RATIOS[0]]);
...
function applyVRatio(img, v) {
  if (!img) return;
  img.dataset.ratio = v;
  img.style.objectFit = v === '맞추기' ? 'fill' : v === '자르기' ? 'cover' : 'contain';
  if (typeof vMvRemember === 'function') vMvRemember(img);
}
/* 공통 제어바의 비율 버튼 = **보이는 영상 전부**를 한 번에 바꾼다 (2026-09-16 구두).
   칸마다 있던 버튼을 없애고 하단 한 곳에서 일괄 제어한다. */
function vRatioAllImgs() {
  /* 팝업(영상 조회)이 열려 있으면 그 안의 영상이 대상이다 */
  const md = document.getElementById('mdVideo');
  if (md && !md.hidden) return [...md.querySelectorAll('.vg-tile > img, .vw-stage > img')].filter(e => e.getClientRects().length);
  const sel = '#dtMulti .mv-tile > img, #cmpGrid .cmp-tile .cmp-vid > img, #dtVideoImg';
  return [...document.querySelectorAll(sel)].filter(e => e.getClientRects().length);
}
document.addEventListener('click', e => {
  const b = e.target.closest('[data-vratio-all]'); if (!b) return;
  e.stopPropagation();
  const imgs = vRatioAllImgs();
  const cur = imgs.length ? vRatioCurrent(imgs[0]) : V_RATIOS[0];
  const next = V_RATIOS[(V_RATIOS.indexOf(cur) + 1) % V_RATIOS.length];
  imgs.forEach(im => applyVRatio(im, next));
  syncVRatioBtn(b, next);
}, true);

/* 화면에 떠 있는 비율 버튼을 **실제 영상 상태**에 맞춘다.
   마크업의 기본값(원본 비율)과 CSS 기본(cover = 자르기)이 어긋나 있어
   첫 누름에서 아이콘이 안 바뀐 것처럼 보이던 것을 없앤다. */
function syncVRatioAll() {
  document.querySelectorAll('[data-vratio]').forEach(b => {
    if (!b.getClientRects().length) return;
    syncVRatioBtn(b, vRatioCurrent(vRatioTarget(b)));
  });
  /* 공통 버튼은 보이는 영상들의 상태를 대표로 따른다 */
  const g = document.querySelector('[data-vratio-all]');
  if (g && g.getClientRects().length) {
    const imgs = vRatioAllImgs();
    syncVRatioBtn(g, imgs.length ? vRatioCurrent(imgs[0]) : V_RATIOS[0]);
  }
}
document.addEventListener('click', e => {
  /* 북마크 : 저장하면 아이콘 활성, 활성 상태에서 다시 누르면 삭제 */
  const bm = e.target.closest('[data-vbm]');
  if (bm) {
    e.stopPropagation();
    const on = bm.classList.toggle('on');
    toast(on ? '북마크에 저장되었습니다.' : '북마크가 삭제되었습니다.');
    return;
  }
...
function openVRatioPop(rb) {
  ...
  pop.innerHTML = V_RATIOS.map(v => `<button class="${v === cur ? 'on' : ''}" data-vr="${v}">${v}</button>`).join('');
  pop._btn = rb; pop.hidden = false;
...
document.addEventListener('pointerdown', e => {
  ...
  e.preventDefault();
  const z = +img.dataset.zoom || 1, x0 = e.clientX - (+img.dataset.zx || 0), y0 = e.clientY - (+img.dataset.zy || 0);
  const box = img.parentElement.getBoundingClientRect();
  const lim = (v, span) => Math.max(-span * (z - 1) / 2, Math.min(span * (z - 1) / 2, v));
  img.style.cursor = 'grabbing';
  const mv = ev => {
    img.dataset.zx = lim(ev.clientX - x0, box.width); img.dataset.zy = lim(ev.clientY - y0, box.height);
    img.style.transform = `translate(${img.dataset.zx}px, ${img.dataset.zy}px) scale(${z})`;
    vZoomMini(img);
  };
  const up = () => { img.style.cursor = 'grab'; removeEventListener('pointermove', mv); removeEventListener('pointerup', up); };
  addEventListener('pointermove', mv); addEventListener('pointerup', up);
});

/* 다중 타일 `⋯` 메뉴 : 북마크 · 영상 비율 · 전체 보기 / 전체보기 버튼 */
document.addEventListener('click', e => {
  const vf = e.target.closest('[data-vfull]');
  if (vf) {
    e.stopPropagation();
    const box = vf.closest('.cmp-tile, .mv-tile, .pn-vid, .dt-video');
    if (box && box.closest('#dtMulti')) { document.querySelectorAll('.v-more-menu').forEach(m => m.remove()); openVideoGrid(); return; }
    const img = box && box.querySelector('img');
    const camEl = box && box.querySelector('.nm, em, #dtCam');
    const cam = ((camEl && camEl.textContent) || '').trim() || '영상';
    document.querySelectorAll('.v-more-menu').forEach(m => m.remove());
    if (typeof openVideoView === 'function') openVideoView({ cam, img: img ? img.getAttribute('src') : '' });
    return;
  }
  const mb = e.target.closest('[data-vmore]');
  const open = document.querySelector('.v-more-menu');
  if (!mb) { if (open && !e.target.closest('.v-more-menu')) open.remove(); return; }
  e.stopPropagation();
  const tile = mb.closest('.cmp-tile, .mv-tile, .pn-vid span, .pn-vid') || mb.parentElement;
  if (open) { const same = open._btn === mb; open.remove(); if (same) return; }
  const m = el('div', 'v-more-menu', `
    <button data-vbm>북마크</button><button data-vratio>영상 비율</button><button data-vfull>전체 보기</button>`);
  m._btn = mb; tile.appendChild(m);
}, true);

/* 인물 선택 팝업 (시안 5184:99800) — 경로를 편집할 인물 한 명을 고른다 */
function openPersonPick(tracks, onPick) {
  let md = document.getElementById('mdPersonPick');
  if (!md) { md = el('div', 'modal md-person-pick'); md.id = 'mdPersonPick'; md.hidden = true; document.body.appendChild(md); }
  md.innerHTML = `
    <div class="md-head"><div><b>인물 선택</b><p class="md-sub">경로를 편집할 인물을 선택해 주세요.</p></div>
      <button class="btn-icon md-x" data-ppclose title="닫기"><i class="i i-g-close"></i></button></div>
    <div class="pp-list">${tracks.slice(0, 4).map((t, i) => `
      <label class="pp-row${i === 0 ? ' on' : ''}"><input type="radio" name="ppk" value="${i}" ${i === 0 ? 'checked' : ''}>
        <i class="rd"></i><span class="nm">${t.label}</span><img src="${(t.clips[0] || {}).img || ''}" alt=""></label>`).join('')}</div>
    <div class="md-foot"><button class="btn-ghost" data-ppclose>취소</button><button class="btn-primary" id="ppGo">선택</button></div>`;
  md.querySelectorAll('input[name=ppk]').forEach(r => r.onchange = () =>
    md.querySelectorAll('.pp-row').forEach(x => x.classList.toggle('on', x.contains(r) && r.checked)));
  md.querySelectorAll('[data-ppclose]').forEach(b => b.onclick = () => closeModal('#mdPersonPick'));
  md.querySelector('#ppGo').onclick = () => {
    const k = +md.querySelector('input[name=ppk]:checked').value;
    closeModal('#mdPersonPick'); onPick(k);
  };
  openModal('#mdPersonPick');
}

/* 영역 검색 · 가상펜스 검색 팝업 (시안 5184:99734 · 5184:99673)
   영역은 캡처 화면과 지정 영역을 함께 보이고, 가상펜스는 추출 목록만.
   지정한 영역·선이 너무 작으면 추출 결과 없음 상태를 보인다(시연). */
function openAreaSearch(a) {
  const line = a.mode === 'line';
  let md = document.getElementById('mdAreaSearch');
  if (!md) { md = el('div', 'modal md-area'); md.id = 'mdAreaSearch'; md.hidden = true; document.body.appendChild(md); }
  md.classList.toggle('fence', line);
  const src = (document.getElementById('dtVideoImg') || {}).getAttribute ? document.getElementById('dtVideoImg').getAttribute('src') : '';
  const close = () => closeModal('#mdAreaSearch');   /* 취소·닫기 : 기존 드로잉 화면으로 복귀 */
  if (!line) {
    /* v0.8 PTS_DTL_SRC_000_2 : 영역(ROI)에서 자동 검출된 인물 → 선택 → 검색 → 영상 선택 팝업 */
    const empty = !a.single && a.rect[2] * a.rect[3] < 40;
    const items = empty ? [] : (a.single ? OBJECTS.slice(0, 1) : OBJECTS.slice(0, 5)).map((o, i) => ({ id: 'as' + i, img: o.img, kind: i % 2 ? '외형' : '얼굴', t: String(o.t).slice(11, 19), o }));
    let kind = '전체', sel = a.single && items[0] ? items[0].id : null;
    const render = () => {
      ...
      md.querySelectorAll('[data-ask]').forEach(b => b.onclick = () => { kind = b.dataset.ask; render(); });
      md.querySelectorAll('[data-asid]').forEach(b => b.onclick = () => { sel = sel === b.dataset.asid ? null : b.dataset.asid; render(); });
      const go = md.querySelector('#asGo');
      if (go) go.onclick = () => {
        const it = items.find(x => x.id === sel); close();
        DT.area = null; renderArea(); applyTools();
        openReid((it && it.o.id) || OBJECTS[0].id);   /* 선택 인물로 전체 영상·타임라인 재검색 → 영상 선택 */
      };
    };
    render(); openModal('#mdAreaSearch'); return;
  }
...
/* ============================================================
   구역 모델 — A동(건물 도면·다층 뷰) / 외부(외곽 지도)
   사양 PTS_DTL_MAP_000_1 (2) 외부 구역 뷰 · (3) 멀티 뷰 · 4-1 영역 전환
   ============================================================ */
function zoneOfCam(c) { return /외부/.test(String(c || '')) ? '외부' : 'A동'; }
function zonePathPts() { return (((typeof MAP_PATHS_CACHE !== 'undefined' && MAP_PATHS_CACHE && MAP_PATHS_CACHE[0])
  || (typeof MOVE_PATHS !== 'undefined' && MOVE_PATHS[0]) || { pts: [] }).pts).slice().sort((a, b) => a.hh - b.hh); }
function zonesInPath() { return [...new Set(zonePathPts().map(t => zoneOfCam(t.cam)))]; }
function zoneAt(now) { const seq = zonePathPts(); if (!seq.length) return 'A동';
  let at = seq[0]; seq.forEach(t => { if (t.hh <= now) at = t; }); return zoneOfCam(at.cam); }
function zoneCounts() { const c = {}; zonePathPts().forEach(t => { const z = zoneOfCam(t.cam); c[z] = (c[z] || 0) + 1; }); return c; }
/* 다음 탐지 구역 — 없으면 직전 탐지 구역 */
function nextZone(cur) {
  const seq = zonePathPts(), now = cursorHours();
  const a = seq.find(t => t.hh > now && zoneOfCam(t.cam) !== cur); if (a) return zoneOfCam(a.cam);
  const b = seq.filter(t => t.hh <= now && zoneOfCam(t.cam) !== cur).pop(); return b ? zoneOfCam(b.cam) : null;
}
/* 구역 드롭다운 : `구역 (N건)`, 다른 창에 떠 있는 구역은 비활성 */
function setBldg(sel, zone, dis) {
  const s = document.querySelector(sel); if (!s) return;
  /* 사양 v0.8 4-1) 층 이동 드롭다운 : 인물이 검출된 층만 `A동 > 1층 (탐지 수)` · 외부 구역은 `외부 (N)` */
  const pts = zonePathPts(), items = [];
  const flLabel = k => (M3_FLOORS.find(f => f.key === k) || {}).label || k;
  M3_FLOORS.forEach(f => { const n = pts.filter(t => zoneOfCam(t.cam) === 'A동' && m3FloorOf(t.cam) === f.key).length;
    if (n) items.push({ zone: 'A동', fl: f.key, text: `A동 > ${flLabel(f.key)} (${n})` }); });
  const nOut = pts.filter(t => zoneOfCam(t.cam) === '외부').length;
  if (nOut) items.push({ zone: '외부', fl: '', text: `외부 (${nOut})` });
  const curFl = zone === 'A동' ? (sel === '#dtBldg' ? (DT.flatFloor || '1F') : null) : '';
  const cur = items.find(x => x.zone === zone && (curFl == null || x.fl === curFl)) || items.find(x => x.zone === zone) || { text: zone };
  s.dataset.value = cur.text; s.title = cur.text;
  /* 시안 4876:149741 : 이름과 탐지 수 `(5)` 는 색을 나눈다 */
  const parts = t => { const m = /^(.*?)\s*(\(\d+\))$/.exec(t); return m ? [m[1], m[2]] : [t, '']; };
  const b = s.querySelector('.select-btn');
  if (b && sel === '#dtBldg') {
    const [t, n] = parts(cur.text);
    b.innerHTML = `<span class="bl-t">${t}</span>${n ? `<em class="bl-n">${n}</em>` : ''}<i class="i i-16 i-chevron i-down caret"></i>`;
  } else if (b && b.childNodes[0] && b.childNodes[0].nodeType === 3) b.childNodes[0].nodeValue = cur.text;
  const menu = s.querySelector('.select-menu');
  if (menu) menu.innerHTML = items.map(x => { const [t, n] = parts(x.text);
    return `<div data-v="${x.text}" data-zone="${x.zone}" data-fl="${x.fl}" title="${x.text}" class="${x === cur ? 'on' : ''}${x.zone === dis ? ' disabled' : ''}"><span>${t}</span>${n ? `<em>${n}</em>` : ''}</div>`; }).join('');
  if (sel === '#dtBldg') renderZoneNav();
}
/* 4-1) 타 구역 이동 : 이전 / 다음 구역 버튼 (호버 툴팁 `이전 구역(구역명)`) — 경로 비교는 인물별 목록 */
function zoneSeqOf(path) {
  const seq = (path ? path.pts : zonePathPts()).slice().sort((a, b) => a.hh - b.hh), out = [];
  seq.forEach(t => { const z = zoneOfCam(t.cam); if (!out.length || out[out.length - 1].zone !== z) out.push({ zone: z, pt: t }); });
  return out;
}
function zoneNeighbors(path) {
  const segs = zoneSeqOf(path), now = cursorHours(), cur = DT.zone || 'A동';
  const nextSeg = segs.find(g => g.pt.hh > now && g.zone !== cur) || null;
  const prevSeg = segs.filter(g => g.pt.hh <= now && g.zone !== cur).pop() || null;
  return { prev: prevSeg, next: nextSeg };
}
function renderZoneNav() {
  const head = document.querySelector('#dtMapSec .map-ctl'); if (!head) return;
  let nav = head.querySelector('.zone-nav');
  if (!nav) { nav = el('div', 'zone-nav'); head.querySelector('#dtBldg').after(nav); }
  const cmp = (S.tabs.find(x => x.id === S.activeTab) || {}).kind === 'compare';
  const paths = cmp ? (MAP_PATHS_CACHE || []) : [null];
  ...
  nav.querySelectorAll('[data-zn]').forEach(btn => btn.onclick = e => {
    ...
    const go = g => { if (!g) return; DT.zone = g.zone; DT.zoneAuto = false; if (g.zone === 'A동') DT.flatFloor = m3FloorOf(g.pt.cam);
      closeZoneMenu(); renderMapLayers(MAP_PATHS_CACHE); toast(`${g.zone} 지도로 이동했습니다.`); };
    if (!cmp) { go(nb[0][k]); return; }
    /* 경로 비교 : 인물별 이전/다음 구역을 드롭다운으로 */
    closeZoneMenu();
    const m = el('div', 'zone-menu');
    m.innerHTML = nb.map((x, i) => `<button type="button" data-zm="${i}" ${x[k] ? '' : 'disabled'}><i style="background:${slotColor(x.p.slot)}"></i>${x.p.label}<em>${k === 'prev' ? '이전' : '다음'} 구역 (${x[k] ? x[k].zone : '없음'})</em></button>`).join('');
    document.body.appendChild(m);
    const r = btn.getBoundingClientRect(); m.style.left = r.left + 'px'; m.style.top = (r.bottom + 4) + 'px';
    m.querySelectorAll('[data-zm]').forEach(o => o.onclick = ev => { ev.stopPropagation(); go(nb[+o.dataset.zm][k]); });
  });
}
function closeZoneMenu() { document.querySelectorAll('.zone-menu').forEach(m => m.remove()); }
document.addEventListener('click', e => { if (!e.target.closest('.zone-menu')) closeZoneMenu(); });

/* ── 외곽 지도 : 레터박스 · 확대/이동 · 클러스터 ─────────────────────────── */
const ZM_AR = 700 / 636;
/* GUI 260914 (5254:69841) : 경로와 무관한 주변 카메라 무리 — 회색 원(개수). 확대하면 숨긴다 */
const ZM_GRAY = [[22, 22, '40'], [78, 52, '50'], [56, 78, '30'], [12, 55, '100+']];
/* 외부 지도 주변 카메라 (2026-09-15 구두) — 평면 도면 원뿔(MAP_CCTV)은 도면 좌표라 외부 지도에는 없었다.
   출현 지점마다 그 카메라를 지점 옆에 지점 쪽을 보게 두고, 인물이 안 잡힌 카메라 몇 대를 더한다 */
const ZM_CAMS_EXTRA = [[32, 10, '외부 정문 CCTV'], [92, 28, '외부 동측 도로'], [8, 88, '외부 후문 CCTV']];
function zoneCams(groups) {
  if (!(DT.mapTools || []).includes('cctv')) return [];
  const seen = new Map();
  groups.forEach(g => g.pts.forEach(t => { if (!seen.has(t.cam)) seen.set(t.cam, t); }));
  const at = [...seen.values()].map(t => ({ cam: t.cam, x: Math.min(96, t.x + 4), y: Math.max(4, t.y - 6), tx: t.x, ty: t.y }));
  const extra = ZM_CAMS_EXTRA.filter(([, , cam]) => !seen.has(cam)).map(([x, y, cam]) => ({ cam, x, y, tx: x + (x > 50 ? -5 : 5), ty: y + (y > 50 ? -6 : 6) }));
  return at.concat(extra).map(c => ({ ...c, rot: Math.round(Math.atan2(c.ty - c.y, (c.tx - c.x) * ZM_AR) * 180 / Math.PI) - 90 }));
}
function renderZoneMap(host, pts, color) {
  let zm = host.querySelector(':scope > .zone-map');
  if (!zm) {
    zm = el('div', 'zone-map'); host.appendChild(zm);
    host._zm = host._zm || { z: 1, tx: 0, ty: 0 };
    zm.addEventListener('wheel', e => { e.preventDefault(); zoneMapZoom(host, e.deltaY < 0 ? 1 : -1); }, { passive: false });
    zm.addEventListener('mousedown', e => {
      const st = host._zm;
      if (e.button !== 0 || st.z <= 1 || e.target.closest('.zm-pt, .zm-cl, .map-cone, .cctv-pop')) return;
      e.preventDefault();
      const x0 = e.clientX - st.tx, y0 = e.clientY - st.ty;
      zm.classList.add('grabbing');
      const mv = ev => { st.tx = ev.clientX - x0; st.ty = ev.clientY - y0; zoneLayout(host); };
      const up = () => { zm.classList.remove('grabbing'); removeEventListener('mousemove', mv); removeEventListener('mouseup', up); };
      addEventListener('mousemove', mv); addEventListener('mouseup', up);
    });
    if ('ResizeObserver' in window) new ResizeObserver(() => zoneLayout(host)).observe(zm);
  }
  zm.hidden = false;
...
=======
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609211033" alt="외부 지도" draggable="false">
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
      <svg class="zm-path" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${groups.map(g => {
        ...
        return seq.length > 1 ? `<polyline points="${seq.map(t => `${t.x},${t.y}`).join(' ')}" fill="none" stroke="${col(g)}" stroke-width="2.5"
            stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke"${g.off ? ' opacity="0.25"' : ''}/>` : ''; }).join('')}</svg>
      ${ZM_GRAY.map(c => `<span class="zm-gray" style="left:${c[0]}%;top:${c[1]}%">${c[2]}</span>`).join('')}
      ${groups.map(g => { const seq = g.pts.slice().sort((a, b) => a.n - b.n); return seq.map((t, i) => `<span class="map-wp zm-pt${i === seq.length - 1 ? ' last' : ''}" data-pt="z-${g.slot || 'A'}-${t.n}" data-slot="${g.slot || ''}"${g.off ? ' data-off="1"' : ''} data-cam="${t.cam}" data-hh="${t.hh}" data-x="${t.x}" data-y="${t.y}"
        title="${t.cam} · 이 지점으로 이동" style="left:${t.x}%;top:${t.y}%;background:${col(g)}${g.off ? ';opacity:.3' : ''}">${t.n}<span class="zm-lb">${String(t.cam).replace(/^외부\s*/, '')}</span></span>`).join(''); }).join('')}
      ${(zm._cams = zoneCams(groups)).map((c, i) => `<span class="map-cone zm-cone ${coneType(c.cam)}" data-zc="${i}" title="${c.cam}" style="left:${c.x}%;top:${c.y}%;rotate:${c.rot}deg"><i></i><b></b></span>`).join('')}
    </div><div class="zm-cls"></div>`;
  /* 카메라에 마우스 → 미리보기, 누르면 영상 조회 */
  zm.querySelectorAll('.zm-cone').forEach(n => {
    const c = zm._cams[+n.dataset.zc];
    const img = (typeof srcVideo === 'function' && srcVideo(c.cam)) || '';
    n.onmouseenter = () => {
      zm.querySelectorAll('.zm-stage > .cctv-pop').forEach(x => x.remove());
      const pop = el('div', 'cctv-pop', `<img src="${img}" alt=""><span class="cap">${c.cam}${ICON2.ext}</span>`);
      pop.style.left = `calc(${Math.min(c.x, 70)}% + 14px)`; pop.style.top = `calc(${c.y}% - 34px)`;
      pop.onclick = e => { e.stopPropagation(); openVideoView({ cam: c.cam, img }); };
      pop.onmouseleave = () => pop.remove();
      zm.querySelector('.zm-stage').appendChild(pop); n._pop = pop;
    };
    n.onmouseleave = () => setTimeout(() => { if (n._pop && !n._pop.matches(':hover')) { n._pop.remove(); n._pop = null; } }, 260);
    n.onclick = e => { e.stopPropagation(); openVideoView({ cam: c.cam, img }); };
  });
  zoneLayout(host);
}
function zoneLayout(host) {
  const zm = host.querySelector(':scope > .zone-map'); if (!zm || zm.hidden) return;
  const st = host._zm || (host._zm = { z: 1, tx: 0, ty: 0 });
  const W0 = zm.clientWidth, H0 = zm.clientHeight; if (!W0 || !H0) return;
  /* 레터박스 : 원본 비율을 지킨 채 영역 안에 맞추고 남는 곳은 여백 */
  let fw = W0, fh = W0 / ZM_AR; if (fh > H0) { fh = H0; fw = H0 * ZM_AR; }
  const w = fw * st.z, h = fh * st.z;
  const mx = Math.max(0, (w - W0) / 2), my = Math.max(0, (h - H0) / 2);
  st.tx = Math.max(-mx, Math.min(mx, st.tx)); st.ty = Math.max(-my, Math.min(my, st.ty));
  const left = (W0 - w) / 2 + st.tx, top = (H0 - h) / 2 + st.ty;
  const stage = zm.querySelector('.zm-stage');
  Object.assign(stage.style, { left: left + 'px', top: top + 'px', width: w + 'px', height: h + 'px' });
  zm.classList.toggle('zoomed', st.z > 1);
  /* 클러스터 : 화면에서 34px 안으로 붙는 지점을 하나의 배지로 — 확대하면 풀린다 */
  const pts = [...stage.querySelectorAll('.zm-pt')].map(n => ({ n, x: left + (+n.dataset.x) / 100 * w, y: top + (+n.dataset.y) / 100 * h }));
  const groups = [];
  pts.forEach(p => {
    const g = groups.find(q => Math.hypot(q.x - p.x, q.y - p.y) < 34);
    if (g) { g.m.push(p); g.x = g.m.reduce((a, b) => a + b.x, 0) / g.m.length; g.y = g.m.reduce((a, b) => a + b.y, 0) / g.m.length; }
    else groups.push({ x: p.x, y: p.y, m: [p] });
  });
  const cl = zm.querySelector('.zm-cls'); cl.innerHTML = '';
  /* 묶인 지점이 있는 배율 : 낱개 지점도 28 원(이름표 없음) — 시안 클러스터 화면 */
  zm.classList.toggle('clustered', groups.some(g => g.m.length > 1));
  groups.forEach(g => {
    const many = g.m.length > 1;
    g.m.forEach(p => p.n.classList.toggle('in-cl', many));
    if (!many) return;
    /* v0.8 : 묶음 배지 = 순번. 연속이면 `시작~끝`, 비연속이면 `최초순번(+N)` */
    const ns = g.m.map(p => +String(p.n.dataset.pt).split('-').pop()).filter(n => !isNaN(n)).sort((a, b) => a - b);
    const consec = ns.every((n, i) => !i || n === ns[i - 1] + 1);
    const lb = !ns.length ? String(g.m.length) : consec ? `${ns[0]}~${ns[ns.length - 1]}` : `${ns[0]}(+${ns.length - 1})`;
    const b = el('button', 'zm-cl', lb); b.type = 'button';
    b.title = g.m.map(p => p.n.dataset.cam).join(' · ') + ' — 눌러서 확대';
    b.style.left = g.x + 'px'; b.style.top = g.y + 'px';
    b.onclick = e => { e.stopPropagation(); zoneFocus(host, g.x, g.y); };
    cl.appendChild(b);
  });
  if (typeof syncMapToCursor === 'function') syncMapToCursor();
}
function zoneMapZoom(host, dir) {
  const st = host._zm || (host._zm = { z: 1, tx: 0, ty: 0 });
  const nz = Math.max(0.5, Math.min(6, st.z * (dir > 0 ? 1.5 : 1 / 1.5)));   /* 원본보다 작게 0.5 까지 */
  const snap = (st.z - 1) * (nz - 1) < 0 || Math.abs(nz - 1) < 0.05 ? 1 : nz;   /* 원본에서 한 번 멈춤 */
  st.tx *= snap / st.z; st.ty *= snap / st.z; st.z = snap;
  zoneLayout(host);
}
/* 클러스터를 누르면 그 지점을 가운데 두고 두 배로 */
function zoneFocus(host, sx, sy) {
  const zm = host.querySelector(':scope > .zone-map'), st = host._zm, k = 2;
  st.tx = (st.tx - (sx - zm.clientWidth / 2)) * k;
  st.ty = (st.ty - (sy - zm.clientHeight / 2)) * k;
  st.z = Math.min(6, st.z * k);
  zoneLayout(host);
}

/* ── 멀티 뷰 둘째 창 ──────────────────────────────────────────────────── */
function ensureMapPanes() {
  const m = document.getElementById('dtMap'); let w = m.parentElement;
  if (!w.classList.contains('map-panes')) { w = el('div', 'map-panes'); m.before(w); w.appendChild(m); }
  return w;
}
function renderFloorPane(host, pts, color) {
  let fp = host.querySelector(':scope > .zp-floor');
  if (!fp) { fp = el('div', 'zp-floor'); host.appendChild(fp); }
  fp.hidden = false;
  const seq = pts.slice().sort((a, b) => a.hh - b.hh), now = cursorHours();
  const last = seq.filter(t => t.hh <= now).pop() || seq[0];
...
=======
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609211033" alt=""><span class="dt-floor">${flb}</span>
>>>>>>> e7f8b1f (명암비 개선 260918: 글자 크기·색 체계 정리와 선택 표시 1px 통일)
    ...
    ${mine.map(t => `<span class="map-wp" data-cam="${t.cam}" data-hh="${t.hh}" data-x="${flatU(t.x)}" data-y="${t.y}"
      title="${t.cam} · 이 지점으로 이동" style="left:${flatU(t.x)}%;top:${t.y}%;background:${color}">${t.n}</span>`).join('')}
    ${(DT.mapTools || []).includes('cctv') ? MAP_CCTV.map((c, i) => `<span class="map-cone zp-cone ${coneType(c.cam)}" data-zpc="${i}" title="${c.cam}" style="left:${flatU(c.x)}%;top:${c.y}%;rotate:${c.deg - 90}deg"><i></i><b></b></span>`).join('') : ''}</div>`;
  /* 두 번째 칸 도면에도 주변 카메라 — 켜도 첫 칸에만 나오던 것 (2026-09-15 구두) */
  fp.querySelectorAll('.zp-cone').forEach(n => {
    const c = MAP_CCTV[+n.dataset.zpc], stage = fp.querySelector('.zp-stage');
    n.onmouseenter = () => {
      stage.querySelectorAll(':scope > .cctv-pop').forEach(x => x.remove());
      const pop = el('div', 'cctv-pop', `<img src="${c.img}" alt=""><span class="cap">${c.cam}${ICON2.ext}</span>`);
      pop.style.left = `calc(${Math.min(flatU(c.x), 62)}% + 14px)`; pop.style.top = `calc(${c.y}% - 34px)`;
      stage.appendChild(pop); n._pop = pop;
    };
    n.onmouseleave = () => setTimeout(() => { if (n._pop) { n._pop.remove(); n._pop = null; } }, 200);
    n.onclick = e => { e.stopPropagation(); openVideoView(c); };
  });
  const fitZp = () => { const st = fp.querySelector('.zp-stage'); if (!st) return; const W = fp.clientWidth, H = fp.clientHeight; if (!W || !H) return;
    const w = Math.min(W / 1.228, (H / 1.054) * FLAT_IMG.w / FLAT_IMG.h), h = w * FLAT_IMG.h / FLAT_IMG.w;
    Object.assign(st.style, { left: (W - w) / 2 + 'px', top: (H - h) / 2 + 'px', width: w + 'px', height: h + 'px' }); };
  fitZp();
  if (!fp._ro && 'ResizeObserver' in window) { fp._ro = new ResizeObserver(fitZp); fp._ro.observe(fp); }
}
function renderSplitPane(paths) {
  const sec = document.getElementById('dtMapSec'); if (!sec || !paths || !paths[0]) return;
  const wrap = ensureMapPanes();
  const zs = zonesInPath();
  const b2 = document.querySelector('#mapLay [data-ml="2"]');
  if (b2) { b2.disabled = zs.length < 2; b2.title = zs.length < 2 ? '탐지 이력이 한 구역에만 있어 쓸 수 없습니다' : '두 구역 나눠 보기'; }
  const on = DT.split && zs.length > 1;
  sec.classList.toggle('split-on', on);
  document.querySelectorAll('#mapLay [data-ml]').forEach(x => x.classList.toggle('on', x.dataset.ml === (on ? '2' : '1')));
  let p2 = document.getElementById('dtMap2');
  if (!on) { if (p2) p2.hidden = true; return; }
  if (!p2) {
    p2 = el('div', 'dt-map zone-pane'); p2.id = 'dtMap2';
    p2.innerHTML = `<div class="zp-head"><div class="select sm dt-bldg" id="dtBldg2" data-value="">
        <button class="select-btn">-<i class="i i-16 i-chevron i-down caret"></i></button><div class="select-menu"></div></div>
        <div class="zp-nav"><button type="button" class="zn-btn" data-zp="prev" title="이전 구역"><i class="i i-g-map-prev"></i></button><button type="button" class="zn-btn" data-zp="next" title="다음 구역"><i class="i i-g-map-next"></i></button></div></div>
      <div class="zp-zoom"><button type="button" title="확대">+</button><button type="button" title="축소">−</button></div>
      <!-- GUI 260914 (5254:69841) : 칸마다 맵 제어 도구 -->
      <div class="zp-tools">
        <button type="button" class="btn-icon zt-layers" data-zt="layers" title="다층 뷰"><i class="i i-g-mt-layers"></i></button>
        <button type="button" class="btn-icon zt-cctv" data-zt="cctv" title="주변 카메라 표시"><i class="i i-g-mt-camera"></i></button>
        <button type="button" class="btn-icon zt-path" data-zt="path" title="이동 경로 리스트"><i class="i i-g-mt-path"></i></button>
        <button type="button" class="btn-icon zt-in" data-zz="1" title="확대"><i class="i i-g-mt-plus"></i></button>
        <button type="button" class="btn-icon zt-out" data-zz="-1" title="축소"><i class="i i-g-mt-minus"></i></button>
      </div>`;
    wrap.appendChild(p2);
    bindSelect('#dtBldg2', v => { const it = document.querySelector(`#dtBldg2 .select-menu [data-v="${v}"]`); const z = it ? it.dataset.zone : v.split(' ')[0]; if (z === DT.zone) return; DT.zone2 = z; renderMapLayers(MAP_PATHS_CACHE); });
    p2.querySelectorAll('[data-zz]').forEach(b => b.onclick = e => { e.stopPropagation(); if (p2.dataset.zone === '외부') zoneMapZoom(p2, +b.dataset.zz); });
    /* 도구 줄은 위 칸(메인) 도구와 같은 동작 */
    p2.querySelectorAll('[data-zt]').forEach(b => b.onclick = e => { e.stopPropagation(); const m = document.querySelector(`#dtMapTools [data-map="${b.dataset.zt}"]`); if (m) m.click(); });
    /* 이전/다음 구역 : 위 칸과 겹치지 않는 다른 구역으로 */
    p2.querySelectorAll('[data-zp]').forEach(b => b.onclick = e => {
      e.stopPropagation();
      const others = zonesInPath().filter(z => z !== DT.zone); if (!others.length) return;
      const i = Math.max(0, others.indexOf(DT.zone2));
      DT.zone2 = others[(i + (b.dataset.zp === 'next' ? 1 : others.length - 1)) % others.length];
      renderMapLayers(MAP_PATHS_CACHE);
    });
  }
  p2.hidden = false;
  const z2 = (DT.zone2 && DT.zone2 !== DT.zone && zs.includes(DT.zone2)) ? DT.zone2 : (nextZone(DT.zone) || zs.find(z => z !== DT.zone));
  DT.zone2 = z2; p2.dataset.zone = z2;
  setBldg('#dtBldg2', z2, DT.zone);
  const color = slotColor(paths[0].slot), pts = paths[0].pts;
  const zmEl = p2.querySelector(':scope > .zone-map'), fpEl = p2.querySelector(':scope > .zp-floor');
  p2.querySelector('.zp-zoom').hidden = z2 !== '외부';
  const nOther = zs.filter(z => z !== DT.zone).length;
  p2.querySelectorAll('[data-zp]').forEach(b => { b.disabled = nOther < 2; });
  p2.querySelectorAll('[data-zt]').forEach(b => { const m = document.querySelector(`#dtMapTools [data-map="${b.dataset.zt}"]`); b.classList.toggle('on', !!(m && m.classList.contains('on'))); });
  if (z2 === '외부') { if (fpEl) fpEl.hidden = true; renderZoneMap(p2, pts.filter(t => zoneOfCam(t.cam) === '외부'), color); }
  else { if (zmEl) zmEl.hidden = true; renderFloorPane(p2, pts.filter(t => zoneOfCam(t.cam) !== '외부'), color); }
  requestAnimationFrame(() => { bindMapSeek(); syncMapToCursor(); });
}

/* ── 영역 전환 : 영상 ↔ 맵 뷰어 위치를 맞바꾼다 (재선택 시 원래대로) ──────── */
/* 영역 전환(사양 4759:35134 · 시안 5223:57627) : 맵이 넓은 영상 자리로, 영상이 오른쪽 좁은 칸으로.
   상세 화면은 영상(#dtVideo), 경로 비교 화면은 **비교 영상 묶음(칸 + 조작 바)** 을 옮긴다.
   — 비교 화면에서도 상세 영상을 옮기던 탓에 지도가 사라지고 엉뚱한 단일 영상이 떴다. */
function swapTarget() {
  const t = S.tabs.find(x => x.id === S.activeTab);
  return (t && t.kind === 'compare') ? document.querySelector('.cmp-stage') : document.getElementById('dtVideo');
}
function swapRestore() {
  const st = swapAreas.st; if (!st) return;
  /* 제어부를 원래 부모(영상 영역)로 되돌린다 */
  { const ctrl = document.getElementById('dtCtrl'); if (ctrl && st.ctrlHome) st.ctrlHome.appendChild(ctrl); }
  st.mkV.after(st.el); st.mkM.after(st.sec); st.mkV.remove(); st.mkM.remove();
  swapAreas.st = null;
  document.body.classList.remove('dt-swap', 'dt-swap-cmp');
}
function swapAreas() {
  const sec = document.getElementById('dtMapSec'), vid = swapTarget();
  if (!sec || !vid) return;
  const on = !swapAreas.st;
  if (on) {
    const mkV = document.createComment('video-home'), mkM = document.createComment('map-home');
    vid.before(mkV); sec.before(mkM);
    mkV.after(sec); mkM.after(vid);
    swapAreas.st = { el: vid, sec, mkV, mkM };
    /* 2026-09-16 구두 : 영상 제어부는 **원래 자리에 남는다.**
       `#dtCtrl` 이 `#dtVideo` 안에 있어 영상과 함께 옮겨 가던 것을,
       원래 자리를 차지하게 된 맵 영역으로 옮겨 붙인다(복귀 때 되돌린다). */
    const ctrl = document.getElementById('dtCtrl');
    if (ctrl && sec) { swapAreas.st.ctrlHome = ctrl.parentElement; sec.appendChild(ctrl); }
    document.body.classList.add('dt-swap');
    document.body.classList.toggle('dt-swap-cmp', vid.classList.contains('cmp-stage'));
  } else swapRestore();
  if (typeof M3D !== 'undefined') M3D.request();
  if (typeof MAP_PATHS_CACHE !== 'undefined' && MAP_PATHS_CACHE) renderMapLayers(MAP_PATHS_CACHE);
  requestAnimationFrame(() => { if (vid.classList.contains('cmp-stage')) fitCmpGrid(); pipAutoFit(); });
  toast(on ? '맵 뷰어를 넓은 영역으로 옮겼습니다.' : '원래 배치로 되돌렸습니다.');
}

/* 구역 이동 칩의 `›` : 다음 구역 지도로 */
document.addEventListener('click', e => {
  const g = e.target.closest('[data-zgo]'); if (!g) return;
  e.stopPropagation();
  DT.zone = g.dataset.zgo; DT.zoneAuto = false;
  if (typeof MAP_PATHS_CACHE !== 'undefined' && MAP_PATHS_CACHE) renderMapLayers(MAP_PATHS_CACHE);
});

/* ── 평면(2D) 도면 확대·축소 : ± 버튼 · 휠 · 확대 시 드래그 이동 ────────────
   도면·경로·지점을 한 판(.flat-stage)으로 묶어 함께 키운다.
   확대 중에는 현재 위치 마커를 화면 좌표로 계산한다(syncMapToCursor 의 solid). */
const FZ = { z: 1, tx: 0, ty: 0 };
/* GUI 260914 (5254:69839) : 도면은 원본 비율로 가운데 — 레터박스.
   경로 데이터 x 는 예전 화면(592×874 에 도면 763×801 을 cover)의 % 라 도면 기준 % 로 바꿔 쓴다 (y 는 같음) */
const FLAT_IMG = { w: 763, h: 801 }, FLAT_REF = { w: 592, h: 874 };
function flatU(x) {
  const s = Math.max(FLAT_REF.w / FLAT_IMG.w, FLAT_REF.h / FLAT_IMG.h), iw = FLAT_IMG.w * s;
  return ((+x / 100) * FLAT_REF.w + (iw - FLAT_REF.w) / 2) / iw * 100;
}
function fitFlatStage() {
  const map = document.getElementById('dtMap'), st = map && map.querySelector(':scope > .flat-stage'); if (!st) return;
  if (!document.body.classList.contains('dt-b')) { st.style.left = st.style.top = st.style.width = st.style.height = ''; return; }
  const W = map.clientWidth, H = map.clientHeight; if (!W || !H) return;
  /* 흰 판(도면 + 좌우 11.4% · 상하 2.7% 여백)이 영역 안에 들어오게 */
  const bandR = 1.228, bandV = 1.054;
  let w = Math.min(W / bandR, (H / bandV) * FLAT_IMG.w / FLAT_IMG.h), h = w * FLAT_IMG.h / FLAT_IMG.w;
  Object.assign(st.style, { left: (W - w) / 2 + 'px', top: (H - h) / 2 + 'px', width: w + 'px', height: h + 'px' });
}
function flatStage() {
  const map = document.getElementById('dtMap'); if (!map) return null;
  let st = map.querySelector(':scope > .flat-stage');
  if (!st) {
    st = el('div', 'flat-stage');
    const img = document.getElementById('dtMapImg');
    img.before(st);
    ['dtMapImg', 'dtMapPath', 'dtMapCones', 'dtMapWps', 'dtFloor'].forEach(id => { const n = document.getElementById(id); if (n) st.appendChild(n); });   /* 층 배지도 도면 모서리에 (GUI 260914) */
    if ('ResizeObserver' in window) new ResizeObserver(() => { fitFlatStage(); if (typeof syncMapToCursor === 'function') syncMapToCursor(); }).observe(map);
    map.addEventListener('wheel', e => {
      if (DT.layers || DT.zone === '외부' || e.target.closest('.dt-map-tools, .dt-zoom')) return;
      e.preventDefault(); flatZoom(e.deltaY < 0 ? 1 : -1);
    }, { passive: false });
    map.addEventListener('mousedown', e => {
      if (DT.layers || DT.zone === '외부' || FZ.z <= 1 || e.button !== 0) return;
      if (e.target.closest('.map-wp, .dt-map-tools, .dt-zoom, .map-zone, .select, button')) return;
      e.preventDefault();
      const x0 = e.clientX - FZ.tx, y0 = e.clientY - FZ.ty;
      st.classList.add('grabbing');
      const mv = ev => { FZ.tx = ev.clientX - x0; FZ.ty = ev.clientY - y0; applyFlatZoom(); };
      const up = () => { st.classList.remove('grabbing'); removeEventListener('mousemove', mv); removeEventListener('mouseup', up); };
      addEventListener('mousemove', mv); addEventListener('mouseup', up);
    });
  }
  return st;
...
function applyFlatZoom() {
  const st = flatStage(); if (!st) return;
  const map = document.getElementById('dtMap');
  /* 원본보다 작게 줄였을 땐(z<1) 가운데 고정 */
  const mx = Math.max(0, map.clientWidth * (FZ.z - 1) / 2), my = Math.max(0, map.clientHeight * (FZ.z - 1) / 2);
  FZ.tx = Math.max(-mx, Math.min(mx, FZ.tx)); FZ.ty = Math.max(-my, Math.min(my, FZ.ty));
  st.style.transform = FZ.z !== 1 ? `translate(${FZ.tx}px, ${FZ.ty}px) scale(${FZ.z})` : '';
  st.classList.toggle('zoomed', FZ.z > 1);          /* 끌어서 이동은 확대했을 때만 */
  st.classList.toggle('scaled', FZ.z !== 1);        /* 현재 위치 마커는 배율이 바뀌면 화면 좌표로 */
  if (typeof syncMapToCursor === 'function') syncMapToCursor();
}
function flatZoom(dir) {
  /* 축소는 원본의 절반(0.5)까지, 확대는 4배까지 */
  const nz = Math.max(0.5, Math.min(4, FZ.z * (dir > 0 ? 1.4 : 1 / 1.4)));
  /* 원본(1)을 건너뛰면 1에서 한 번 멈춘다 — 0.98·1.02 처럼 어중간한 배율이 남지 않게 */
  const snap = (FZ.z - 1) * (nz - 1) < 0 || Math.abs(nz - 1) < 0.05 ? 1 : nz;
  FZ.tx *= snap / FZ.z; FZ.ty *= snap / FZ.z; FZ.z = snap;
  applyFlatZoom();
}
flatStage(); fitFlatStage();

/* ── 경로 비교 추가 팝업 (사양 2-2 · 시안 경로 비교 추가) ─────────────────────
   동선 생성이 끝난 대상 인물 목록(날짜별). 이미 분석 중인 대상은 체크 고정,
   기준 인원 외 최대 3명(총 4명). 4명이 차면 나머지는 비활성. `추가`는 1명 이상일 때. */
function openCompareAdd(allTracks, shown) {
  let md = document.getElementById('mdCmpAdd');
  if (!md) { md = el('div', 'modal md-cmp-add'); md.id = 'mdCmpAdd'; md.hidden = true; document.body.appendChild(md); }
  const base = new Set(shown && shown.length ? shown : [0]);
  const sel = new Set();
  const groups = {};
  allTracks.forEach((t, i) => { const c = t.clips[0] || {}; const d = String(c.from || '').slice(0, 10) || '-';
    (groups[d] = groups[d] || []).push(i); });
  const render = () => {
    const full = base.size + sel.size >= 4;
    md.innerHTML = `
      <div class="md-head"><div><h3>경로 비교 추가</h3><p>경로를 함께 비교할 인물을 추가해 주세요. (최대 3명)</p></div>
        <button class="btn-icon md-x" data-caclose title="닫기"><i class="i i-g-close"></i></button></div>
      <div class="ca-list">${Object.keys(groups).sort().reverse().map(d => `
        <div class="ca-sec"><div class="ca-date">${d}</div>
        ${groups[d].map(i => {
          const t = allTracks[i], c = t.clips[0] || {};
          const locked = base.has(i), on = locked || sel.has(i), dis = !on && full;
          return `<label class="ca-row${locked ? ' locked' : ''}${dis ? ' dis' : ''}${on ? ' on' : ''}">
            <input type="checkbox" data-cai="${i}" ${on ? 'checked' : ''} ${locked || dis ? 'disabled' : ''}>
            <img src="${c.img || ''}" alt="">
            <span class="bd"><span class="nm"><b>${t.label}</b><em class="cnt" title="출현 지점 ${t.clips.length}곳"><i class="i i-g-pin"></i>${t.clips.length}</em></span>
              <span class="sub">${String(c.from || '').slice(0, 16)}</span></span><i class="ck"></i></label>`;
        }).join('')}</div>`).join('')}</div>
      <div class="md-foot"><button class="btn-ghost" data-caclose>취소</button>
        <button class="btn-primary" id="caGo" ${sel.size ? '' : 'disabled'}>추가</button></div>`;
    md.querySelectorAll('[data-caclose]').forEach(b => b.onclick = () => closeModal('#mdCmpAdd'));
    md.querySelectorAll('[data-cai]').forEach(c => c.onchange = () => {
      const i = +c.dataset.cai; c.checked ? sel.add(i) : sel.delete(i); render(); });
    md.querySelector('#caGo').onclick = () => {
      const idx = [...base, ...sel].sort((a, b) => a - b).slice(0, 4);
      closeModal('#mdCmpAdd');
      const ids = idx.map(i => { const c = allTracks[i].clips[0] || {};
        return (OBJECTS.find(x => x.cam === c.cam) || OBJECTS[0]).id; });
...
/* ── 동시 포착 PIP (시안 5223:57285 기본 · 5223:58374 호버 · 5223:57679 접힘) ─────────────
...
function simulCams(clip) {
  ...
  const all = (typeof MULTI_TILES !== 'undefined' ? MULTI_TILES : []).map(x => ({ cam: x.cam, img: x.img }))
    .filter(x => x.cam !== clip.cam && !extra.some(e => e.cam === x.cam));
  const same = x => m3FloorOf(x.cam) === m3FloorOf(clip.cam) && zoneOfCam(x.cam) === zoneOfCam(clip.cam);
...
function pipHTML(key, list) {
  ...
  return `<div class="pipc${col ? ' col' : ''}" data-pipk="${key}"${col == null ? ' data-auto="1"' : ''}>
    <div class="pip-box">
      <div class="pip-hd"><b>주변 카메라 (${n})</b><button type="button" class="pip-x" data-piptg title="접기">${PIP_X}</button></div>
      <div class="pip-cams">${list.map((x, i) => `<button type="button" class="pip-cam" data-pipcam="${i}" title="${x.cam}"><img src="${x.img}" alt=""><em>${x.cam}</em></button>`).join('')}</div>
    </div>
    <button type="button" class="pip-open" data-piptg title="주변 카메라 펼치기">주변 카메라 (${n})${PIP_MAX}</button>
  </div>`;
}
/* 자리가 좁으면 처음엔 접힌 버튼으로 (펼친 PIP ≈ 420×110) */
function pipAutoFit(root) {
  (root || document).querySelectorAll('.pipc[data-auto]').forEach(p => {
    const host = p.parentElement; if (!host || !host.clientWidth) return;
    const stacked = document.body.classList.contains('dt-swap') && !!host.closest('.cmp-grid, #dtMulti');
    p.classList.toggle('col', stacked || host.clientWidth < 470 || host.clientHeight < 230);
  });
}
addEventListener('resize', () => pipAutoFit());
/* 접기 ✕ / 펼치기 버튼 — 캡처 단계에서 받아 칸 클릭(영상 조회 등)으로 번지지 않게 */
document.addEventListener('click', e => {
  const tg = e.target.closest('[data-piptg]'); if (!tg) return;
  e.stopPropagation(); e.preventDefault();
  const p = tg.closest('.pipc'); if (!p) return;
  const col = !p.classList.contains('col');
  p.classList.toggle('col', col); p.removeAttribute('data-auto');
  PIPS[p.dataset.pipk] = col;
}, true);
...
function pipResolve(key, main, list) {
  ...
  const i = list.findIndex(x => x.cam === sw.cam);
  if (i < 0) { delete PIP_MAIN[key]; return { main, list }; }
  const l = list.slice(); l[i] = { cam: main.cam, img: main.img };
  return { main: { cam: sw.cam, img: sw.img }, list: l };
}
function pipSwap(btn) {
  const p = btn.closest('.pipc'), frame = btn.closest('.cmp-tile, .mv-tile, #dtVideo');
  if (!p || !frame) return;
  const single = frame.id === 'dtVideo';
  const mainImg = single ? document.getElementById('dtVideoImg') : frame.querySelector('.cmp-vid > img, :scope > img');
  const label = single ? document.getElementById('dtCam') : frame.querySelector('.cmp-head .nm, .mv-head .nm');
  const bImg = btn.querySelector('img'), bEm = btn.querySelector('em');
  if (!mainImg || !bImg) return;
  const prev = { cam: label ? label.textContent.trim() : '', img: mainImg.getAttribute('src') };
  const next = { cam: btn.title, img: bImg.getAttribute('src') };
  mainImg.setAttribute('src', next.img); if (label) label.textContent = next.cam;
  bImg.setAttribute('src', prev.img); btn.title = prev.cam; if (bEm) bEm.textContent = prev.cam;
  PIP_MAIN[p.dataset.pipk] = next;
  if (!single) { frame.classList.remove('pip-flash'); void frame.offsetWidth; frame.classList.add('pip-flash'); }
}

/* 클립 길이 편집 : 끌고 있는 끝 지점의 프레임 · 시각을 커서 미리보기와 같은 팝오버로 */
function trimPreview(x, th, clip, t, tr) {
  let pv = document.getElementById('tlCurPv');
  if (!pv) { pv = el('div', 'tl-cur-pop'); pv.id = 'tlCurPv'; pv.hidden = true; document.body.appendChild(pv); }
  pv.innerHTML = `<span class="cp-img" style="border-color:${slotColor(tr ? tr.slot : 'A')}"><img src="${(typeof srcVideo === 'function' && srcVideo(clip.cam)) || clip.img}" alt=""></span>`
    + `<b class="cp-t">${String(t).slice(11, 19).replace(/:/g, ' : ')}</b>`;
  pv.hidden = false;
  const r = th ? th.getBoundingClientRect() : { top: 200 };
  pv.style.left = Math.max(8, Math.min(x - pv.offsetWidth / 2, innerWidth - pv.offsetWidth - 8)) + 'px';
  pv.style.top = Math.max(8, r.top - pv.offsetHeight - 10) + 'px';
}

/* 카메라(위치) 배지 ✕ : 선택 해제 (배지는 접이 머리말 버튼 안에 있어 캡처 단계에서 받는다) */
document.addEventListener('click', e => {
  const x = e.target.closest('[data-camclear]'); if (!x) return;
  e.stopPropagation(); e.preventDefault();
  S.cams = []; if (typeof savePrefs === 'function') savePrefs();
  buildFilters(S.mode); if (typeof markDirty === 'function') markDirty();
}, true);

/* ── 인접 카메라 전환 (사양 v0.8 PTS_DTL_003_1 3) ────────────────────────
   아이콘을 누르면 메인 영상이 그 카메라로 즉시 바뀐다(재생 시점 유지).
   분석·뷰어 도구는 비활성, 머리말 왼쪽 `← 이전 영상` 으로 원래 카메라 복귀 */
function adjSwitch(c) {
  const im = document.getElementById('dtVideoImg'), lb = document.getElementById('dtCam');
  if (!im || !c) return;
  if (!DT.adjPrev) DT.adjPrev = { cam: lb ? lb.textContent : '', img: im.getAttribute('src') };
  im.src = c.img; if (lb) lb.textContent = c.cam;
  DT.tools = DT.tools.filter(x => !['heat', 'path', 'cctv'].includes(x));
  if (typeof applyTools === 'function') applyTools();
  adjPaint();
}
function adjPaint() {
  const on = !!DT.adjPrev;
  const bk = document.getElementById('dtAdjBack'); if (bk) bk.hidden = !on;
  document.querySelectorAll('#dtTools [data-tool="heat"], #dtTools [data-tool="path"], #dtTools [data-tool="cctv"]').forEach(b => b.disabled = on || b.disabled && document.getElementById('dtCtrl').classList.contains('multi-mode'));
}
{ const bk = document.getElementById('dtAdjBack');
  if (bk) bk.onclick = e => {
    e.stopPropagation(); const p = DT.adjPrev; if (!p) return;
    const im = document.getElementById('dtVideoImg'), lb = document.getElementById('dtCam');
    if (im) im.src = p.img; if (lb) lb.textContent = p.cam;
    DT.adjPrev = null;
    document.querySelectorAll('#dtTools [data-tool], #dtAreaTools [data-tool]').forEach(b => { if (['heat', 'path', 'cctv'].includes(b.dataset.tool)) b.disabled = false; });
    if (typeof applyTools === 'function') applyTools();
    adjPaint();
  }; }

/* ── 영상 확대 (사양 v0.8 2-1 · PTS_DTL_004_1 확대 모드) ───────────────────
   `영상 확대` 버튼 → 1.0 / 2x / 4x / 6x 팝오버. 1.0 보다 크면 확대 모드 :
   영상 일시정지 · 분석/뷰어 도구 비활성 · 휠로 배율 조절(팝오버 값 연동) · 드래그 이동 ·
   좌측 하단 전체 썸네일에 현재 보는 부분 표시(드래그로 이동) · ESC 또는 휠 최소 도달 시 1.0 */
function vZoomTarget(btn) { return vRatioTarget(btn); }
function vZoomFrame(img) { return img.closest('.cmp-tile, .vw-stage, .dt-video, .mv-tile, .dt-pane') || img.parentElement; }
function applyVZoom(img, v) {
  if (!img) return;
  const z = v === '1.0' ? 1 : parseInt(v, 10);
  img.dataset.zoom = z; img.dataset.zlabel = v;
  if (z === 1) { img.dataset.zx = img.dataset.zy = 0; }
  img.style.transformOrigin = 'center center';
  img.style.transform = z === 1 ? '' : `translate(${img.dataset.zx || 0}px, ${img.dataset.zy || 0}px) scale(${z})`;
  img.style.cursor = z > 1 ? 'grab' : '';
  img.classList.toggle('v-zoomed', z > 1);
  const frame = vZoomFrame(img);
  const btn = frame && frame.querySelector('[data-vzoom]'); if (btn) btn.classList.toggle('on', z > 1);
  if (frame && frame.id === 'dtVideo') {
    DT.zoomMode = z > 1;
    /* 확대 모드 : 재생 멈춤 · 분석/뷰어 도구 비활성 */
    const pl = document.getElementById('dtPlay');
    if (z > 1 && pl && pl.classList.contains('playing')) pl.click();
    document.querySelectorAll('#dtTools [data-tool="heat"], #dtTools [data-tool="path"], #dtTools [data-tool="cctv"]').forEach(b => { b.disabled = z > 1 || !!DT.adjPrev; });
    document.getElementById('dtCtrl').classList.toggle('zoom-mode', z > 1);
  }
  vZoomMini(img);
  vMvRemember(img);
}
function vMvRemember(img) {
  const t = img.closest('#dtMulti .mv-tile[data-mv]'); if (!t) return;
  if (!DT.mvView) DT.mvView = {};
  DT.mvView[t.dataset.mv] = { ratio: img.dataset.ratio || '원본 비율', zoom: img.dataset.zlabel || '1.0', zx: img.dataset.zx || 0, zy: img.dataset.zy || 0 };
}
function vZoomMini(img) {
  const frame = vZoomFrame(img); if (!frame) return;
  let mini = frame.querySelector(':scope > .v-zoom-mini');
  const z = +img.dataset.zoom || 1;
  if (z <= 1) { if (mini) mini.remove(); return; }
  if (!mini) {
    mini = el('div', 'v-zoom-mini', '<img alt=""><i class="vzm-view"></i>');
    frame.appendChild(mini);
    mini.addEventListener('pointerdown', e => {
      e.preventDefault(); e.stopPropagation();
      const r = mini.getBoundingClientRect();
      const move = ev => {
        const zz = +img.dataset.zoom || 1, box = img.parentElement.getBoundingClientRect();
        const fx = Math.max(0, Math.min(1, (ev.clientX - r.left) / r.width)), fy = Math.max(0, Math.min(1, (ev.clientY - r.top) / r.height));
        const lim = (v, span) => Math.max(-span * (zz - 1) / 2, Math.min(span * (zz - 1) / 2, v));
        img.dataset.zx = lim((0.5 - fx) * box.width * zz, box.width); img.dataset.zy = lim((0.5 - fy) * box.height * zz, box.height);
        img.style.transform = `translate(${img.dataset.zx}px, ${img.dataset.zy}px) scale(${zz})`;
        vZoomMini(img);
      };
      move(e);
  ...
  mini.querySelector('img').src = img.getAttribute('src');
  const box = img.parentElement.getBoundingClientRect();
  const w = 100 / z, h = 100 / z;
  const cx = 50 - (+img.dataset.zx || 0) / (box.width || 1) / z * 100, cy = 50 - (+img.dataset.zy || 0) / (box.height || 1) / z * 100;
  Object.assign(mini.querySelector('.vzm-view').style, { width: w + '%', height: h + '%', left: (cx - w / 2) + '%', top: (cy - h / 2) + '%' });
}
/* ── 확대(돋보기) = **지금 보고 있는 화면을 큰 팝업으로** (2026-09-16 구두) ──────────
   분할 수와 무관하다. 1분할이면 그 영상 하나, 2·3·4분할이면 분할 배치 그대로 띄운다.
   칸 하나만 크게 보는 확대는 **더블클릭**으로 옮겼다(아래 dblclick 처리). */
function stageTiles() {
  /* 지금 화면에 깔린 영상 칸을 DOM 에서 그대로 걷는다 — 멀티 뷰든 경로 비교든 같은 방식 */
  const sel = document.querySelector('#dtMulti') && document.querySelector('#dtMulti').offsetParent !== null
    ? '#dtMulti .mv-tile' : '#cmpGrid .cmp-tile';
  return [...document.querySelectorAll(sel)]
    .filter(t => t.getClientRects().length && !t.classList.contains('empty') && !t.classList.contains('off'))
    .map(t => {
      const img = t.querySelector('.cmp-vid > img, :scope > img');
      const nm = t.querySelector('.nm'), tm = t.querySelector('.tm');
      return { img: img ? img.getAttribute('src') : '', cam: (nm && nm.textContent || '').trim(), range: (tm && tm.textContent || '').trim() };
    })
    .filter(t => t.img);
}
function openStageGrid(tiles) {
  /* 2026-09-16 구두 : **채널 수 그대로** 배치한다 (2채널이면 2칸만, 2×2 로 늘리지 않는다).
     영상 보기 모드(원본/맞추기/자르기)는 공통 제어로 **우측 하단**에 둔다. */
  $('#vwBody').innerHTML = `<div class="vw">
    <div class="vw-stage vw-grid" data-n="${tiles.length}">${tiles.map(s => `<div class="vg-tile"><img src="${s.img}" alt="">
      <div class="vw-head"><span class="nm">${s.cam}</span><span class="tm">${s.range}</span><span class="vh-ic">${VH_BM}</span></div></div>`).join('')}</div>
    <div class="vw-prog" title="재생 위치"><i class="buf" style="width:70%"></i><i class="cur" style="width:27%"></i></div>
    <div class="dt-ctrl vw-ctrl"><div class="grp l"></div><div class="grp c">${playCtrlHTML()}</div>
      <div class="grp r"><button class="btn-icon" data-vratio-all data-ratio-state="원본 비율" title="화면 비율 변경 (원본 비율)"><i class="i i-24 i-g-ratio-original"></i></button></div></div>
  </div>`;
...
function openCurrentViewPopup(btn) {
  const tiles = stageTiles();
  if (tiles.length > 1) { openStageGrid(tiles); return; }   /* 2·3·4분할 → 배치 그대로 */
  /* 1분할 → 지금 보고 있는 영상 하나 */
  const box = btn.closest('.cmp-tile, .mv-tile, .pn-vid, .dt-video, .dt-pane');
  const img = (box && box.querySelector('.cmp-vid > img, img')) || document.getElementById('dtVideoImg');
  const camEl = (box && box.querySelector('.nm')) || document.getElementById('dtCam');
  const cam = ((camEl && camEl.textContent) || '영상').trim();
  openVideoView({ cam, img: img ? img.getAttribute('src') : '' });
}
document.addEventListener('click', e => {
  const zb = e.target.closest('[data-vzoom]');
  const pop0 = document.getElementById('vZoomPop');
  if (!zb) { if (pop0 && !e.target.closest('#vZoomPop')) pop0.hidden = true; return; }
  e.stopPropagation();
  if (pop0) pop0.hidden = true;   /* 종전 배율 팝오버가 떠 있으면 닫는다 */
  openCurrentViewPopup(zb);
}, true);
/* 영상을 **두 번 누르면 크게 보기**(팝업) (2026-09-16 구두 · 배율 확대에서 바뀜).
   - 확대(돋보기) 버튼 = 지금 보고 있는 **화면 전체**를 팝업 (분할이면 배치 그대로)
   - 두 번 클릭      = **그 칸 하나**를 크게 보기 팝업
   - 배율 확대/축소   = **휠** (그대로)
   ※ 영상 칸 위에서는 네이티브 `dblclick` 이 문서까지 오지 않아(칸 쪽에서 먼저 먹는다)
     결과 카드와 같은 방식으로 **클릭 수를 직접 센다.** 첫 클릭의 기존 동작은 그대로 둔다. */
const TILE_DBL = 400;
let tileClick = { key: null, t: 0 };
function vZoomTile(e) {
  if (!e.target.closest) return null;
  if (e.target.closest('.pipc, .mv-head, .cmp-head, .dt-vhead, .dt-ctrl, #dtCtrl, #cmpCtrl, .v-zoom-mini, .v-more-menu, .area-bar, button, input, select')) return null;
  const mv = e.target.closest('#dtMulti .mv-tile[data-mv]:not(.empty)');
  const cmp = !mv && e.target.closest('#cmpGrid .cmp-tile[data-tile]:not(.noclip):not(.off)');
  const single = !mv && !cmp && e.target.closest('#dtVideo:not(.has-multi)');
  const img = mv ? mv.querySelector(':scope > img')
    : cmp ? cmp.querySelector('.cmp-vid > img')
    : (single && !DT.area) ? document.getElementById('dtVideoImg') : null;
  if (!img) return null;
  /* 다시 그려도 같은 칸인지 알아볼 수 있게 요소가 아니라 키로 센다 */
  const key = mv ? 'mv' + mv.dataset.mv : cmp ? 'cmp' + cmp.dataset.tile : 'single';
  const box = mv || cmp || single;
  const camEl = (box && box.querySelector('.nm')) || document.getElementById('dtCam');
  const cam = ((camEl && camEl.textContent) || '영상').trim();
  return { img, key, cam };
}
document.addEventListener('click', e => {
  const hit = vZoomTile(e);
  if (!hit) { tileClick = { key: null, t: 0 }; return; }
  const now = Date.now();
  if (tileClick.key === hit.key && now - tileClick.t < TILE_DBL) {
    tileClick = { key: null, t: 0 };
    /* 두 번 누르면 그 칸을 **크게 보기** — 배율 확대가 아니다 (2026-09-16 구두) */
    openVideoView({ cam: hit.cam, img: hit.img.getAttribute('src') || '' });
    return;
  }
  tileClick = { key: hit.key, t: now };
}, true);
/* 확대 모드에서 휠 : 한 단계씩, 팝오버 값과 연동 */
document.addEventListener('wheel', e => {
  if (!e.target.closest) return;
  /* 영상 위에서 휠 = 그 영상 확대/축소 — 멀티 뷰 칸(2026-09-14)만 되던 것을
     단일(1분할) 영상 · 경로 비교 칸(2·3·4명)까지 같은 동작으로 (2026-09-15 구두).
     재생 바 · 머리말 · 주변 카메라 · 축소 지도 · 떠 있는 패널 위에서는 확대하지 않는다 */
  const onUi = e.target.closest('.pipc, .mv-head, .cmp-head, .dt-vhead, .dt-ctrl, #dtCtrl, #cmpCtrl, .v-zoom-mini, .v-more-menu, #pnlTracks, .pn-tools, .area-bar, button, input, select');
  let img = e.target.closest('.v-zoomed');
  if (!img && !onUi) {
    const mv = e.target.closest('#dtMulti .mv-tile[data-mv]:not(.empty)');
    const cmp = !mv && e.target.closest('#cmpGrid .cmp-tile[data-tile]:not(.noclip):not(.off)');
    const single = !mv && !cmp && e.target.closest('#dtVideo:not(.has-multi)');
    img = mv ? mv.querySelector(':scope > img')
      : cmp ? cmp.querySelector('.cmp-vid > img')
      : single && !DT.area ? document.getElementById('dtVideoImg') : null;
  }
  if (!img) return;
  e.preventDefault();
  const cur = V_ZOOMS.indexOf(img.dataset.zlabel || '1.0');
  const next = Math.max(0, Math.min(V_ZOOMS.length - 1, cur + (e.deltaY < 0 ? 1 : -1)));
  applyVZoom(img, V_ZOOMS[next]);
  const pop = document.getElementById('vZoomPop');
  if (pop && !pop.hidden) pop.querySelectorAll('[data-vz]').forEach(b => b.classList.toggle('on', b.dataset.vz === V_ZOOMS[next]));
}, { passive: false });
/* ESC : 확대 모드 즉시 해제(1.0) */
document.addEventListener('keydown', e => {
  if (e.key !== 'Escape') return;
  document.querySelectorAll('img.v-zoomed').forEach(img => applyVZoom(img, '1.0'));
  const pop = document.getElementById('vZoomPop'); if (pop) pop.hidden = true;
});
try { S.noAskTabClose = localStorage.getItem('svms.noAskTabClose') === '1'; } catch (_) {}

/* ── 영상 클립 편집 모드 (사양 v0.8 PTS_DTL_002_1 영상 클립 편집) ─────────────
   상세 = DT.edit / 경로 비교 = CMP.edit. 진입 시 모든 트랙 구간을 스냅샷해
   변경 여부로 `저장` 활성, `취소` 는 변경이 있을 때만 편집 취소 alert. ESC = 취소 */
function tlEditCmp() { const t = S.tabs.find(x => x.id === S.activeTab); return !!(t && t.kind === 'compare'); }
function tlEditOn() { return tlEditCmp() ? !!CMP.edit : !!DT.edit; }
function tlSnap() { return JSON.stringify(TL_TRACKS.map(t => t.clips.map(c => [c.from, c.to]))); }
function tlEditDirty() { return !!TL._snap && TL._snap !== tlSnap(); }
function tlEditSet(on) {
  if (tlEditCmp()) CMP.edit = on; else DT.edit = on;
  if (!on) TL._snap = null;
  const pv = document.getElementById('tlCurPv'); if (pv) pv.hidden = true;
  if (typeof syncPanels === 'function') syncPanels();
}
function tlEditStart() { TL._snap = tlSnap(); tlEditSet(true); }
function tlEditCancel() {
  if (!tlEditDirty()) { tlEditSet(false); return; }
  alertSpec('editCancel', () => {
    const snap = JSON.parse(TL._snap);
    TL_TRACKS.forEach((t, i) => t.clips.forEach((c, j) => { if (snap[i] && snap[i][j]) { c.from = snap[i][j][0]; c.to = snap[i][j][1]; } }));
    tlEditSet(false);
  });
}
function tlEditSave() {
  if (!tlEditDirty()) return;
  tlEditSet(false);
  toast('저장되었습니다.');
}
document.addEventListener('keydown', e => {
  if (e.key !== 'Escape' || !tlEditOn()) return;
  if ([...document.querySelectorAll('.modal')].some(m => !m.hidden)) return;
  tlEditCancel();
});

/* v0.8 MAP_000_2 카메라 아이콘 유형 : 현재 재생 · 대상 포착 · 대상 미포착 */
function coneType(cam) {
  const lb = document.getElementById('dtCam');
  if (lb && lb.textContent.trim() === cam) return 'play';
  const hit = (MAP_PATHS_CACHE || []).some(p => p.pts.some(t => t.cam === cam));
  return hit ? 'hit' : 'miss';
}

/* v0.8 : 영역 검색 · 가상펜스 드로잉 모드는 ESC 로 해제 */
document.addEventListener('keydown', e => {
  if (e.key !== 'Escape' || !DT.area) return;
  if ([...document.querySelectorAll('.modal')].some(m => !m.hidden)) return;
  DT.area = null; renderArea(); applyTools(); renderNear(DT._tab);
});

/* ============================================================
   영상 레이어 (2026-09-14) — 목업 이미지를 실제 영상처럼
   재생 영역의 프레임 이미지(assets/video/vNN.jpg) 위에 같은 크기·비율·확대 상태의
   <video>(vNN.mp4, 무음 반복)를 겹친다. 클릭·휠·드래그는 그대로 아래 이미지가 받는다.
   재생/일시정지 · 배속(역방향은 되감기) · 10초 이동 · 타임라인 커서와 시각을 맞춘다.
   ============================================================ */
...
function vidPlace(img, v) {
  ...
  v.style.display = hidden ? 'none' : 'block';
...
function vidSync(img) {
  ...
  let v = img._vid;
  if (!m) { if (v) { v.remove(); img._vid = null; img.classList.remove('has-vid'); } return; }
  if (!v || !v.isConnected || v.previousElementSibling !== img) {
    if (v) v.remove();
  ...
  if (v.dataset.src !== want) { v.dataset.src = want; v.src = want; v.poster = img.getAttribute('src'); }
...
/* 스페이스바 = 재생 / 멈춤 (2026-09-16 구두).
   화면에 보이는 재생 버튼을 눌러 주므로 아이콘·상태 동기화는 기존 흐름을 그대로 탄다.
   글자를 입력하는 중이거나 버튼에 포커스가 있으면 그쪽이 우선이라 건너뛴다. */
document.addEventListener('keydown', e => {
  if (e.code !== 'Space' && e.key !== ' ') return;
  if (e.ctrlKey || e.altKey || e.metaKey) return;
  const t = e.target;
  if (t && t.closest && t.closest('input, textarea, select, button, [contenteditable="true"]')) return;
  const btn = [...document.querySelectorAll('#dtPlay, [data-playtg]')].filter(b => b.getClientRects().length)[0];
  if (!btn) return;
...
document.addEventListener('click', e => {
  ...
  setTimeout(() => {
    VID.playing = b.classList.contains('playing');
    document.querySelectorAll('#dtPlay, [data-playtg]').forEach(x => {
      if (x === b || x.classList.contains('playing') === VID.playing) return;
      x.classList.toggle('playing', VID.playing); x.title = VID.playing ? '일시정지' : '재생';
      x.innerHTML = VID.playing
        ? `<i class="i i-24 i-g-pause"></i>`
        : `<i class="i i-24 i-g-play"></i>`;
    });
    if (VID.playing) {
