...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609161108" alt="맵뷰">
...
function renderArea() {
  ...
  if (a.mode === 'line' && a.line) {
    ...
    if (a.done) {
      ...
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609161108" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609161108" alt="">
      ${poly}
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609161108' : 'assets/img/floor.png?v=202609161108';
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609161108' : 'assets/img/floor.png?v=202609161108';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609161108" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609161108" alt="외부 지도" draggable="false">
...
function renderFloorPane(host, pts, color) {
  ...
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609161108" alt=""><span class="dt-floor">${flb}</span>
...
/* 각 화면별 확대 = **더블클릭** (2026-09-16 구두) — 누를 때마다 1.0 → 2x → 3x → 4x → 1.0.
   휠 확대와 같은 대상·같은 단계를 쓴다. 머리말·조작 바·버튼 위에서는 동작하지 않는다.
   ※ 영상 칸 위에서는 네이티브 `dblclick` 이 문서까지 오지 않아(칸 쪽에서 먼저 먹는다)
     결과 카드와 같은 방식으로 **클릭 수를 직접 센다.** 첫 클릭의 기존 동작은 그대로 두고,
     짧은 시간 안에 두 번째가 오면 그때 확대만 한 단계 올린다. */
const TILE_DBL = 400;
...
document.addEventListener('click', e => {
  ...
  if (tileClick.key === hit.key && now - tileClick.t < TILE_DBL) {
    ...
    const cur = hit.img.dataset.zlabel || '1.0';
    applyVZoom(hit.img, V_ZOOMS[(V_ZOOMS.indexOf(cur) + 1) % V_ZOOMS.length]);
    return;
  }
