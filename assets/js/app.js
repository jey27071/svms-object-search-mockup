...
function openCtx(e, id) {
  ...
  /* 시안 : 카드 하단에 북마크 버튼(.cbk)이 따로 있으므로 더보기에는 북마크를 넣지 않는다.
     (2026-09-15 구두 — 썸네일 북마크와 ⋯ 안 북마크가 중복이던 것을 정리)
     사양서의 `북마크 / 오탐지 신고` 2종 중 남는 것은 오탐지 신고. 모든 검색 유형에 둔다
     — 빼면 알고리즘 외 결과에서 ⋯ 가 빈 메뉴가 되는데, 시안은 모든 카드에 ⋯ 를 두고 있다 */
  m.innerHTML = (S.mode === 'algo' ? `<div data-act="pfind">인물 검색</div>` : '') +
    `<div data-act="report">오탐지 신고</div>`;
...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609151320" alt="맵뷰">
...
function renderArea() {
  ...
  if (a.mode === 'line' && a.line) {
    ...
    if (a.done) {
      ...
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609151320" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609151320" alt="">
      ${poly}
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609151320' : 'assets/img/floor.png?v=202609151320';
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609151320' : 'assets/img/floor.png?v=202609151320';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609151320" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609151320" alt="외부 지도" draggable="false">
...
function renderFloorPane(host, pts, color) {
  ...
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609151320" alt=""><span class="dt-floor">${flb}</span>
