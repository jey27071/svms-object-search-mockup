...
function renderClipPreview() {
  ...
  host.innerHTML = `
    ...
    <div class="cp-zoom">
      ...
      <input type="range" class="cp-zr" min="1" max="4" step="1" value="${CP_ZOOM}" aria-label="미리보기 확대">
  ...
  const setZ = v => { CP_ZOOM = Math.max(1, Math.min(4, v)); if (zr) zr.value = CP_ZOOM; fitClipPreview(); };
...
function openClipReselect(host, tracks, opt, ti = 0) {
  ...
  renderClips(ids);
  openModal('#mdClips');
  requestAnimationFrame(() => renderClipStrip());
}
...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609161642" alt="맵뷰">
...
function renderArea() {
  ...
  if (a.mode === 'line' && a.line) {
    ...
    if (a.done) {
      ...
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609161642" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609161642" alt="">
      ${poly}
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609161642' : 'assets/img/floor.png?v=202609161642';
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609161642' : 'assets/img/floor.png?v=202609161642';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609161642" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609161642" alt="외부 지도" draggable="false">
...
function renderFloorPane(host, pts, color) {
  ...
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609161642" alt=""><span class="dt-floor">${flb}</span>
