...
function pickSideHTML(opt) {
  ...
  return `
    ...
    <div class="vs-filter">
      ...
      <button type="button" class="btn-primary sm vf-go" id="vsResearch"${vsDirty() ? '' : ' disabled'}>재검색</button>
    </div>`;
}
...
function paneBody(kind, i) {
  if (kind === 'map') {
    return `<div class="pn-map">
      <img src="assets/img/floor.png?v=202609161525" alt="맵뷰">
...
function renderArea() {
  ...
  if (a.mode === 'line' && a.line) {
    ...
    if (a.done) {
      ...
      h += `<button class="area-dir" data-abarrow title="눌러서 방향 바꾸기" style="left:${(x1 + x2) / 2}%;top:${(y1 + y2) / 2}%"><img src="assets/img/area-direction.svg?v=202609161525" alt="" style="transform:rotate(${ang + 45}deg)"></button>`;
...
function renderMap3d(paths) {
  ...
  host.innerHTML = M3_FLOORS.slice(0, 4).map((f, fi) => {
    ...
    return `<div class="m3-floor${pl.some(({ pts }) => onFl(pts).length) ? '' : ' dim'}" data-fl="${f.label}" style="--i:${fi};z-index:${M3_FLOORS.length - fi}">
      <img src="assets/img/floor.png?v=202609161525" alt="">
      ${poly}
...
$$('#dtMapSeg button').forEach(b => b.onclick = () => {
  ...
  DT.map = b.dataset.m;
  $('#dtMapImg').src = DT.map === 'map' ? 'assets/img/map.png?v=202609161525' : 'assets/img/floor.png?v=202609161525';
...
function renderMapView() {
  ...
  const mapImg = DT.map === 'map' ? 'assets/img/map.png?v=202609161525' : 'assets/img/floor.png?v=202609161525';
  ...
  const pth = `<div style="display:flex;gap:10px">
      ...
      <div style="width:300px;flex:0 0 300px">
        ...
        <div style="position:relative;height:196px;border-radius:6px;overflow:hidden;background:var(--bg-1);border:1px solid var(--ln-subtle)">
          <img src="assets/img/map.png?v=202609161525" style="width:100%;height:100%;object-fit:cover;opacity:.85" alt="">
          ${c.path.map((p, i) => {
...
function renderZoneMap(host, pts, color) {
  ...
  zm.innerHTML = `<div class="zm-stage"><img src="assets/img/map.png?v=202609161525" alt="외부 지도" draggable="false">
...
function renderFloorPane(host, pts, color) {
  ...
  fp.innerHTML = `<div class="zp-stage"><img src="assets/img/floor.png?v=202609161525" alt=""><span class="dt-floor">${flb}</span>
